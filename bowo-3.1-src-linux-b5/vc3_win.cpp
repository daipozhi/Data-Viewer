	...
	if (bw_buffa1.get_tmpc1(p_pagecnt,i,j)<=' ')
	{
	  ...
	  if (bw_buffa1.get_tmpc1(p_pagecnt,i,j)!=' ')
	  {
		...
		if (s2[0]=='@')
		{
		  ...
		  w_cre_ci_butt(p_pagecnt,s1,SMG_LEN,p6,p7,p8,i,s2,strlen(s2));
...
int bw_win::w_cre_ci_butt(int p_pagecnt,char *p_s1,int len,int p6,int p7,int p8,int i,char *p_s2,int len2)
{
  HWND hnd;
  int  l,j;
  //char s2[SMG_LEN];
...
*/
  ...
  if (bw_win_bua1.get_win_ci_mode(p_pagecnt)==0)
  {
    ...
    if (bw_win_bua1.get_win_hnd(win_hpp5)==0) bw_main1.win_cre_ok=0;
  }
  else
  {
    hnd=CreateWindow("button",s3
