...
//  char  c1;
  ...
  while (1)
  {
    ...
    if (j<bw_xbase_bua1.get_win_lcnt(p_fcnt,1)+1) break;
...
*/
  ...
  /* copy back */
  s_fh=/*s*/open(s_fn2,O_RDWR/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  if (s_fh<0) return(1);
...
int bw_xbase::w_append_fox(int p_fcnt)
{
  ...
  while (1)
  {
    ...
    t=bw_xbase_bua1.read_win_recvar(win_use_fh,s);
