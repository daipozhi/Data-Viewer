...
int  bw_bmp::DIB(char *p_bfn,int p_bfn_len)
{
  //char s_fn4[FN_SIZE];
  int                fh;
...
int bw_bmp::echo_bmp(HWND hwnd,char *comm,int comm_len,int mcnt,int wpp)
{
  FILE *s_fp;
  int  i,j,k,l,m;
  int  exist;
...
//  char str[SMG_LEN];
  ...
  bw_main1.set_string(s_fn4,SMG_LEN,comm,comm_len);
...
int bw_bmp::w_bmp_add(int p_fcnt,int wpp,char *p_fldname,int p_fldname_len)
{
  int  i,j,k;
  char str[SMG_LEN];
  char sfld[SMG_LEN];
...
//  char               str[SMG_LEN];
  ...
  switch(msg)
  {
    case WM_PAINT:
    {
      ...
      bw_xbase1.get_win_bmp_name(s1,SMG_LEN);
