...
int bw_fldstru::w_read_stru(char *p_fn1,int p_fn1_len,int p_stn)
{
  int  exist;
  int  i,j,k,l;
  ...
  while (!feof(fp1))
  {
    ...
    bw_inkey1.strpcut(tmpc2,p4,p5-p4+1,SMG_LEN,str,SMG_LEN);
    l=bw_inkey1.str2int(str,SMG_LEN);
    ...
    bw_inkey1.strpcut(tmpc2,p6,p7-p6+1,SMG_LEN,str,SMG_LEN);
    l=bw_inkey1.str2int(str,SMG_LEN);
...
int bw_fldstru::w_find_fldnum(char *p_str,int p_str_len,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_LEN];
...
int bw_fldstru::w_find_fldlen(char *p_str,int p_str_len,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_LEN];
...
int bw_fldstru::w_find_fldtype(char *p_str,int p_str_len,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_LEN];
...
int bw_fldstru::w_find_flddec(char *p_str,int p_str_len,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_LEN];
...
int bw_fldstru::w_find_fldposi(char *p_str,int p_str_len,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_LEN];
...
int bw_fldstru::w_find_fldname(int p1,int p2,char *p_str,int p_str_len)
{
  char stc[SMG_LEN];
  ...
  else
  {
    ...
    bw_main1.set_string(p_str,p_str_len,stc,SMG_LEN);
  }
...
int bw_fldstru::w_echo_winrec(int wpp)
{
  ...
  {
    if ((bw_win1.get_win_pp9(k,wpp)!=1)||(bw_xbase1.get_win_lnktab(k)!=0)) continue;
...
int bw_fldstru::w_tv_get_read_paint(char *p_str,int p_str_len,int wpp,int l,int c,char comm)
{
  HWND hnd;
  int  i,j,q;
  char s1[SMG_LEN];
  ...
  bw_inkey1.strpcut(p_str,1,strlen(p_str)-1,p_str_len,s2,SMG_LEN);
  ...
  if ((w_find_fldtype(s2,SMG_LEN,j)=='n')||(w_find_fldtype(s2,SMG_LEN,j)=='c'))
  {
    ...
    bw_xbase1.set_win_scrnvar(wpp,s1,SMG_LEN);
  ...
  else
  {
    if (w_find_fldtype(s2,SMG_LEN,j)=='l')
    {
      ...
      bw_xbase1.get_win_scrnvar(wpp,s1,SMG_LEN);
    ...
    else
    {
      if (w_find_fldtype(s2,SMG_LEN,j)=='m')
      {
        ...
        bw_xbase1.set_win_memno(j,s1,SMG_LEN);
	...
	bw_inkey1.strtrim(s1,SMG_LEN);

	bw_xbase1.set_win_bmp_name(s1,SMG_LEN);
  ...
  if (p_str[0]==':')
  {
    bw_xbase1.get_win_scrnvar(wpp,s3,SMG_LEN);
    ...
    j=bw_getread1.get_read(l-bw_win1.get_win_con(wpp,1)+1,
			 c-bw_win1.get_win_con(wpp,0)+1,
		       ...
		       bw_win1.get_win_pp11(wpp));

  }

  return(j);
}
int bw_fldstru::w_tv_get_read(char *p_str,int p_str_len,int wpp,int l,int c,char comm)
{
  HWND hnd;
  int  i,j,q;
  char s1[SMG_LEN];
...
int bw_fldstru::w_tv_get_read1(char *p_str,int p_str_len,int wpp,int l,int c,char comm)
{
  int  i,j;
  char s2[SMG_LEN];
  char s3[SMG_LEN];
...
int bw_fldstru::w_get_tab_var(char *p_fn,int p_fn_len,int p_fcnt,char *p_ca,int p_ca_len)
{
  int  i,j,k;
//  char str[SMG_LEN];
//  HDC  hdc;
  ...
  i=w_find_fldnum(p_fn,p_fn_len,p_fcnt);
  if (i!=0) return(1);
  j=w_find_fldlen(p_fn,p_fn_len,p_fcnt);
  k=w_find_fldposi(p_fn,p_fn_len,p_fcnt);
...
int bw_fldstru::w_cmp_tab_var(char *p_fn,int p_fn_len,int p_fcnt,char *p_ca,int p_ca_len)
{
  int  exist;
  int  i,j,k,l;
  ...
  if (i!=0) return(1);
  j=strlen(p_ca);
  l=w_find_fldlen(p_fn,p_fn_len,p_fcnt);
  k=w_find_fldposi(p_fn,p_fn_len,p_fcnt);
  if (j!=l) return(1);
...
int bw_fldstru::w_sav_tab_var(char *p_fn,int p_fn_len,int p_fcnt,char *p_ca,int p_ca_len)
{
  int  i,j,k,l;
  char c1;
  ...
  if (p_ca_len>0) p_ca[p_ca_len-1]=0;
...
//  char str[SMG_LEN];
  ...
  else
  {
	...
	if (bw_xbase1.win_state3==1) bw_xbase1.set_win_recno(p_fcnt,wpp,1,1);
	break;
      }
    }
...
//  char str[SMG_LEN];
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    for (i=1;i<=s_tmpn2;i++)
    {
      ...
      r1=r2;
      if (bw_xbase1.win_state3==1) return(0);
    }
...
int bw_fldstru::w_link_window(int p_fcnt,int wpp)
{
  ...
  if (bw_xbase1.get_win_state9(wpp,p_fcnt)==1) return(1);

  i=w_is_card(p_fcnt,wpp);

  j=bw_win1.get_win_pp6(bw_win1.get_win_pp5(wpp),0);
...
//  int   i,j,k;
  ...
  bw_main1.get_view(sfn1,FN_SIZE,bw_main1.win_hwnd,bw_win1.get_win_pp6(wpp,0));
...
int bw_fldstru::w_find_recno(int p_fcnt,int wpp)
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    if (bw_xbase1.get_win_state9(wpp,i)==1)
    {
      exist=1;
      break;
    }
  ...
  if (exist==1)
  {
    ...
    bw_xbase1.set_win_state9(wpp,i,0);
  ...
  bw_xbase1.set_win_state9(wpp,p_fcnt,1);
...
//  int   exist;
     ...
     ;i++)
  {
    ...
    {
      str2[0]=0;
      bw_dialog1.w_mv_save_mem(str,SMG_LEN,zero,str2,SMG_LEN);
    }
...
int bw_fldstru::w_find_end(int p_fcnt,int wpp)
{
  ...
  if (bw_xbase1.get_win_state9(wpp,p_fcnt)==1)
  {
    close(bw_xbase1.win_fp2[wpp]);
    ...
    bw_xbase1.set_win_state9(wpp,p_fcnt,0);
...
int bw_fldstru::w_find_exit(int wpp)
{
      ...
      ;p1++)
  {
    ...
    {
      if (bw_xbase1.get_win_state9(p1,p2)==1)
      {
        close(bw_xbase1.win_fp2[p1]);
        bw_xbase1.set_win_state9(p1,p2,0);
...
int bw_fldstru::w_find_cond_ok(int p_fcnt)
{
     ...
     ;i++)
  {
    ...
    if ((bw_dialog1.w_mv_exist(stra,SMG_LEN,zero)==1)||(bw_dialog1.w_mv_exist(str4,SMG_LEN,zero)==1))
    {
      ...
      bw_inkey1.strtrim(str1,SMG_LEN);
      bw_inkey1.strltrim(str1,SMG_LEN);
      ...
      {
	k=bw_inkey1.strlocc(str1,SMG_LEN,';');
	...
	else
	{
      ...
      {
        f1=bw_inkey1.str2float(str2,SMG_LEN);
	...
	while (1)
	{
	  ...
	  else
	  {
            ...
            else
	    {
	      ...
	      f2=bw_inkey1.str2float(str3,SMG_LEN);
...
int bw_fldstru::w_edit_memo(int p_fcnt,int wpp,char *p_fldname,int p_fldname_len)
{                           
  char fn1[FN_SIZE];
  char str[SMG_LEN];
  char str1[SMG_LEN];
...
//  char stra[SMG_LEN];
  ...
  if (bw_inkey1.w_find_file(fn1)==0)
  {
    ...
    bw_xbase1.get_win_memno(p_fcnt,str9,SMG_LEN);
    ...
    if (bw_inkey1.strempty(str9)==0)
    {
      ...
      mno=0;
      mark=1;
      bw_inkey1.long2str(mno,str9,SMG_LEN);
    }
    else
    {
      mno=bw_inkey1.str2long(str9,SMG_LEN);
      mark=0;
      mnu=mno;
    }
...
//    fclose(fh);
    ...
    strcpy(str3,str2);
    bw_xbase1.get_win_mem_ext(p_fcnt,str8,SMG_LEN);
    strcat(str3,str8);
  ...
  else
  {
    ...
    if (exist==1)
    {
      ...
      bw_inkey1.strtrim(str,SMG_LEN);
      mnu=bw_inkey1.str2long(str,SMG_LEN);
    }
    else
    {
      fseek(fh,0,SEEK_END);
      ...
      {
	mnu=bw_inkey1.str2long(str9,SMG_LEN);
      }
    ...
    bw_inkey1.strtrim(str5,SMG_LEN);
    ...
    while (!feof(fh))
    {
      ...
      bw_inkey1.strtrim(str4,SMG_LEN);
...
//      fclose(fh);
  ...
  if (mark==1)
  {
    ...
    else
    {
      ...
      bw_inkey1.long2str(mnu,str,SMG_LEN);
...
int bw_fldstru::w_edit_memo_add(int p_fcnt,int wpp,char *p_fldname,int p_fldname_len)
{
/*
  int  i,j,k,l,m,n;
  char s_tc0[SMG_LEN];
...
//  FILE *fh1;
  ...
  strcpy(s_tc7,st2);
  bw_inkey1.strtrim(s_tc7,SMG_LEN);
  i=bw_inkey1.strlocc(s_tc7,SMG_LEN,'.');
  s_tc7[i]=0;
  ...
  bw_xbase1.get_win_memno(p_fcnt,str8,SMG_LEN);
  ...
  if (bw_inkey1.w_find_file(fn1)==1)
  {
    ...
    if (bw_inkey1.strempty(str8)==0)
    {
      ...
      mno=mnu;
      mark=1;
      bw_inkey1.long2str(mno,str8,SMG_LEN);
    }
    else
    {
      mno=bw_inkey1.str2long(str8,SMG_LEN);
      mark=0;
    }
  ...
  bw_xbase1.set_win_mem_inuse(p_fcnt,1);
  bw_xbase1.set_win_mem_fn(p_fcnt,s_tc8,SMG_LEN);
  bw_win1.set_win_memo_basep(wpp,p_fcnt);
...
int bw_fldstru::w_wps_to_txt(char *p_s1,int p_s1_len,char *p_s2,int p_s2_len)
{
  FILE     *s_fp1;
  FILE     *s_fp2;
  unsigned char c1;
...
int bw_fldstru::get_t2_fldname(int fbpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_fldstru::set_t2_fldname(int fbpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_fldstru::w_last_line(int p_fcnt,int wpp)
{
  ...
  for (i=bw_win1.pline+1;i<LIN_NUM;i++) /* find next screen field */
  {
	...
	l=bw_win1.get_1fldbase(k);
        bw_win1.get_1fldname(k,str,SMG_LEN);
	if ((str[0]==':')&&(l==p_fcnt)&&(m==0)) { exist=1; break; }
/*
      if (bw_buff1.get_linecol(wpp,i,j)>0)
      {
