...
//  long li;
  ...
  if (bw_xbase1.get_win_state5(l)==1)
  {
    ...
    //sprintf(str,"atten9 return=%d,",i);
    //bw_getread1.deb_record(str);

    if (i==2) 
    { /* not save file*//*w_mrollback_base(l,wpp);*/

      //sprintf(str,"not save");
      //bw_getread1.deb_record(str);
...
//  char c1;
  ...
  //bw_getread1.deb_record("open xbase");

  m=bw_xbase1.w_open_xbase(p_bn,p_bn_len,o,l);

  if (m!=0) return(1);

  //bw_getread1.deb_record("open xbase ok");
