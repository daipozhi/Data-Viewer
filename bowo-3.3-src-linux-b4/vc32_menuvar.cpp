...
int bw_menuvar::w_read_mdc(char *p_fn1,int p_fn1_len,int p_win)
{
  int  i,j;
...
//  int  state5;
  ...
  while (!feof(fp1))
  {
    ...
    get_t5_m_button(pp1,sb1,SMG_LEN);
...
int bw_menuvar::tst_menu()
{
  ...
  for (i=0;i<60;i++)
  {
    get_t5_m_name(i,sm1,SMG_LEN);
    get_t5_m_button(i,sb1,SMG_LEN);
    get_t5_m_comm(i,so1,SMG_LEN);
    get_t5_m_focus(i,sf1,SMG_LEN);
...
int bw_menuvar::w_dele_menu(int wp1,int wp2)
{
  ...
  else
  {
    if (p3>=p2)
    {
      ...
      {
	get_t5_m_name(i,sm1,SMG_LEN);
        set_t5_m_name(p1+i-p2,sm1,SMG_LEN);

	get_t5_m_button(i,sb1,SMG_LEN);
	set_t5_m_button(p1+i-p2,sb1,SMG_LEN);

	get_t5_m_comm(i,so1,SMG_LEN);
	set_t5_m_comm(p1+i-p2,so1,SMG_LEN);

	get_t5_m_focus(i,sf1,SMG_LEN);
	set_t5_m_focus(p1+i-p2,sf1,SMG_LEN);
...
int bw_menuvar::w_menu_comm(int p1,int state)
{
  ...
  t5_m_pp2=0;
  t5_m_como[0]=0;
  if (state==0) get_t5_m_comm(p1,s1,SMG_LEN);
  else get_t5_m_focus(p1,s1,SMG_LEN);
...
int bw_menuvar::w_menu_poin(char *p_s1,int p_s1_len,int p1)
{
  int  i;
  int  exist;
  char sm1[SMG_LEN];
...
int bw_choose::w_read_cdc(char *p_fn1,int p_fn1_len,int wpp)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
//  int  state2,state3;
//  int  state4,state5;
  ...
  while (!feof(fp1))
  {
    ...
    bw_inkey1.strpcut(tmpc2,p1+1,p2-p1-1+1,SMG_LEN,str2,SMG_LEN);
    o=bw_inkey1.str2int(str2,SMG_LEN);
    k=pp1;
    bw_choose1.set_win_choose(k,0,1);
    bw_choose1.set_win_choose(k,2,o);
    bw_choose1.set_win_choose(k,3,wpp);  //wpp to 0
    ...
    bw_inkey1.strpcut(tmpc2,p5,p6-p5+1,SMG_LEN,str2,SMG_LEN);
    l=bw_inkey1.str2int(str2,SMG_LEN);
    bw_choose1.set_win_choose(k,1,l);

    bw_choose1.get_win_choose_name(k,sc1,SMG_LEN);

    if (bw_dialog1.w_mv_exist(sc1,SMG_LEN,wpp)==1)
    {
      bw_dialog1.w_mv_fet_val(sc1,SMG_LEN,0,str,SMG_LEN);
      //bw_inkey1.strpcut(str,0,strlen(str)-1-0+1,SMG_LEN,str2,SMG_LEN);
      bw_choose1.set_win_choose(k,1,bw_inkey1.str2int(str,SMG_LEN));
    }
...
int bw_choose::tst_choose()
{
  ...
  for (i=0;i<60;i++)
  {
    bw_choose1.get_win_choose_name(i,sc1,SMG_LEN);
...
/*
  ...
  else
  {
    if (p3>=p2)
    {
      ...
      {
	bw_choose1.get_win_choose_name(i,sc1,SMG_LEN);
	bw_choose1.set_win_choose_name(p1+i-p2,sc1,SMG_LEN);
...
int bw_choose::w_save_choose(int wpp)
{
  ...
  //for (j=bw_win1.get_win_pp6(bw_win1.get_win_pp5(wpp),0);j<=bw_win1.get_win_pp6(bw_win1.get_win_pp5(wpp),2);j++)
  //{
    for (i=0;i<win_choose_pp1;i++)
    {
      //if (bw_choose1.get_win_choose(i,3)==wpp)
      //{
	bw_inkey1.int2str(bw_choose1.get_win_choose(i,1),str,SMG_LEN);
        bw_choose1.get_win_choose_name(i,sc1,SMG_LEN);
	bw_dialog1.w_mv_save_mem(sc1,SMG_LEN,wpp,str,strlen(str));
      //}
    }
  //}
  return(0);
...
int bw_choose::w_echo_winrec_chs(int wpp)
{
  ...
  while (1)
  {
    ...
    if (str[0]=='[')
    {
      bw_inkey1.strpcut(str,1,strlen(str)-1-1+1,SMG_LEN,str2,SMG_LEN);
      n=bw_inkey1.str2int(str2,SMG_LEN);
...
int bw_switch::w_read_sdc(char *p_fn1,int p_fn1_len,int wpp)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
//  int  state2,state3;
//  int  state4,state5;
  ...
  while (!feof(fp1))
  {
    ...
    set_win_switch(k,3,wpp);  // wpp to 0
    ...
    get_win_switch_name(k,st1,SMG_LEN);

    if (bw_dialog1.w_mv_exist(st1,SMG_LEN,wpp)==1)
    {
      bw_dialog1.w_mv_fet_val(st1,SMG_LEN,0,str,SMG_LEN);
      //bw_inkey1.strpcut(str,0,strlen(str)-1-0+1,SMG_LEN,str2,SMG_LEN);
      set_win_switch(k,1,bw_inkey1.str2int(str,SMG_LEN));
    }
...
int bw_switch::w_save_switch(int wpp)
{
  ...
  //{
    for (i=0;i<win_switch_pp1;i++)
    {
      ...
      //{
	bw_inkey1.int2str(get_win_switch(i,1),str,SMG_LEN);
	get_win_switch_name(i,st1,SMG_LEN);
	bw_dialog1.w_mv_save_mem(st1,SMG_LEN,wpp,str,strlen(str));
      //}
    }
  //}
...
/*
  ...
  else
  {
    ...
    {
      get_win_switch_name(i,st1,SMG_LEN);
      set_win_switch_name(p1+i-p2,st1,SMG_LEN);
...
int bw_dialog::w_read_ddc(char *p_fn1,int p_fn1_len,int wpp)
{
  int  i,j,k,l;
...
//  int  exist;
  ...
  while (!feof(fp1))
  {
	 ...
	 if (w_mv_exist(s_tmpc1,SMG_LEN,0)==0) k=w_mv_add_name(s_tmpc1,SMG_LEN);  //wpp2 to 0
	 else k=w_mv_seri(s_tmpc1,SMG_LEN,0);  //wpp2 to 0
	 ...
	 if (k>=0)
	 {
	   ...
	   bw_inkey1.strpcut(tmpc2,p7,p8-p7+1,SMG_LEN,str2,SMG_LEN);
	   l=bw_inkey1.str2int(str2,SMG_LEN);
	   set_win_mv_type(k,2,l);

	   bw_inkey1.strpcut(tmpc2,p9,p10-p9+1,SMG_LEN,str2,SMG_LEN);
	   l=bw_inkey1.str2int(str2,SMG_LEN);
	   set_win_mv_type(k,3,l);
	   set_win_mv_type(k,4,0);  // wpp2 to 0

	   if (p11==0)
	   {
             s_tmpc4[0]=0;
	     w_mv_add_val(s_tmpc1,SMG_LEN,wpp2,s_tmpc4,SMG_LEN); // wpp2 to 0
	   }
	   else
	   {
		 ...
		 s_tmpc2[j-p11+1]=0;
	       ...
	       w_mv_add_val(s_tmpc1,SMG_LEN,wpp2,s_tmpc3,SMG_LEN); //wpp2 to 0
	     }
	     else
	     {
               s_tmpc4[0]=0;
           ...
           if (w_mv_exist(s_tmpc1,SMG_LEN,wpp2)==1)
           {
             w_mv_fet_val(s_tmpc1,SMG_LEN,   0,str,SMG_LEN);
             w_mv_add_val(s_tmpc1,SMG_LEN,wpp2,str,SMG_LEN); //wpp2 to 0
	   }
...
/*
  for (j=bw_win1.get_win_pp6(bw_win1.get_win_pp5(wpp),0);j<=bw_win1.get_win_pp6(bw_win1.get_win_pp5(wpp),2);j++)
  {
    for (i=0;i<=win_mv_pp1;i++)
    {
      ...
      {
        get_win_mv_name(i,sv1,SMG_LEN);
	k=w_mv_fet_val(sv1,SMG_LEN,j,str,SMG_LEN);
        l=get_win_mv_type(i,2);
	if (k==0) w_mv_save_mem(sv1,SMG_LEN,wpp,str,l);
      }
    }
  }
*/
...
int bw_dialog::w_mv_save_mem(char *p_str,int p_str_len,int wpp,char *val,int val_len)
{
  int  i,k;
  char sd1[SMG_LEN];
  ...
  //sprintf(sd1,"(((into w_mv_save_mem(),p_str=%s,p_str_len=%d,wpp=%d,val=%s,val_len=%d,",p_str,p_str_len,wpp,val,val_len);
  //bw_getread1.deb_record(sd1);

  bw_main1.set_string(sd1,SMG_LEN,val,val_len);

  val_len=(int)strlen(sd1);

  if (val_len<=0) // set empty string
  {
	  sd1[0]=' ';
	  sd1[1]=0;
	  val_len=1;
  ...
  if (w_mv_exist(p_str,p_str_len,wpp)==0)
  {
    k=w_mv_add_name(p_str,p_str_len);
    if (k>=0)
    {
      set_win_mv_type(k,0,2);            /* edit able   */
      set_win_mv_type(k,1,'c');          /* var type    */
      set_win_mv_type(k,2,val_len);      /* var len     */      //remember
  ...
  {
    k=w_mv_seri(p_str,p_str_len,wpp);
    if (k>=0)
    {
      set_win_mv_type(k,2,val_len);         /* var len */      //remember
    }
  ...
  w_mv_add_val(p_str,p_str_len,wpp,sd1,val_len);
...
/*
  ...
  else
  {
    if ((p2>=0)&&(p3>=p2))
    {
      ...
      {
	get_win_mv_name(i,sv1,SMG_LEN);
        set_win_mv_name(p1+i-p2,sv1,SMG_LEN);
...
int bw_dialog::w_mv_add_val(char *p_s1,int p_s1_len,int wpp,char *p_s2,int p_s2_len)
{
  int  i,j,k,l;
  int  exist;
  char s3[SMG_LEN];
  ...
  //sprintf(sv1,"(((into w_mv_add_val(),p_s1=%s,p_s1_len=%d,wpp=%d,p_s2=%s,p_s2_len=%d,",p_s1,p_s1_len,wpp,p_s2,p_s2_len);
  //bw_getread1.deb_record(sv1);
  ...
  if (exist==0)
  {
    //sprintf(sv1,")))return from w_mv_add_val(),exist=%d,win_mv_pp1=%d,win_mv_pp2=%d,",exist,win_mv_pp1,win_mv_pp2);
    //bw_getread1.deb_record(sv1);
  ...
  if (l!=0) 
  {
    //sprintf(sv1,")))return from w_mv_add_val(),move bef or aft error,win_mv_pp1=%d,win_mv_pp2=%d,",exist,win_mv_pp1,win_mv_pp2);
    //bw_getread1.deb_record(sv1);
  ...
  //sprintf(sv1,")))return from w_mv_add_val(),win_mv_pp1=%d,win_mv_pp2=%d,posi(i,0)=%d,posi(i,1)=%d,",win_mv_pp1,win_mv_pp2,get_win_mv_posi(i,0),get_win_mv_posi(i,1));
  //bw_getread1.deb_record(sv1);

  return(0);
}

int bw_dialog::w_mv_fet_val(char *p_s1,int p_s1_len,int wpp,char *p_s2,int p_s2_len)
{
  int  i,j;
  int  exist;
  char sv1[SMG_LEN];
  ...
  //sprintf(sv1,"(((into w_mv_fet_val(),p_s1=%s,p_s1_len=%d,wpp=%d,",p_s1,p_s1_len,wpp);
  //bw_getread1.deb_record(sv1);
  ...
  if (exist==0)
  {
    //sprintf(sv1,")))return from w_mv_fet_val(),exist==0,");
    //bw_getread1.deb_record(sv1);
...
/*
  if (get_win_mv_posi(i,1)<p_s2_len)
  {
...
*/
  ...
  //sprintf(sv1,")))return from w_mv_fet_val(),return val=%s,posi(i,0)=%d,posi(i,1)=%d,",p_s2,get_win_mv_posi(i,0),get_win_mv_posi(i,1));
  //bw_getread1.deb_record(sv1);

  return(0);
}

int bw_dialog::w_mv_exist(char *p_s1,int p_s1_len,int wpp)
{
  int  i;
  int  exist;
  char sv1[SMG_LEN];
  ...
  //sprintf(sv1,"(((into w_mv_exist(),p_s1=%s,p_s1_len=%d,wpp=%d,",p_s1,p_s1_len,wpp);
  //bw_getread1.deb_record(sv1);
  ...
  //sprintf(sv1,")))return from w_mv_exist(),exist=%d,",exist);
  //bw_getread1.deb_record(sv1);

  return(exist);
}
int bw_dialog::w_mv_add_name(char *p_s1,int p_s1_len)
{
  char str[SMG_LEN];
  int  i;

  //sprintf(str,"(((into w_mv_add_name(),p_s1=%s,p_s1_len=%d,",p_s1,p_s1_len);
  //bw_getread1.deb_record(str);

  set_win_mv_name(win_mv_pp1,p_s1,p_s1_len);
  ...
  if (win_mv_pp1<=0)// ---bug ok---
  {
    ...
    //sprintf(str,"===set posi(i,0) val , win_mv_pp1=%d,posi(i,0)=%d,",win_mv_pp1,get_win_mv_posi(win_mv_pp1,0));
    //bw_getread1.deb_record(str);
  ...
  //sprintf(str,")))return from w_mv_add_name(),return val=%d,",i);
  //bw_getread1.deb_record(str);

  return(i);
}
int bw_dialog::w_mv_seri(char *p_s1,int p_s1_len,int wpp)
{
  int  i,j;
  int  exist;
  char sv1[SMG_LEN];
  ...
  //sprintf(sv1,"(((into w_mv_seri(),p_s1=%s,p_s1_len=%d,wpp=%d,",p_s1,p_s1_len,wpp);
  //bw_getread1.deb_record(sv1);
  ...
  //sprintf(sv1,")))return from w_mv_seri(),return val=%d,",j);
  //bw_getread1.deb_record(sv1);
...
int bw_dialog::w_mv_move_aft(int dd,int num)
{
  ...
  //sprintf(sv1,"(((into w_mv_move_aft(),dd=%d,num=%d,",dd,num);
  //bw_getread1.deb_record(sv1);

  if (dd<=0)
  {
    //sprintf(sv1,")))return from w_mv_move_aft(),dd<=0,");
    //bw_getread1.deb_record(sv1);
  ...
  {
    //sprintf(sv1,")))return from w_mv_move_aft(),win_mv_pp2>=MEM_LEN,");
    //bw_getread1.deb_record(sv1);
...
*/
    ...
    //sprintf(sv1,"---move aft i=%d,posi(i,0)=%d,dd=%d,",i,get_win_mv_posi(i,0),dd);
    //bw_getread1.deb_record(sv1);

    set_win_mv_posi(i,0,get_win_mv_posi(i,0)+dd);

    //sprintf(sv1,"---move aft posi(i,0)=%d,",get_win_mv_posi(i,0));
    //bw_getread1.deb_record(sv1);

  }

  //sprintf(sv1,")))return from w_mv_move_aft(),");
  //bw_getread1.deb_record(sv1);
...
int bw_dialog::w_mv_move_bef(int dd,int num)
{
  ...
  //sprintf(sv1,"(((into w_mv_move_bef(),dd=%d,num=%d,",dd,num);
  //bw_getread1.deb_record(sv1);

  if (dd<=0)
  {
    //sprintf(sv1,")))return from w_mv_move_bef(),dd<=0,");
    //bw_getread1.deb_record(sv1);
  ...
  if (win_mv_pp2-dd<0)
  {
    //sprintf(sv1,")))return from w_mv_move_bef(),win_mv_pp2<0,");
    //bw_getread1.deb_record(sv1);
  ...
  for (i=num+1;i<win_mv_pp1;i++)
  {
    ...
    //sprintf(sv1,"---move bef i=%d,posi(i,0)=%d,dd=%d,",i,get_win_mv_posi(i,0),dd);
    //bw_getread1.deb_record(sv1);

    set_win_mv_posi(i,0,get_win_mv_posi(i,0)-dd);

    //sprintf(sv1,"---move bef posi(i,0)=%d,",get_win_mv_posi(i,0));
    //bw_getread1.deb_record(sv1);

  }

  //sprintf(sv1,")))return from w_mv_move_bef(),");
  //bw_getread1.deb_record(sv1);

  return(0);
}

int bw_dialog::w_mv_sav_var(char *p_s1,int p_s1_len,int wpp)
{
//  HDC    hdc;
  char   sm1[SMG_LEN];
  //char   sd1[SMG_LEN];
  ...
  if (w_mv_exist(p_s1,p_s1_len,wpp)!=1) return(1);

  i=w_mv_seri(p_s1,p_s1_len,wpp);

  j=get_win_mv_type(i,2);

  setc_win_mv_getv(wpp,j,0);

  get_win_mv_getv(wpp,sm1,SMG_LEN);

  w_mv_add_val(p_s1,p_s1_len,wpp,sm1,SMG_LEN);

  return(0);
}

int bw_dialog::w_mv_get_var(char *p_s1,int p_s1_len,int wpp)
{
//  HDC    hdc;
  char   sm1[SMG_LEN];
  //char   sd1[SMG_LEN];
  ...
  w_mv_fet_val(p_s1,p_s1_len,wpp,sm1,SMG_LEN);

  set_win_mv_getv(wpp,sm1,SMG_LEN);
...
int bw_dialog::w_mv_get_read_paint(char *p_str,int p_str_len,int wpp,char comm)
{
  HWND hnd;
  int  i,j,q;
  char sm1[SMG_LEN];
  ...
  if (w_mv_exist(p_str,p_str_len,wpp)!=1) return(1);

  w_mv_get_var(p_str,p_str_len,wpp);

  i=w_mv_seri(p_str,p_str_len,wpp);
...
int bw_dialog::w_mv_get_read(char *p_str,int p_str_len,int wpp,char comm)
{
  HWND hnd;
  int  i,j,q;
  char sm1[SMG_LEN];
...
int bw_dialog::w_mv_get_read1(char *p_str,int p_str_len,int wpp,char comm)
{
  int  i,j;
  char sm1[SMG_LEN];
  //char sd1[SMG_LEN];
...
//  char s_fldname[SMG_LEN];
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	if (str[0]=='*')
	{
	  ...
	  if (m==8)  bw_inkey1.long2str(bw_xbase1.get_win_recno(p_fcnt,wpp,0),s_tmpc2,SMG_LEN);
	  if (m==9)  bw_inkey1.long2str(bw_xbase1.get_win_rcnt(p_fcnt),s_tmpc2,SMG_LEN);
...
int bw_dialog::w_get_yy(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
  char s_tmpc3[SMG_LEN];
  if (p_s1_len<10) return(0);
...
int bw_dialog::w_get_mm(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
  ...
  bw_inkey1.int2str(i,s_tmpc5,SMG_LEN);
  p_s1[0]=s_tmpc5[ 9];
  p_s1[1]=s_tmpc5[10];
*/
  return(0);
...
int bw_dialog::w_get_dd(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
...
int bw_dialog::w_get_year(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
...
int bw_dialog::w_get_mon(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
...
int bw_dialog::w_get_time(char *p_s1,int p_s1_len)
{
/*
  time_t t;
  int i;
...
int bw_dialog::w_get_usr(char *p_s1,int p_s1_len)
{
  int i;
  if (p_s1_len<30) return(0);
...
int bw_dialog::get_win_mv_name(int mvpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_dialog::set_win_mv_name(int mvpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_dialog::get_win_mv_getv(int wpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_dialog::set_win_mv_getv(int wpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_switch::get_win_switch_name(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_switch::set_win_switch_name(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_choose::get_win_choose_name(int cpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_choose::set_win_choose_name(int cpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::get_t5_m_name(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::set_t5_m_name(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::get_t5_m_button(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::set_t5_m_button(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::get_t5_m_comm(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::set_t5_m_comm(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::get_t5_m_focus(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_menuvar::set_t5_m_focus(int mpp,char *p_s1,int p_s1_len)
{
  int i,j;
