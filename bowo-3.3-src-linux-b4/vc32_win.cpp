...
//  MessageBox(bw_main1.win_hwnd1,"cc","cc",MB_OK);
  ...
  for (s_ps1=0;s_ps1<=s_ps;s_ps1++)  /* fetch stru & base name from tmpc5 */
  {
    ...
    if (strncmp(str3,"1",1)==0) bw_xbase1.set_win_base_backup(tmpc6[s_ps1],1);
	...
	if (bw_buff1.get_tmpc1(p_pagecnt,i,j)<=' ')
	{
	  ...
	  if (bw_buff1.get_tmpc1(p_pagecnt,i,j)!=' ')
	  {
		...
		if (s2[0]=='@')
		{
		  ...
		  w_cre_ci_button(p_pagecnt,s1,SMG_LEN,p6,p7,p8,i,s2,strlen(s2));
...
int bw_win::w_cre_ci_button(int p_pagecnt,char *p_s1,int p_s1_len,int p6,int p7,int p8,int i,char *p_s2,int len2)
{
  HWND hnd;
  int  l,j;
  //char s2[SMG_LEN];
...
int bw_win::w_cre_ci_radio(int p_pagecnt,int p6,int p7,int p8,int i,char *p_str,int len)
{
  HWND hnd;
  char str[SMG_LEN];
...
int bw_win::w_cre_ci_check(int p_pagecnt,int p6,int p7,int p8,int i,char *p_str,int len)
{
  HWND hnd;
  char str[SMG_LEN];
