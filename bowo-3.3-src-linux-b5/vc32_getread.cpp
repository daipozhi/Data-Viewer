...
int bw_getread::tst_getread()
{
  ...
  for (i=0;i<=12;i++)
  {
    get_smg_string(i,s1,SMG_LEN);

    sprintf(str,"l=%d,c=%d,d=%s,t=%d,n=%d,n2=%d,dc=%d,l=%d,c=%d,id=%d,"
	    ,get_smg_line(i),get_smg_colu(i),s1
...
//  char  str[SMG_LEN];
  ...
  if (command=='g')
  {
    ...
    get_smg_string(smg_pp,s1,SMG_LEN);
    i=strlen(s1);
  ...
  else
  {
    if (command=='r')
    {
      ...
      get_smg_string(smg_pp2,s1,SMG_LEN);
	...
	if (i>=(int)strlen(s1)) set_smg_data(i,' ');
      ...
      for (i=i;i<SMG_LEN;i++) set_smg_data(i,' ');

      if (get_smg_type(smg_pp2)=='c')
      {
	get_smg_atte(smg_pp2,s2,SMG_LEN);
...
/*
	...
	if (win_p1+n2>datalen_real)
	{
	  ...
	  for (i=1;i<=datalen;i++)
	  {
	    if (i<=datalen_real) str[i-1]=get_smg_data(i-1);
	    else str[i-1]=' ';
	  }
	  str[datalen]=0;
	  HideCaret(bw_main1.win_hwnd);
  ...
  for (k=0;k<300;k++)
  {
	...
	else
	{
	  ...
	  for (i=1/*n2*/;i<=datalen;i++)
	  {
	    ...
	    else str[i-1/*n2*/]=get_smg_data(win_p1+i-1);
	  }
...
/*
  ...
  for (k=0;k<300;k++)
  {
	...
	else
	{
      ...
      *posi=n2;
      ...
      for (i=1;i<=datalen;i++)
      {
	if (i<=datalen_real) str[i-1]=bw_getread1.get_smg_data(i-1);
        else str[i-1]=' ';
      }
      str[datalen]=0;
      HideCaret(bw_main1.win_hwnd);
  ...
  for (k=0;k<300;k++)
  {
        ...
        for (i=n2;i<=datalen;i++)
	{
	  ...
	  else str[i-n2]=get_smg_data(win_p1+i-1);
	}
	str[datalen-n2+1]=0;
	HideCaret(bw_main1.win_hwnd);
	TextOut(hdc,win_p_x,win_p_y,str,strlen(str));
...
*/
  ...
  for (j=0;j<=i-1;j++) s_tmpc1[30-i+j]=get_smg_data(j);
  s_tmpc1[30]='.';
  for (j=i+1;j<=datalen_real-1;j++) s_tmpc1[30+j-i]=get_smg_data(j);
  for (j=0;j<=datalen_real-1;j++)
    if (dec==0)
      set_smg_data(j,s_tmpc1[30-datalen_real+j]);
    else
      set_smg_data(j,s_tmpc1[30-(datalen_real-dec-1)+j]);
...
int bw_getread::gets_smg_data(char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_getread::get_smg_string(int gpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_getread::get_smg_atte(int gpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_getread::set_smg_string(int gpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_getread::set_smg_atte(int gpp,char *p_s1,int p_s1_len)
{
  int i,j;
