...
//  long    recno;
  ...
  switch(msg)    
  {
	...
	MoveWindow(hwnd
      ...
      k=bw_link1.w_read_link(s_fn1,FN_SIZE,bw_main1.win_hnd_id,bw_main1.win_hnd_id-bw_win1.get_win_pp6(bw_win1.get_win_pp5(bw_main1.win_hnd_id),1)+1);
      l=0;
	...
	bw_win1.set_win_is_link(bw_main1.win_hnd_id,1);
      ...
      if (bw_win1.get_win_style(bw_main1.win_hnd_id,3)==1) bw_win1.w_echo_win_chs(bw_main1.win_hnd_id,1);
      ...
      bw_win1.set_win_pp11(bw_main1.win_hnd_id,bw_getread1.get_read_login());
...
//	ReleaseDC(win_hwnd,hdc);
	...
	if (str[0]=='?') bw_dialog1.w_mv_get_read_paint(str,SMG_LEN,bw_main1.win_hnd_id,'g'); // wpp to 0
	else
        {
	  if (str[0]==':')
	  {
...
*/      
	  ...
	  {
	    bw_win1.pline =bw_win1.get_win_hpp4(bw_win1.win_menu_seri,2);
	    bw_win1.pfield=bw_win1.get_win_hpp4(bw_win1.win_menu_seri,1);
          }
...
/*	  return(0);*/
	...
	if ((str[0]=='?')||(str[0]=='-'))
	{
	  ...
	  if (k==1)
	  {
	    if (str[0]=='?')
            {
	      bw_getread1.get_smg_string(bw_win1.get_win_pp11(bw_main1.win_hnd_id),sf1,SMG_LEN);
	    }
	    else
	    {
	    ...
	    bw_xbase1.set_win_scrnvar(bw_main1.win_hnd_id,sf1,SMG_LEN);
	    bw_dialog1.set_win_mv_getv(bw_main1.win_hnd_id,sf1,SMG_LEN);
	...
	if ((str[0]==':')||(str[0]=='+'))
	{
	  ...
	  if (k==1)
	  {
	    ...
	    if (str[0]==':')
	      ...
	      bw_getread1.get_smg_string(bw_win1.get_win_pp11(bw_main1.win_hnd_id),sf1,SMG_LEN);
	      bw_xbase1.set_win_scrnvar(bw_main1.win_hnd_id,sf1,SMG_LEN);
	      bw_xbase1.get_win_scrnvar(bw_main1.win_hnd_id,fldvar,SMG_LEN);
	    }
	    else
	    {
		...
		GetWindowText(hnd,fldvar,SMG_LEN);
	      ...
	      bw_xbase1.set_win_scrnvar(bw_main1.win_hnd_id,fldvar,SMG_LEN);
	    }
...
//	    MessageBox(hwnd,str2,"aa",MB_OK);
		...
		bw_link1.w_link_modi_mark(bw_main1.win_hnd_id
			     ,m
			     ,bw_fldstru1.w_find_fldnum(fldname,SMG_LEN,m)-bw_fldstru1.get_t2_fldpoin(m,0)
			     ,bw_win1.get_1fldseri(l));

		bw_win1.set_win_state12(bw_main1.win_hnd_id,1);
	      }
	    }
...
/*	    return(0);*/
	...
	if (bw_win1.get_win_state12(bw_main1.win_hnd_id)==1)
	{
	  bw_link1.w_echo_winrec_lnk(bw_main1.win_hnd_id);

	  bw_win1.set_win_state12(bw_main1.win_hnd_id,0);
	}
      }
...
//	      MessageBox(hwnd,str,"aa",MB_OK);
	...
	if (m==1)
	{
      ...
      bw_win1.get_1fldname(i,str2,SMG_LEN);
	  ...
	  if (str2[0]==':')
	  {
       ...
       if (exist==0) // don't have so many records, locate at old position
	   {
	      bw_win1.pline =bw_win1.get_win_linep(bw_main1.win_hnd_id,0);
	      bw_win1.pfield=bw_win1.get_win_linep(bw_main1.win_hnd_id,1);
	   }
      ...
      bw_xbase1.set_win_recno(k,bw_main1.win_hnd_id,0,r2);
          ...
          if (bw_xbase1.win_state3==1)
          {
	        ...
	        if (bw_xbase1.win_state3==1) bw_xbase1.set_win_recno(k,bw_main1.win_hnd_id,1,1);
          }
...
//	  hcursor=LoadCursor(NULL,"cursor2.cur");
    ...
    case WM_PROGRAM1:
      ...
      bw_getread1.set_smg_string(bw_win1.get_win_pp11(bw_main1.win_hnd_id),str2,SMG_LEN);
...
//  HDC  hdc;
  ...
  switch(msg)
  {
	...
	if ((bw_main1.win_key==274)||
	    ...
	    (bw_main1.win_key<=32))
	{
	  k=bw_win1.get_win_hpp4(bw_win1.win_menu_seri,4);
...
int bw_main::w_fetch_filename(char *p_s1,int p_s1_len)
{
  int i;
  int exist;
  ...
  if (p_s1_len>0) p_s1[p_s1_len-1]=0;

  bw_inkey1.strtrim(p_s1,p_s1_len);
  ...
  if (exist==1)
  {
    bw_inkey1.strpcut(p_s1,  0,               i,p_s1_len,win_seadir ,FN_SIZE);
    bw_inkey1.strpcut(p_s1,i+1,strlen(p_s1)-i-1,p_s1_len,win_seaname,FN_SIZE);
...
int bw_main::set_string(char *p_s1,int p_s1_len,char *p_s2,int p_s2_len)
{
  int i;

  p_s1[0]=0;

  for (i=0;i<p_s2_len;i++)
  {
    if (i+1>=p_s1_len) continue;
    else
    {
      if (p_s2[i]==0) break;
      else
...
int bw_main::get_view(char *p_str,int p_str_len,HWND hwnd,int pp5)
{
  HMENU hm1;
  HWND  hwnd2;
...
//  MessageBox(bw_main1.win_hwnd1,"bb","bb",MB_OK);
  ...
  i=bw_inkey1.strlocc(bw_main1.s_bs1,FN_SIZE,'.');
  bw_main1.s_bs1[i]=0;
  ...
  if (bw_inkey1.w_find_file(bw_main1.s_bs2)==1) bw_font1.w_read_fdc(bw_main1.s_bs2,strlen(bw_main1.s_bs2),bw_main1.win_hnd_id);
  else
  {
    bw_inkey1.w_get_dir(win_work_dir,FN_SIZE);
    bw_inkey1.w_set_dir(win_home_dir);
  ...
  {
    if (bw_win1.get_win_pp9(j,bw_main1.win_hnd_id)==1)
    {
      bw_main1.win_hnd_id      = tmpn1;
      bw_win1.win_pp1 = tmpn2;
      ...
      bw_win1.get_win_tabname(0,j,bw_main1.s_bs2,FN_SIZE);
      k=bw_inkey1.strlocc(bw_main1.s_bs2,FN_SIZE,'.');
      bw_main1.s_bs2[k]=0;
  ...
  //bw_getread1.deb_record("open dbf");
  ...
  for (j=0;j<STRU_NUM;j++)
  {
    if (bw_win1.get_win_pp9(j,bw_main1.win_hnd_id)==1)
    {
      ...
      bw_win1.get_win_tabname(1,j,bw_main1.s_bs2,FN_SIZE);
  ...
  //sprintf(str,"open dbf ok=%d,",open_base_ok);
  ...
  if (open_base_ok==0)
  {
    for (j=0;j<STRU_NUM;j++)
    {
      ...
      if ((bw_win1.get_win_pp9(j,bw_main1.win_hnd_id)==1)&&(open_base[j]==1))
      {
	bw_win1.get_win_tabname(1,j,bw_main1.s_bs2,FN_SIZE);

	bw_mbase1.w_close_base(j,0);
      }
      if ((bw_win1.get_win_pp9(j,bw_main1.win_hnd_id)==1)&&(open_stru[j]==1))
      {
  ...
  //bw_getread1.deb_record("open dbf check ok");
  ...
  l=bw_win1.get_win_pp5(bw_main1.win_hnd_id);

  for (k=bw_win1.get_win_pp6(l,0);k<=bw_win1.get_win_pp6(l,2);k++)
  {
   ...
   bw_win1.set_win_hnd(j+k-bw_win1.get_win_pp6(l,0),bw_win1.get_win_phh(k));
...
*/
    ...
    {
      bw_bmp1.echo_bmp(hwnd,bw_menuvar1.t5_m_como,SMG_LEN,bw_menuvar1.get_t5_m_mcnt(bw_menuvar1.t5_m_pp3),bw_main1.win_hnd_id);
...
//  char sf1[SMG_LEN];
  ...
  if (str[0]=='@')
  {
    ...
    {
      bw_main1.get_view(bw_menuvar1.t5_m_como,SMG_LEN,bw_main1.win_hwnd,bw_win1.get_win_pp6(hnd_id,0));
      return(0);
    }
    ...
    if (bw_menuvar1.t5_m_pp2==14)
    {
      ...
      bw_win1.get_win_window_name(hnd_id,s_win,SMG_LEN);
      bw_inkey1.strtrim(s_win,SMG_LEN);
      l=bw_inkey1.strlocc(s_win,SMG_LEN,'.');
      s_win[l]=0;
	...
	bw_win1.set_win_state12(hnd_id,1);
      }

      bw_fldstru1.w_page_up(j,hnd_id);

      if (bw_win1.get_win_state12(hnd_id)==1)
      {
	bw_link1.w_echo_winrec_lnk(hnd_id);
	bw_win1.set_win_state12(hnd_id,0);
      }
    }
...
int bw_main::get_ci_save(int hnd_id)
{
  ...
  bw_win1.w_find_winfname(hnd_id,bw_win1.get_win_linep(hnd_id,0),bw_win1.get_win_linep(hnd_id,1),str1,SMG_LEN);

  if ((str1[0]=='?')||(str1[0]=='-'))
  {
    if (str1[0]=='?')
    {
      bw_getread1.get_smg_string(bw_win1.get_win_pp11(hnd_id),sf1,SMG_LEN);
      bw_xbase1.set_win_scrnvar(hnd_id,sf1,SMG_LEN);
      bw_dialog1.set_win_mv_getv(hnd_id,sf1,SMG_LEN);
    }
    else
    {
  ...
  if ((str1[0]==':')||(str1[0]=='+'))
  {
    ...
    bw_xbase1.get_win_scrnvar(hnd_id,fldvar,SMG_LEN);
    o=bw_fldstru1.w_cmp_tab_var(fldname,SMG_LEN,m,fldvar,SMG_LEN);
...
*/
    ...
    if (p1==6)
    {
      i=bw_inkey1.strlocc(tc5,SMG_LEN,',');
...
/*
      bw_inkey1.int2str(win_main_xsize,tc6,SMG_LEN);
      bw_inkey1.int2str(win_main_ysize,tc7,SMG_LEN);
      strcpy(tc8,"size=");
      strcat(tc8,tc6);
      strcat(tc8,",");
      strcat(tc8,tc7);
...
*/
    ...
    {
/*
      bw_inkey1.int2str(win_main_xposi,tc6,SMG_LEN);
      bw_inkey1.int2str(win_main_yposi,tc7,SMG_LEN);
      strcpy(tc8,"position=");
      strcat(tc8,tc6);
      strcat(tc8,",");
      strcat(tc8,tc7);
...
int bw_main::get_win_face_bmp1(int p019,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_main::set_win_face_bmp1(int p019,char *p_s1,int p_s1_len)
{
  int i,j;
