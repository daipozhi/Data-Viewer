...
int bw_print::w_open_print(char *p_s1,int p_s1_len)
{
  char s2[FN_SIZE];
  //char s_fn4[FN_SIZE];
  int  i,j;
...
int bw_print::w_read_page(char *p_fn2,int p_fn2_len,int page)
{
  FILE *s_fp1;
...
//  int  read_type;
  ...
  for (i=1;i<PAGE_LINE;i++) for (j=1;j<PAGE_COLU;j++) set_t6_tmpc1(i,j,' ');
  lcnt=0;
...
//  int  state4;
  ...
  for (i=1;i<PAGE_LINE;i++)
  {
    ...
    {
      if ((get_t6_tmpc1(i,j)>' ')||(get_t6_tmpc1(i,j)<0))
      {
	p1=j;
	break;
      }
	...
	else
	{
	  if (state1==1)
	  {
	    if ((p4==0)&&(get_t6_tmpc1(i,j-1)=='%'))
	    {
	      set_t6c_fldname(pp1,p4,'%');
	      p4++;
	    }
	    set_t6c_fldname(pp1,p4+0,get_t6_tmpc1(i,j));
...
//  char ts_tmpc1[SMG_LEN];
  ...
  for (k=0;k<=t6_pp1;k++)
  {
      ...
      ||(bw_win1.w_field_type(sfn)==8))
    {
      ...
      get_t6_fldname(k,ts_tmpc2,SMG_LEN);
	...
	&&(bw_inkey1.strempty(ts_fldname)==0))
      {
	set_t6_fldname(k,ts_tmpc2,SMG_LEN);
      }
      else
      {
	strcpy(c2,c1);
        strcat(c2,ts_fldname);
	set_t6_fldname(k,c2,SMG_LEN);
      }
	...
	set_t6_fldedit(k,w_fld_set_edit(ts_fldname,SMG_LEN));
...
int bw_print::w_fld_set_edit(char *p_fldname,int p_fldname_len)
{
//  int i,j;
  //char s_fn4[SMG_LEN];
...
int bw_print::w_open_memo_print(char *p_s1,int p_s1_len)
{
  char s2[FN_SIZE];
//  char s3[FN_SIZE];
  //char s_fn4[FN_SIZE];
...
//  int exist;
  ...
  {
    if ((get_t6_tmpc1(l,i)>' ')||(get_t6_tmpc1(l,i)<0)) break;
    else j=1;
  }
...
int bw_print::w_read_memo(char *p_s1,int p_s1_len,int m5)
{
  int i,j;
  char tc1[SMG_LEN];
...
int bw_print::w_put_memo_char(int l,char *p_s1,int p_s1_len)
{
  int  i,j;
  //char s_fn4[SMG_LEN];
...
*/
	...
	//bw_print1.w_print_echo1(hwnd);
	  ...
	  if (exist==0) bw_print1.set_t6_tmpc2(i,0,0);
          ...
          bw_print1.get_t6s_tmpc2(i,sfn,SMG_LEN);
...
int bw_print::w_out_print(char *p_s1,int p_s1_len)
{
  int  i,j;
//  int  pp1;
//  char c1,c2;
...
//  int  s_state;
  ...
  set_win_print_fname(p_s1,p_s1_len);
...
//  int  pp1,pp2;
  ...
  get_win_print_fname(s2,SMG_LEN);

  i=bw_inkey1.strlocc(s2,SMG_LEN,'.');
  s2[i]=0;
  strcat(s2,".txt");
  s_fp2=fopen(s2,"r");
  if (s_fp2==NULL)
...
//win_state7[win_fcnt][win_pp4]=win_print_state1;
  ...
  for (i=0;i<PAGE_LINE;i++)
    for (j=0;j<PAGE_COLU;j++)
      set_t6_tmpc2(i,j,get_t6_tmpc1(i,j));
...
//	    win_tst1=0;
	    ...
	    if (bw_xbase1.win_state3==0) bw_xbase1.set_win_enseri(k,wpp,l);
          }
...
//  char s_fldname[SMG_LEN];
  ...
  for (k=0;k<=t6_pp1;k++)
  {
	...
	get_t6_fldname(k,sfn,SMG_LEN);
      ...
      if (m==8)  bw_inkey1.long2str(0,s_tmpc2,SMG_LEN);
      if (m==9)  bw_inkey1.long2str(0,s_tmpc2,SMG_LEN);
...
//  int  s_tmpn1,s_tmpn2;
  ...
  {
    if (bw_link1.get_win_svwind(i)==bw_win1.win_pp1) break;
  ...
  for (p=0;p<=t6_pp1;p++)
  {
    if (get_t6_fldedit(p)==1)
    {
      ...
      bw_link1.set_win_svwind(p2,bw_win1.win_pp1);
...
int bw_print::w_print(int wpp,char *p_s1,int p_s1_len)
{
/*
  int  i,j,k,l;
  char s2[FN_SIZE];
...
int bw_print::get_t6_fldname(int fpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::set_t6_fldname(int fpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::get_t6_tmpc1(int lpp,int cpp)
{
  if ((lpp<0)||(lpp>=PAGE_LINE)) return(0);
  if ((cpp<0)||(cpp>=PAGE_COLU)) return(0);
  return(t6_tmpc1[lpp][cpp]);
}
int bw_print::set_t6_tmpc1(int lpp,int cpp,int val)
{
  if ((lpp<0)||(lpp>=PAGE_LINE)) return(0);
  if ((cpp<0)||(cpp>=PAGE_COLU)) return(0);
...
int bw_print::get_t6s_tmpc1(int lpp,char *p_s1,int p_s1_len)
{
  int i,j;

  if ((lpp<0)||(lpp>=PAGE_LINE)) return(0);
  if (p_s1_len>PAGE_COLU) i=PAGE_COLU;
  else i=p_s1_len;
...
int bw_print::get_t6s_tmpc2(int lpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::get_ts_field(int fpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::set_ts_field(int fpp,char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::get_win_print_fname(char *p_s1,int p_s1_len)
{
  int i,j;
...
int bw_print::set_win_print_fname(char *p_s1,int p_s1_len)
{
  int i,j;
