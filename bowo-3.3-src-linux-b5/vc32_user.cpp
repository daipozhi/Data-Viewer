...
int bw_user::w_user_triger(char *p_win,int p_win_len,char *p_msg,int p_msg_len,char *p_itm,int p_itm_len,char *p_stt,int p_stt_len)
{
  int  i;
  char str[SMG_LEN];
  char str1[SMG_LEN];
      ...
      (strncmp(p_msg,"WM_CLOSE",8)==0))
  {
    ...
    if (bw_dialog1.w_mv_exist("fdca1",6,0)==1)
    {
      ...
      i=bw_inkey1.str2int(str,SMG_LEN);
    }
