...
char bw_buff::get_t_winfile(int wpp,int line,int col)
{
  if ((wpp<1)||(wpp>=WIN_NUM)) return(0);
  if ((line<1)||(line>=LIN_NUM)) return(0);
  if ((col<1)||(col>=COL_NUM)) return(0);

  return(t_winfile[wpp][line][col]);
}

int bw_buff::set_t_winfile(int wpp,int line,int col,char val)
{
  if ((wpp<1)||(wpp>=WIN_NUM)) return(0);
  if ((line<1)||(line>=LIN_NUM)) return(0);
  if ((col<1)||(col>=COL_NUM)) return(0);

  t_winfile[wpp][line][col]=val;
  return(0);
}
