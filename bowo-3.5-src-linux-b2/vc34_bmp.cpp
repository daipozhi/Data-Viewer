...
int bw_bmp::echo_bmp(HWND hwnd,char *comm,int comm_size,int mcnt,int wpp)
{
/*
  FILE *s_fp;
  int  i,j,k,l,m;
...
//  char str[SMG_SIZE];
  ...
  bw_main1.set_string(s_fn4,SMG_SIZE,comm,comm_size);
  ...
  {
    j=i+1;
    bw_inkey1.strpcut(s_fn4,0,i,SMG_SIZE,str1,SMG_SIZE);
  }
  else return(1);
  ...
  if (exist==1)
  {
    bw_inkey1.strpcut(s_fn4,j,i-j,SMG_SIZE,str2,SMG_SIZE);
  }
  else return(1);

  bw_inkey1.strpcut(s_fn4,i+1,strlen(s_fn4)-i-1,SMG_SIZE,str3,FN_SIZE);

  k=bw_inkey1.str2int(str1,strlen(str1)+1);
  l=bw_inkey1.str2int(str2,strlen(str2)+1);

  if (bw_win1.get_win_ci_mode(wpp)==0) k=(int)(k*bw_main1.win_edit_xchar);
  else k=(int)(k*bw_main1.win_xchar);

  if (bw_win1.get_win_ci_mode(wpp)==0) l=(int)(l*bw_main1.win_edit_ychar);
  else l=(int)(l*bw_main1.win_ychar);

  i=bw_inkey1.strlocc(str3,FN_SIZE,'.');
  bw_inkey1.strpcut(str3,i,4,FN_SIZE,str4,SMG_SIZE);
  ...
  if (strncmp(str4,".smf",4)==0)
  {
    ...
    lk=bw_menuvar1.get_t5_m_mcnt(bw_menuvar1.t5_m_pp3);
    if (lk>lj) lk=lk-lj;
    bw_menuvar1.set_t5_m_mcnt(bw_menuvar1.t5_m_pp3,lk);
...
int bw_bmp::w_bmp_add(int p_fcnt,int wpp,char *p_fldname,int p_fldname_size)
{
/*
  int  i,j,k;
  char str[SMG_SIZE];
  ...
  bw_fldstru1.w_sav_tab_var(sfld,SMG_SIZE,p_fcnt,str,SMG_SIZE);
...
//  char               str[SMG_SIZE];
  ...
  switch(msg)
  {
    case WM_PAINT:
    {
      ...
      bw_xbase1.get_win_bmp_name(s1,SMG_SIZE);

      i=bw_bmp1.DIB(s1,SMG_SIZE);
