...
{
//  int i,j,k;
  return(get_t2_fldpoin(p_fcnt,1)-get_t2_fldpoin(p_fcnt,0)+1);
}
int bw_fldstru::w_cnt_reclen(int p_fcnt)
{
  int i,j;
  j=0;
  for (i=get_t2_fldpoin(p_fcnt,0);i<=get_t2_fldpoin(p_fcnt,1);i++)
  {
    j=j+get_t2_fldlen(i);
  }
  return(j);
...
int bw_fldstru::w_read_stru(char *p_fn1,int p_fn1_size,int p_stn)
{
  int  exist;
  int  i,j,k,l;
  ...
  while (!feof(fp1))
  {
    ...
    if (i>=SMG_SIZE) break;
    for (i=i;i<SMG_SIZE;i++) if (tmpc2[i]==' ') break; /* scan for fldrem */
    if (i>=SMG_SIZE)
    {
      fclose(fp1);
      bw_win1.w_atten2();
    ...
    p1=i;
    for (i=i;i<SMG_SIZE;i++) if (tmpc2[i]==' ') break; /* scan for fldname */
    if (i>=SMG_SIZE)
    {
      fclose(fp1);
      bw_win1.w_atten2();
    ...
    p3=i;
    for (i=i;i<SMG_SIZE;i++) if (tmpc2[i]==' ') break; /* scan for fldtype */
    if (i>=SMG_SIZE)
    {
      fclose(fp1);
      bw_win1.w_atten2();
    ...
    {
      if (tmpc2[j]==' ') break;
      setc_t2_fldname(pp1+k-1,j-p1,tmpc2[j]);
      setc_t2_fldname(pp1+k-1,j-p1+1,0);
    }

    bw_inkey1.strpcut(tmpc2,p4,p5-p4+1,SMG_SIZE,str,SMG_SIZE);
    l=bw_inkey1.str2int(str,SMG_SIZE);

    set_t2_fldtype(pp1+k-1,tmpc2[p3]);
    ...
    bw_inkey1.strpcut(tmpc2,p6,p7-p6+1,SMG_SIZE,str,SMG_SIZE);
    l=bw_inkey1.str2int(str,SMG_SIZE);
    set_t2_flddec(pp1+k-1,l);
  }

  set_t2_fldpoin(p_stn,1,pp1+k-1);

  fclose(fp1);

  bw_xbase1.set_win_lcnt(p_stn,0,w_cnt_tabfield(p_stn)); /*fldnum 2 */
  bw_xbase1.set_win_lcnt(p_stn,1,w_cnt_reclen(p_stn));   /*reclen 3 */
  w_cnt_fldposi(p_stn);
...
int bw_fldstru::w_dele_stru(int p_fcnt)
{
  ...
  n1=get_t2_fldpoin(p_fcnt,1)-get_t2_fldpoin(p_fcnt,0)+1;
  p1=get_t2_fldpoin(p_fcnt,1);
  p4=0;
  ...
  while (1)
  {
    ...
    {
      get_t2_fldname(j,sf1,20);
      set_t2_fldname(j-n1,sf1,20);
  ...
  if (p4==0) p4=get_t2_fldpoin(p_fcnt,1);
  ...
  set_t2_fldpoin(p_fcnt,0,0);
  set_t2_fldpoin(p_fcnt,1,0);

  bw_xbase1.set_win_lcnt(p_fcnt,0,0); /*fldnum 2 */
  bw_xbase1.set_win_lcnt(p_fcnt,1,0);   /*reclen 3 */

  return(0);
}

int bw_fldstru::w_find_fldnum(char *p_str,int p_str_size,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_SIZE];
...
int bw_fldstru::w_find_fldlen(char *p_str,int p_str_size,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_SIZE];
...
int bw_fldstru::w_find_fldtype(char *p_str,int p_str_size,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_SIZE];
...
int bw_fldstru::w_find_flddec(char *p_str,int p_str_size,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_SIZE];
...
int bw_fldstru::w_find_fldposi(char *p_str,int p_str_size,int p_fcnt)
{
  int  i;
  int  exist;
  char sf1[SMG_SIZE];
...
int bw_fldstru::w_find_fldname(int p1,int p2,char *p_str,int p_str_size)
{
  ...
  get_t2_fldname(p2,stc,11);
  bw_main1.set_string(p_str,p_str_size,stc,SMG_SIZE);
...
//		bw_xbase1.set_win_recno(k,wpp,0,s_seri[k]);
	      }
	    }

            if ((l==1)&&(s_first[k]==1))  // set page recno
            {
              bw_xbase1.set_win_recno(k,wpp,1,s_recno[k]);
              s_first[k]=0;
            }
...
int bw_fldstru::w_tv_get_read_paint(char *p_str,int p_str_size,int wpp,int l,int c,char comm)
{
  HWND hnd;
  int  i,j,q;
  char s1[SMG_SIZE];
  ...
  if ((w_find_fldtype(s2,SMG_SIZE,j)=='n')||(w_find_fldtype(s2,SMG_SIZE,j)=='c'))
  {
    ...
    bw_xbase1.set_win_scrnvar(wpp,s1,SMG_SIZE);
  ...
  else
  {
    if (w_find_fldtype(s2,SMG_SIZE,j)=='l')
    {
      ...
      logic=bw_inkey1.upper(bw_xbase1.getc_win_scrnvar(wpp,0));

      if ((logic=='1')||(logic=='Y')||(logic=='T')) logic='T';
      else logic='F';

      bw_xbase1.setc_win_scrnvar(wpp,0,logic);
      bw_xbase1.setc_win_scrnvar(wpp,1,0);

      bw_xbase1.get_win_scrnvar(wpp,s1,SMG_SIZE);
    ...
    else
    {
      if (w_find_fldtype(s2,SMG_SIZE,j)=='m')
      {
        ...
        bw_xbase1.set_win_scrnvar(wpp,"                     ",20);

	bw_xbase1.setc_win_scrnvar(wpp,bw_win1.get_1fldlen(i),0);
	...
	bw_xbase1.setc_win_scrnvar(wpp,0,0);
  ...
  if (p_str[0]==':')
  {
    bw_xbase1.get_win_scrnvar(wpp,s3,SMG_SIZE);

    j=bw_getread1.get_read(l-bw_win1.get_win_con(wpp,1)+1,
			 c-bw_win1.get_win_con(wpp,0)+1,
		       "",
		       s3,
		       ...
		       1,1,
		       bw_dialog1.get_win_mv_getp(wpp),
...
int bw_fldstru::w_tv_get_read_g(char *p_str,int p_str_size,int wpp,int l,int c,char comm)
{
  HWND hnd;
  int  i,j,q;
  char s1[SMG_SIZE];
...
int bw_fldstru::w_tv_get_read_r(char *p_str,int p_str_size,int wpp,int l,int c,char comm)
{
  int  i,j;
  char s2[SMG_SIZE];
  char s3[SMG_SIZE];
  ...
  //printf("fldstru field len=%d,real len=%d,\n",bw_win1.get_1fldlen(i),w_find_fldlen(s2,SMG_SIZE,j));
...
int bw_fldstru::w_get_tab_var(char *p_fn,int p_fn_size,int p_fcnt,char *p_ca,int p_ca_size)
{
  int  i,j,k;
//  char str[SMG_SIZE];
//  HDC  hdc;
  ...
  i=w_find_fldnum(p_fn,p_fn_size,p_fcnt);
  if (i!=0) return(1);
  j=w_find_fldlen(p_fn,p_fn_size,p_fcnt);
  k=w_find_fldposi(p_fn,p_fn_size,p_fcnt);
...
int bw_fldstru::w_cmp_tab_var(char *p_fn,int p_fn_size,int p_fcnt,char *p_ca,int p_ca_size)
{
  int  exist;
  int  i,j,k,l;
  ...
  if (i!=0) return(1);
  j=strlen(p_ca);
  l=w_find_fldlen(p_fn,p_fn_size,p_fcnt);
  k=w_find_fldposi(p_fn,p_fn_size,p_fcnt);
  if (j!=l) return(1);
...
int bw_fldstru::w_sav_tab_var(char *p_fn,int p_fn_size,int p_fcnt,char *p_ca,int p_ca_size)
{
  int  i,j,k,l;
  char c1;
...
int bw_fldstru::w_fetch_rec(int p_fcnt,int wpp,int seri)
{
  ...
  if (bw_xbase1.get_win_cuseri(p_fcnt,wpp)==0)
  {
    bw_xbase1.set_win_cuseri(p_fcnt,wpp,1);
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,0);

    recno=0;

    if (bw_xbase1.get_win_lnktab(p_fcnt)!=0)
    {
      while (1)
      {
    ...
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,recno);
  }

  if (bw_xbase1.get_win_rcnt(p_fcnt)<1)
  {
  ...
  if (bw_xbase1.get_win_cuseri(p_fcnt,wpp)==seri)
  {
    ...
    if (bw_xbase1.get_win_lnktab(p_fcnt)!=0)
    {
      if (bw_xbase1.get_win_lnktab(p_fcnt)>1) recno=0;
    ...
    {
      bw_xbase1.set_win_cuseri(p_fcnt,wpp,seri);
      bw_xbase1.set_win_recno(p_fcnt,wpp,0,recno);
      return(0);
    }
  }
  ...
  if (bw_xbase1.get_win_cuseri(p_fcnt,wpp)<seri)
  {
    if (bw_xbase1.get_win_lnktab(p_fcnt)<=1)
    {
      for (i=bw_xbase1.get_win_cuseri(p_fcnt,wpp)+1;i<=seri;i++)
      {
	if (bw_xbase1.get_win_lnktab(p_fcnt)!=0)
	{
	  while (1)
...
//  char str[SMG_SIZE];
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)>1) return(0);
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    for (i=1;i<=s_tmpn1;i++)
    {
      bw_xbase1.set_win_recno(p_fcnt,wpp,1,bw_mbase1.w_up_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,1)));
      if (bw_xbase1.win_state_dbf_err==1) break;
    }
    ...
    bw_xbase1.set_win_cuseri(p_fcnt,wpp,1);
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,bw_xbase1.get_win_recno(p_fcnt,wpp,1));
  ...
  else
  {
    ...
    if (bw_xbase1.win_state_dbf_err==1)
    {
	bw_xbase1.set_win_recno(p_fcnt,wpp,1,bw_xbase1.get_win_recno(p_fcnt,wpp,1)-1);
	while (1)
        {
	  bw_xbase1.set_win_recno(p_fcnt,wpp,1,bw_mbase1.w_down_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,1)));
	  j=bw_buff1.get_linecol(wpp,bw_win1.pline,bw_win1.pfield);
	  if (j>0) k=bw_win1.get_1fldseri(j);
	  else k=1;
	  l=bw_link1.w_link_number(wpp,p_fcnt);
	...
	if (bw_xbase1.win_state_dbf_err==1) bw_xbase1.set_win_recno(p_fcnt,wpp,1,1);
    }
...
//  char str[SMG_SIZE];
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    r1=bw_xbase1.get_win_recno(p_fcnt,wpp,1)-1; // try if there are n lines in after
    r2=0;

    for (i=1;i<=s_tmpn2;i++)
    {
      r2=bw_mbase1.w_down_rec(p_fcnt,wpp,r1);
      r1=r2;
      if (bw_xbase1.win_state_dbf_err==1) break;
    }
...
int bw_fldstru::w_first_editposi(int p_fcnt,int wpp)
{
  ...
  for (i=1;i<LIN_NUM;i++) /* find first screen field */
  {
    for (j=1;j<COL_NUM;j++)
    {
      ...
      if ((l!=0)&&(bw_win1.get_1fldedt(l)==0)&&(bw_win1.get_1fldbase(l)==p_fcnt))
      {
	bw_win1.pline=i;
        bw_win1.set_win_linep(wpp,0,i);
	bw_win1.pfield=j;
	bw_win1.set_win_linep(wpp,1,j);
	k=i;
	break;
      }
    }
...
int bw_fldstru::w_insert_rec(int p_fcnt,int wpp)
{
  ...
  s_state1=bw_mbase1.w_ins_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0));
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==1)
  {
    if ((bw_xbase1.get_win_carry_on(p_fcnt)!=1)||(bw_xbase1.get_win_enseri(p_fcnt,wpp)<=0))
    {
  ...
  bw_xbase1.set_win_state_dbf_modi(p_fcnt,1);
...
int bw_fldstru::w_add_rec(int p_fcnt,int wpp)
{
  ...
  s_state1=bw_mbase1.w_add_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0));
...
int bw_fldstru::w_delete_rec(int p_fcnt,int wpp)
{
  ...
  s_state1=bw_mbase1.w_del_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0));
  ...
  if (w_have_next_rec2(p_fcnt,wpp,recno))
  { 
  }  // adjust cursor
  else
  {
    j=bw_win1.w_cnt_winline(p_fcnt,wpp,LIN_NUM);
    k=w_first_line(p_fcnt,wpp);
    
    if ((j>1)&&(k==0)) bw_win1.w_up_field(wpp);  // multi line windows and not at first line
    else
    {
      if (w_have_prev_rec2(p_fcnt,wpp,recno)) bw_fldstru1.w_up_line2(p_fcnt,wpp); // single line window or in first line
    }
  }
...
int bw_fldstru::w_have_next_rec2(int p_fcnt,int wpp,int recno)
{
  int i;
  i=bw_mbase1.w_down_rec(p_fcnt,wpp,recno-1);
  if (bw_xbase1.win_state_dbf_err==0) return(1);
  else return(0);
}


int bw_fldstru::w_have_prev_rec2(int p_fcnt,int wpp,int recno)
{
  int i;
  i=bw_mbase1.w_up_rec(p_fcnt,wpp,recno);
  if (bw_xbase1.win_state_dbf_err==0) return(1);
  else return(0);
}
...
int bw_fldstru::w_rollback_base(int p_fcnt,int wpp)
{
  ...
  if (bw_xbase1.get_win_state_dbf_modi(p_fcnt)==0) return(0);
  ...
  bw_win1.get_win_tabname(1,p_fcnt,s_fn3,FN_SIZE);

  bw_xbase1.w_rollback_xbase(/*s_fn3,FN_SIZE,*/bw_xbase1.get_win_base_backup(p_fcnt),p_fcnt);

  bw_xbase1.set_win_state_dbf_modi(p_fcnt,0);

  for (p1=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(wpp),0)
      ;p1<=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(wpp),2)
      ;p1++)
  {
    if (bw_xbase1.get_win_state_in_srch(p1,p_fcnt)==1)
    {
      close(bw_xbase1.win_fp2[p1]);
      bw_xbase1.set_win_state_in_srch(p1,p_fcnt,0);
  ...
  w_first_editposi(p_fcnt,wpp);
  bw_xbase1.set_win_cuseri(p_fcnt,wpp,1);
  bw_xbase1.set_win_recno(p_fcnt,wpp,0,1);
  bw_xbase1.set_win_recno(p_fcnt,wpp,1,1);
...
int bw_fldstru::w_commit_base(int p_fcnt,int wpp)
{
  ...
  bw_xbase1.w_commit_xbase(bw_xbase1.get_win_base_backup(p_fcnt),p_fcnt);
...
int bw_fldstru::w_link_window(int p_fcnt,int wpp)
{
  ...
  if (bw_xbase1.get_win_state_in_srch(wpp,p_fcnt)==1) return(1);

  i=w_is_card(p_fcnt,wpp);

  j=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(wpp),0);

  if (i==1) bw_xbase1.set_win_recno(p_fcnt,wpp,1,bw_xbase1.get_win_recno(p_fcnt,j,0));
  else bw_xbase1.set_win_recno(p_fcnt,wpp,1,bw_xbase1.get_win_recno(p_fcnt,j,1));
...
//  int   i,j,k;
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)>1) return(1);
  ...
  bw_win1.get_win_find_name(wpp,sfn1,FN_SIZE);
  bw_main1.get_view(sfn1,FN_SIZE,bw_main1.win_hwnd,bw_win1.get_win_pp_grp_pp(wpp,0));
...
int bw_fldstru::w_find_key(int p_fcnt,int wpp)
{
  ...
  for (j=0;j<=2;j++) bw_xbase1.set_win_recno(p_fcnt,wpp,j,1);
  bw_xbase1.set_win_cuseri(p_fcnt,wpp,1);
  w_echo_winrec(wpp);
  w_first_editposi(p_fcnt,wpp);
...
int bw_fldstru::w_find_recno(int p_fcnt,int wpp)
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    if (bw_xbase1.get_win_state_in_srch(wpp,i)==1)
    {
      exist=1;
      break;
    }
  ...
  {
    close(bw_xbase1.win_fp2[wpp]);
    bw_xbase1.set_win_state_in_srch(wpp,i,0);
  ...
  bw_win1.get_win_window_name(wpp,s_fn2,FN_SIZE);
  ...
  bw_xbase1.win_fp2[wpp]=/*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
  ...
  while (1)
  {
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,bw_mbase1.w_down_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0)));
    if (bw_xbase1.win_state_dbf_err!=0) break;
    if (w_find_cond_ok(p_fcnt)!=1) continue;

    c1=(unsigned char)(bw_xbase1.get_win_recno(p_fcnt,wpp,0)/256/256/256);
    c2=(unsigned char)((bw_xbase1.get_win_recno(p_fcnt,wpp,0)-c1*256*256*256)/256/256);
    c3=(unsigned char)((bw_xbase1.get_win_recno(p_fcnt,wpp,0)-c1*256*256*256-c2*256*256)/256);
    c4=(unsigned char)(bw_xbase1.get_win_recno(p_fcnt,wpp,0)-c1*256*256*256-c2*256*256-c3*256);
  ...
  bw_xbase1.set_win_state_in_srch(wpp,p_fcnt,1);
...
//  int   exist;
  ...
  for (i=w_find_fldpoin0(p_fcnt)
     ...
     ;i++)
  {
    w_find_fldname(p_fcnt,i,stra,SMG_SIZE);

    strcpy(str,"?");
    strcat(str,stra);

    if (bw_dialog1.w_mv_exist(str,SMG_SIZE,zero)==1)
    {
      str2[0]=0;
      bw_dialog1.w_mv_save_mem(str,SMG_SIZE,zero,str2,SMG_SIZE);
    }
...
int bw_fldstru::w_find_exit(int wpp)
{
      ...
      ;p1++)
  {
	...
	bw_xbase1.get_win_basedir(p2,st1,FN_SIZE);
...
int bw_fldstru::w_find_cond_ok(int p_fcnt)
{
  ...
  int   exist;
  int   zero=0;
  char  stra[SMG_SIZE]; // scrn field name
  char  strb[SMG_SIZE]; // tab field name
  ...
  for (i=w_find_fldpoin0(p_fcnt)
     ...
     ;i++)
  {
    w_find_fldname(p_fcnt,i,strb,SMG_SIZE);
    ...
    if ((bw_dialog1.w_mv_exist(stra,SMG_SIZE,zero)==1)||(bw_dialog1.w_mv_exist(str4,SMG_SIZE,zero)==1))
    {
      if (bw_dialog1.w_mv_exist(str4,SMG_SIZE,zero)==1) strcpy(stra,str4);

      bw_dialog1.w_mv_fet_val(stra,SMG_SIZE,zero,str1,SMG_SIZE);
      ...
      w_get_tab_var(strb,SMG_SIZE,p_fcnt,str2,SMG_SIZE);

      //MessageBox(0,"cc str2",str2,MB_OK);

      if (w_find_fldtype(strb,SMG_SIZE,p_fcnt)=='c')
      {
	...
	else
	{
      ...
      if (w_find_fldtype(strb,SMG_SIZE,p_fcnt)=='n')
      {
	...
	while (1)
	{
      ...
      if (w_find_fldtype(strb,SMG_SIZE,p_fcnt)=='l')
      {
	if (str1[0]==' ')
	{
          break;
...
int bw_fldstru::w_edit_memo(int p_fcnt,int wpp,char *p_fldname,int p_fldname_size)
{                           
  char fn1[FN_SIZE];
  char str[SMG_SIZE];
  char str1[SMG_SIZE];
...
//  char stra[SMG_SIZE];
  ...
  if (bw_xbase1.get_win_mem_inuse(p_fcnt)==1) return(0);
  ...
  // make file name
  bw_xbase1.get_win_basename(p_fcnt,st2,FN_SIZE);
  ...
  if (bw_inkey1.w_find_file(fn1)==0)
  {
    ...
    bw_xbase1.get_win_memno(p_fcnt,str9,SMG_SIZE);
...
//    fclose(fh);
    ...
    strcpy(str3,str2);
    bw_xbase1.get_win_mem_ext(p_fcnt,str8,SMG_SIZE);
    strcat(str3,str8);
...
//      MessageBox(bw_main1.win_hwnd1,str4,str5,MB_OK);
    ...
    if (exist==1)
    {
      ...
      strcpy(str3,str2);
      bw_xbase1.get_win_mem_ext(p_fcnt,str8,FN_SIZE);
      strcat(str3,str8);
      bw_inkey1.strtrim(str3,FN_SIZE);
...
//      fclose(fh);
  ...
  if (mark==1)
  {
    bw_mbase1.w_down_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0)-1);

    w_sav_tab_var(str6,SMG_SIZE,p_fcnt,str9,SMG_SIZE);
    bw_mbase1.w_save_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0));
    bw_xbase1.set_win_state_dbf_modi(p_fcnt,1);
  ...
  bw_xbase1.set_win_mem_inuse(p_fcnt,1);
  bw_xbase1.set_win_mem_wpp(p_fcnt,wpp);
  bw_xbase1.set_win_mem_fn(p_fcnt,str3,FN_SIZE);
  bw_win1.set_win_memo_basep(wpp,p_fcnt);

  bw_edit1.get_edit(str3,FN_SIZE,wpp,bw_main1.win_hwnd);
...
//  long memp;
  ...
  p_fcnt=bw_win1.get_win_memo_basep(wpp);
  bw_xbase1.set_win_mem_inuse(p_fcnt,0);

  // save mem ext name
  bw_xbase1.get_win_mem_ext(p_fcnt,str0,10);
...
//  MessageBox(bw_main1.win_hwnd1,fn1,"aa",MB_OK);
  ...
  else
  {
    ...
    bw_xbase1.get_win_mem_ext(p_fcnt,str7,10);
    strcpy(str2,str1);
    strcat(str2,str7);
...
//    MessageBox(bw_main1.win_hwnd1,"copy & open ok","bb",MB_OK);
    ...
    bw_xbase1.get_win_memno(p_fcnt,str8,FN_SIZE);
    strcat(str4,str8);
...
int bw_fldstru::w_edit_memo_add(int p_fcnt,int wpp,char *p_fldname,int p_fldname_size)
{
/*
  int  i,j,k,l,m,n;
  char s_tc0[SMG_SIZE];
...
//  FILE *fh1;
  ...
  bw_xbase1.w_find_empty_mfile(s_tc7,SMG_SIZE,p_fcnt);
  ...
  bw_xbase1.get_win_memno(p_fcnt,str8,SMG_SIZE);
  ...
  if (bw_inkey1.w_find_file(fn1)==1)
  {
    ...
    if (exist!=1)
    {
      ...
      bw_xbase1.set_win_memno(p_fcnt,str8,strlen(str8)+1);
    }
    else  bw_xbase1.set_win_memno(p_fcnt,str8,strlen(str8)+1);
  }
  else return(1);
  ...
  if (mark==1)
  {
    ...
    w_sav_tab_var(str6,SMG_SIZE,p_fcnt,str8,SMG_SIZE);
    bw_mbase1.w_save_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,0));
    bw_xbase1.set_win_state_dbf_modi(p_fcnt,1);
  ...
  bw_xbase1.set_win_mem_inuse(p_fcnt,1);
  bw_xbase1.set_win_mem_fn(p_fcnt,s_tc8,SMG_SIZE);
  bw_win1.set_win_memo_basep(wpp,p_fcnt);
...
int bw_fldstru::get_t2_fldpoin(int spp,int p01)
{
...
int bw_fldstru::set_t2_fldpoin(int spp,int p01,int val)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
...
int bw_fldstru::w_down_line(int p_fcnt,int wpp)
{
  int i,j,k,l;
  int s_tmpn1;
  int r1,r2;

  i=w_last_line(p_fcnt,wpp);
  if (i==0) { /*MessageBox(0,"aa","aa",MB_OK);*/ bw_win1.w_down_field(wpp);}
  else
  {
    j=w_be_linked(p_fcnt,wpp);
    if (j==1) { /*MessageBox(0,"bb","bb",MB_OK);*/ bw_win1.w_down_field(wpp);}
    else
    {
      k=w_have_next_rec(p_fcnt,wpp);
      if (k==0) { /*MessageBox(0,"cc","cc",MB_OK);*/ bw_win1.w_down_field(wpp);}
      else 
      {
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    bw_win1.set_win_linep(wpp,0,bw_win1.pline);
    bw_win1.set_win_linep(wpp,1,bw_win1.pfield);

    i=bw_buff1.get_linecol(wpp,bw_win1.pline,bw_win1.pfield);
    j=bw_win1.get_1fldseri(i);
    bw_xbase1.set_win_cuseri(p_fcnt,wpp,j);

    r1=bw_xbase1.get_win_recno(p_fcnt,wpp,1)-1;
    r2=0;
    ...
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,r2);
...
int bw_fldstru::w_down_line2(int p_fcnt,int wpp) // button command
{
  int i,j,k,l;
//  int s_tmpn1;
  int r1,r2;
...
int bw_fldstru::w_last_line(int p_fcnt,int wpp)
{
  int  i,j,k,l,m;
  int  exist;
  char str[SMG_SIZE];
...
/*
      if (bw_buff1.get_linecol(wpp,i,j)>0)
      {
	if ((bw_win1.get_1fldedt(bw_buff1.get_linecol(wpp,i,j))==0)&&
            (bw_win1.get_1fldbase(bw_buff1.get_linecol(wpp,i,j))==p_fcnt))
	{
	  exist=1;
	  break;
	}
...
int bw_fldstru::w_be_linked(int p_fcnt,int wpp)
{
  if (bw_xbase1.get_win_lnktab(p_fcnt)>1) return(1);
  else return(0);
}

int bw_fldstru::w_have_next_rec(int p_fcnt,int wpp)
{
  int  i,j,k,l;
  int  s_tmpn1;
  int  r1,r2;
//  char str[SMG_SIZE];
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    s_tmpn1=1+bw_win1.w_cnt_winline(p_fcnt,wpp,LIN_NUM);
    for (i=1;i<=s_tmpn1;i++)
    {
      r2=bw_mbase1.w_down_rec(p_fcnt,wpp,r1);
      r1=r2;
...
int bw_fldstru::w_up_line(int p_fcnt,int wpp)
{
  int i,j,k,l;
  int r1,r2;
  int s_tmpn1;

  i=w_first_line(p_fcnt,wpp);
  if (i==0) { /*MessageBox(0,"aa","aa",MB_OK);*/ bw_win1.w_up_field(wpp); }
  else
  {
    j=w_be_linked(p_fcnt,wpp);
    if (j==1) { /*MessageBox(0,"bb","bb",MB_OK);*/ bw_win1.w_up_field(wpp); }
    else
    {
      k=w_have_prev_rec(p_fcnt,wpp);
      if (k==0) { /*MessageBox(0,"cc","cc",MB_OK);*/ bw_win1.w_up_field(wpp); }
      else 
      {
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    s_tmpn1=j/*bw_win1.w_cnt_winline(p_fcnt,wpp,LIN_NUM)*/;
    for (i=1;i<=s_tmpn1;i++)
    {
      r2=bw_mbase1.w_down_rec(p_fcnt,wpp,r1);
      if (bw_xbase1.win_state_dbf_err==1) break;
...
int bw_fldstru::w_up_line2(int p_fcnt,int wpp)
{
  int i,j,k,l;
  int r1,r2;
...
/*
    ...
    r1=bw_xbase1.get_win_recno(p_fcnt,wpp,1);
    bw_xbase1.set_win_recno(p_fcnt,wpp,0,r1);
*/
    bw_win1.set_win_linep(wpp,0,bw_win1.pline);
    bw_win1.set_win_linep(wpp,1,bw_win1.pfield);
...
int bw_fldstru::w_first_line(int p_fcnt,int wpp)
{
  int  i,j,k,l,m;
  int  exist;
  char str[SMG_SIZE];
...
int bw_fldstru::w_have_prev_rec(int p_fcnt,int wpp)
{
  int i,j,k,l;
  int s_tmpn1;
  int r1,r2;
  ...
  if (bw_xbase1.get_win_lnktab(p_fcnt)==0)
  {
    ...
    for (i=1;i<=s_tmpn1;i++)
    {
      bw_mbase1.w_up_rec(p_fcnt,wpp,bw_xbase1.get_win_recno(p_fcnt,wpp,1));
      if (bw_xbase1.win_state_dbf_err==1) return(0);
    }
    return(1);
  }
  else
  {
      ...
      while (1)
      {
	r2=bw_mbase1.w_up_rec(p_fcnt,wpp,r1);
	j=bw_buff1.get_linecol(wpp,bw_win1.pline,bw_win1.pfield);
	if (j>0) k=bw_win1.get_1fldseri(j);
	else k=1;
	l=bw_link1.w_link_number(wpp,p_fcnt);
