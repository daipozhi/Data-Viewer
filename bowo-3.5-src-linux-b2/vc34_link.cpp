...
int bw_link::w_read_link(char *p_fn,int p_fn_size,int wpp,int num)
{
  FILE *fp1;
  int   i,j,k,l;
  ...
  for (i=1+1;i<=num;i++)
  {
    while (1)
    {
      for (j=0;j<SMG_SIZE;j++) s_tmpc1[j]=0;
      fgets(s_tmpc1,SMG_SIZE,fp1);
      if (feof(fp1)) break;  
  ...
  while (1)
  {
    ...
    if (s_tmpc1[0]=='&') break;
    k=w_find_link_tn(s_tmpc1,SMG_SIZE,wpp,i);
    if (k==1)
    {
      fclose(fp1);
      return(1);
    ...
    if (bw_inkey1.strempty(s_tmpc1)==0) break;
    if (s_tmpc1[0]=='&') break;
    l=w_find_link_cd(s_tmpc1,SMG_SIZE,i,l);
    if (l<0)
    {
      fclose(fp1);
      return(1);
...
int bw_link::w_find_link_tn(char *p_s1,int p_s1_size,int wpp,int i)
{
  int  j,l,m,n;
  ...
  {
    if (bw_win1.get_win_pp_wins_dbf(m,wpp)==1)
    {
  ...
  {
    if (bw_win1.get_win_pp_wins_dbf(n,wpp)==1)
    {
...
//  int  j;
  ...
  if (l>=p_s1_size) return(-1);
  p2=l-1;
  bw_inkey1.strpcut(p_s1,p1,p2-p1+1,p_s1_size,tmpc2,SMG_SIZE);
  l=k;
...
int bw_link::w_link_modi(int wpp)  // fetch link var in cond // and link var if modi // prepare for link
{
  char s1[SMG_SIZE];
  char tn[SMG_SIZE];
  char fn[SMG_SIZE];
  ...
  for (i=0;i<COND_NUM;i++)
  {
    ...
    while (1)
    {
      p2=w_fetch_link_var(i,p2,s1,SMG_SIZE);
      if (p2<0) break;

      bw_win1.w_fetch_fld_style(s1,SMG_SIZE,tn,SMG_SIZE,&tr,fn,SMG_SIZE,wpp);
      if (bw_inkey1.strempty(fn)==0) continue;
      if (bw_inkey1.strempty(tn)==0) continue;

      for (j=0;j<STRU_NUM;j++)
      {
	if (bw_win1.get_win_pp_wins_dbf(j,wpp)==1)
        {
      ...
      w_link_modi_set(tn,SMG_SIZE,fn,SMG_SIZE,p1,wpp);
...
int bw_link::w_link_modi_set(char *p_tn,int p_tn_size,char *p_fn,int p_fn_size,int p1,int wpp)  // prepare for link
{
  int  i,j;
  char stn[FN_SIZE];
  char sf1[20];
...
int bw_link::w_link_modi_mark(int wpp,int mark,int seri,int recn)   // if changed ,mark it /* echo win rec ,link rec should change */
{
  int i,j,k;
...
//  char str[SMG_SIZE];
  ...
  while (1)
  {
    p2=w_fetch_link_var2(i,p2,s1,SMG_SIZE);
    if (p2<0) break;
...
int bw_link::w_get_sv(int wpp,int base,int seri,int recn,char *p_str,int p_str_size)  /*save scrn var to arr*/
{
  int i,j;
  int exist;
  ...
  for (j=get_win_svpoin(i,0);j<get_win_svpoin(i,1);j++)
  {
    if (j-get_win_svpoin(i,0)<p_str_size)
    {
      p_str[j-get_win_svpoin(i,0)]=get_win_svar(j);
    }
  }

  if (get_win_svpoin(i,1)-get_win_svpoin(i,0)<p_str_size)
  {
...
int bw_link::w_keep_linkfld(int wpp,int p_fcnt,int dir)
{
  ...
  l=get_win_lkseri(k);
  w_get_sv(wpp,j,l,1,str,SMG_SIZE);
  ...
  bw_fldstru1.w_sav_tab_var(str3,SMG_SIZE,p_fcnt,str,SMG_SIZE);
