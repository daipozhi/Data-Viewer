...
*/
      ...
      {
        if (bw_win1.get_win_pp_wins_dbf(j,bw_main1.win_hnd_id)==1)
        {
	...
	bw_win1.set_win_create_menu(bw_main1.win_hnd_id,0);
      ...
      if (bw_win1.get_win_ci_mode(bw_main1.win_hnd_id)==0)
      {
	MoveWindow(hwnd
      ...
      k=bw_link1.w_read_link(s_fn1,FN_SIZE,bw_main1.win_hnd_id,bw_main1.win_hnd_id-bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(bw_main1.win_hnd_id),1)+1);
      l=0;
	...
	bw_win1.set_win_is_link(bw_main1.win_hnd_id,1);
      ...
      if (bw_win1.get_win_style(bw_main1.win_hnd_id,2)==1) bw_win1.w_echo_win_radio(bw_main1.win_hnd_id,1);

      if (bw_win1.get_win_style(bw_main1.win_hnd_id,3)==1) bw_win1.w_echo_win_check(bw_main1.win_hnd_id,1);

      if (bw_win1.get_win_style(bw_main1.win_hnd_id,9)==1) bw_win1.w_echo_win_edit(bw_main1.win_hnd_id,1);

      bw_win1.w_adjust_edit(bw_main1.win_hnd_id);

      bw_win1.set_win_pp_get_rd(bw_main1.win_hnd_id,bw_getread1.get_read_login());

      bw_win1.set_win_linep(bw_main1.win_hnd_id,0,1);  // cursor position
	...
	bw_dialog1.w_echo_winrec_var(bw_main1.win_hnd_id); //wpp to 0
	...
	if (i>0) j=bw_win1.get_1fldbase(i);
	else j=0;

	bw_win1.w_find_winfname(bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,str,SMG_SIZE);
...
//	ReleaseDC(win_hwnd,hdc);
	...
	if (str[0]=='?') bw_dialog1.w_mv_get_read_paint(str,SMG_SIZE,bw_main1.win_hnd_id,'g'); // wpp to 0
	else
        {
	  if (str[0]==':')
	  {
            if (bw_xbase1.win_answer==0)
            {
	      bw_mbase1.w_down_rec(j,bw_main1.win_hnd_id,bw_xbase1.get_win_recno(j,bw_main1.win_hnd_id,0)-1);
	      bw_fldstru1.w_tv_get_read_paint(str,SMG_SIZE,bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,'g');
            }
	  }
	  else
	  {
...
*/      
	  ...
	  {
	    bw_win1.pline =bw_win1.get_win_h_ci_pp(bw_win1.win_menu_seri,2);
	    bw_win1.pfield=bw_win1.get_win_h_ci_pp(bw_win1.win_menu_seri,1);
          }
...
/*	  return(0);*/
	...
	if (str[0]=='[')
	{
	  ...
	  SendMessage(bw_win1.get_win_hnd(j),BM_SETSTATE,0,(LPARAM)str);
...
/*	  return(0);*/
	...
	if ((str[0]=='?')||(str[0]=='-'))
	{
	  ...
	  if (str[0]=='?')
          {
	    k=bw_dialog1.w_mv_get_read_r(str,SMG_SIZE,bw_main1.win_hnd_id,'r');
	  }
	  else
	  {
	    k=1;
	  ...
	  if (k==1)
	  {
	    if (str[0]=='?')
            {
	      bw_getread1.get_smg_string(bw_win1.get_win_pp_get_rd(bw_main1.win_hnd_id),sf1,SMG_SIZE);
	    }
	    else
	    {
		...
		GetWindowText(hnd,sf1,SMG_SIZE);
	      }
              else sf1[0]=0;
	    }

	    bw_xbase1.set_win_scrnvar(bw_main1.win_hnd_id,sf1,SMG_SIZE);
	    bw_dialog1.set_win_mv_getv(bw_main1.win_hnd_id,sf1,SMG_SIZE);

	    bw_dialog1.w_mv_sav_var(str,SMG_SIZE,bw_main1.win_hnd_id);
	    ...
	    if (bw_main1.win_key==CTRL_E)
	    {
	      bw_dialog1.setc_win_mv_getv(bw_main1.win_hnd_id,0,0);
	    }
...
/*	    return(0);*/
	...
	if ((str[0]==':')||(str[0]=='+'))
	{
	  ...
	  m=bw_win1.get_1fldbase(l);

	  if (str[0]==':')
          {
	    k=bw_fldstru1.w_tv_get_read_r(str,SMG_SIZE,bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,'r');
	  }
	  else
	  {
            k=1; // &+fx ci
	  ...
	  if (k==1)
	  {
	    bw_inkey1.strpcut(str,1,strlen(str)-1,SMG_SIZE,fldname,SMG_SIZE);
	    m=bw_win1.get_1fldbase(l);
...
//	    MessageBox(hwnd,str2,"aa",MB_OK);
	    ...
	    if (bw_main1.win_key==CTRL_E)
	    {
	      if (str[0]==':') bw_xbase1.setc_win_scrnvar(bw_main1.win_hnd_id,0,0);
	      else
	      {
		q=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
	        if (q!=0)
...
/*	    return(0);*/
	...
	if (i>0)
	{
	  ...
	  bw_dialog1.w_echo_winrec_read(j,bw_main1.win_hnd_id,0/*bw_win1.win_pp21*/);
        }
...
//	      MessageBox(hwnd,str,"aa",MB_OK);
	...
	if (m==1)
	{
	  ...
	  bw_dialog1.set_win_mv_getp(bw_main1.win_hnd_id,bw_main1.win_field_posi);
          ...
          k=bw_win1.get_1fldbase(i);
          bw_win1.get_1fldname(i,str2,SMG_SIZE);

	  if (str2[0]==':')
	  {
            if (bw_xbase1.get_win_lnktab(k)==0) // not linked table
            {

              r1=bw_xbase1.get_win_recno(k,bw_main1.win_hnd_id,1)-1;  // to find out if there are n records
              r2=0;
              exist=1;
              ...
              for (i=1;i<=j;i++)
              {
                r2=bw_mbase1.w_down_rec(k,bw_main1.win_hnd_id,r1);
                if (bw_xbase1.win_state_dbf_err==1)
		{
		  ...
		  break;
              ...
              if (exist==0) // don't have so many records, locate at old position,old tab
	      {
	        bw_win1.pline =bw_win1.get_win_linep(bw_main1.win_hnd_id,0);
	        bw_win1.pfield=bw_win1.get_win_linep(bw_main1.win_hnd_id,1);
	      }
              ...
              bw_win1.set_win_linep(bw_main1.win_hnd_id,0,bw_win1.pline); // to be ready
              bw_win1.set_win_linep(bw_main1.win_hnd_id,1,bw_win1.pfield);
              ...
              if (bw_xbase1.get_win_lnktab(k)==0) // not linked table,maybe old tab
              {
                bw_xbase1.set_win_cuseri(k,bw_main1.win_hnd_id,j);

                r1=bw_xbase1.get_win_recno(k,bw_main1.win_hnd_id,1)-1;
                r2=0;
                ...
                bw_xbase1.set_win_recno(k,bw_main1.win_hnd_id,0,r2);
	      }
	      else // linked tab,maybe old tab
	      {
                bw_xbase1.set_win_cuseri(k,bw_main1.win_hnd_id,j);
	            ...
	            l=bw_link1.w_link_number(bw_main1.win_hnd_id,k);
                    r1=r2;
                    if (bw_link1.w_link_cond_ok(l,bw_main1.win_hnd_id,k,n,bw_main1.win_hnd_id)==1) break;
                    if (bw_xbase1.win_state_dbf_err==1) break;
                  }
                  if (bw_xbase1.win_state_dbf_err==1) break;
                }
	          ...
	          while (1)
	          {
	            r2=bw_mbase1.w_up_rec(k,bw_main1.win_hnd_id,r1);
	            m=bw_buff1.get_linecol(bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield);
	            if (m>0) n=bw_win1.get_1fldseri(m);
	            else n=1;
                    l=bw_link1.w_link_number(bw_main1.win_hnd_id,k);
	          ...
	          if (bw_xbase1.win_state_dbf_err==1)
                  {
                    //bw_xbase1.set_win_recno(k,bw_main1.win_hnd_id,0,1);
                    r2=1;
                  }
                }
            ...
            else  // linked table
            {
              ...
              if (exist==0) // don't have so many records, locate at old position,maybe old tab
              {
                bw_win1.pline =bw_win1.get_win_linep(bw_main1.win_hnd_id,0);
                bw_win1.pfield=bw_win1.get_win_linep(bw_main1.win_hnd_id,1);
              }
...
//	  hcursor=LoadCursor(NULL,"cursor2.cur");
    ...
    case WM_PROGRAM1:
      ...
      bw_getread1.set_smg_string(bw_win1.get_win_pp_get_rd(bw_main1.win_hnd_id),str2,SMG_SIZE);
...
//  HDC  hdc;
  ...
  switch(msg)
  {
	...
	if ((bw_main1.win_key==274)||
	    ...
	    (bw_main1.win_key<=32))
	{
	  k=bw_win1.get_win_h_ci_pp(bw_win1.win_menu_seri,4);
	  bw_main1.win_key=10000+bw_main1.win_key;
	  SendMessage(bw_win1.get_win_hnd(k),WM_CHAR,1002,(LPARAM)str);
	}
      }
    }
    case WM_LBUTTONUP:
  ...
  return (CallWindowProc((WNDPROC)bw_win1.get_win_fpp(j),hwnd,msg,wp,lp));
}
...
int bw_main::w_fetch_filename(char *p_s1,int p_s1_size)
{
  ...
  if (exist==1)
  {
    bw_inkey1.strpcut(p_s1,  0,               i,p_s1_size,win_seadir ,FN_SIZE);
    bw_inkey1.strpcut(p_s1,i+1,strlen(p_s1)-i-1,p_s1_size,win_seaname,FN_SIZE);
...
int bw_main::set_string(char *p_s1,int p_s1_size,char *p_s2,int p_s2_size)
{
  int i;
...
int bw_main::get_view(char *p_str,int p_str_size,HWND hwnd,int pp5)
{
  HMENU hm1;
  HWND  hwnd2;
...
/*
  if (bw_win1.get_win_subhndpp(pp5)+1>=SUB_WIN_NUM)
  {
    bw_win1.w_atten67();
    return(1);
  }
...
//  MessageBox(bw_main1.win_hwnd1,"bb","bb",MB_OK);
  ...
  bw_main1.win_hnd_id=bw_win1.win_pp_new_win;

  tmpn1=bw_main1.win_hnd_id;

  i=1;
  strcpy(bw_main1.win_m_bs2,bw_main1.win_m_bs1);
  if (bw_main1.win_lang==1) strcat(bw_main1.win_m_bs2,".sc3");
  else strcat(bw_main1.win_m_bs2,".sc4");
  //strcat(bw_main1.win_m_bs2,".sc3");
  if (bw_inkey1.w_find_file(bw_main1.win_m_bs2)==1)
  {
    i=bw_win1.w_read_winfile(bw_main1.win_m_bs2,FN_SIZE);
  }
  else
  {
  ...
  if (i!=0)
  {
    bw_win1.win_pp_new_win=tmpn1;
    return(1);
  }

  tmpn2=bw_win1.win_pp_new_win;

  m=1;
  strcpy(bw_main1.win_m_bs2,bw_main1.win_m_bs1);
  strcat(bw_main1.win_m_bs2,".tb1");
  if (bw_inkey1.w_find_file(bw_main1.win_m_bs2)==1)
      m=bw_menuvar1.w_read_mdc(bw_main1.win_m_bs2,FN_SIZE,bw_main1.win_hnd_id);
  else
  {
    bw_inkey1.w_get_dir(win_work_dir,FN_SIZE);
    bw_inkey1.w_set_dir(win_home_dir);
  ...
  bw_win1.win_pp_new_win     = tmpn2;

  m=1;
  strcpy(bw_main1.win_m_bs2,bw_main1.win_m_bs1);
  strcat(bw_main1.win_m_bs2,".tb6");
  if (bw_inkey1.w_find_file(bw_main1.win_m_bs2)==1)
      m=bw_choose1.w_read_cdc(bw_main1.win_m_bs2,FN_SIZE,bw_main1.win_hnd_id);
  else
  {
    bw_inkey1.w_get_dir(win_work_dir,FN_SIZE);
    bw_inkey1.w_set_dir(win_home_dir);
  ...
  if (bw_inkey1.w_find_file(bw_main1.win_m_bs2)==1)
      m=bw_switch1.w_read_sdc(bw_main1.win_m_bs2,FN_SIZE,bw_main1.win_hnd_id);
  else
  {
    bw_inkey1.w_get_dir(win_work_dir,FN_SIZE);
    bw_inkey1.w_set_dir(win_home_dir);
  ...
  if (bw_inkey1.w_find_file(bw_main1.win_m_bs2)==1)
      m=bw_dialog1.w_read_ddc(bw_main1.win_m_bs2,FN_SIZE,bw_main1.win_hnd_id);
  else
  {
    bw_inkey1.w_get_dir(win_work_dir,FN_SIZE);
    bw_inkey1.w_set_dir(win_home_dir);
  ...
  if (open_base_ok==0)
  {
    for (j=0;j<STRU_NUM;j++)
    {
      ...
      if ((bw_win1.get_win_pp_wins_dbf(j,bw_main1.win_hnd_id)==1)&&(open_base[j]==1))
      {
	bw_win1.get_win_tabname(1,j,bw_main1.win_m_bs2,FN_SIZE);

	bw_mbase1.w_close_base(j,0);
      }
      if ((bw_win1.get_win_pp_wins_dbf(j,bw_main1.win_hnd_id)==1)&&(open_stru[j]==1))
      {
    ...
    bw_win1.win_pp_new_win=tmpn1;    // add close_win() part
  ...
  for (m=bw_main1.win_hnd_id;m<bw_win1.win_pp_new_win;m++)
  {
    bw_win1.set_win_create_menu(m,o);
  }

  j=bw_win1.win_h_w_total_pp;

  bw_win1.w_login_wwin(bw_main1.win_hnd_id);

  l=bw_win1.get_win_pp_grp_id(bw_main1.win_hnd_id);

  for (k=bw_win1.get_win_pp_grp_pp(l,0);k<=bw_win1.get_win_pp_grp_pp(l,2);k++)
  {
   ...
   bw_win1.set_win_hnd(j+k-bw_win1.get_win_pp_grp_pp(l,0),bw_win1.get_win_phh(k));

  }
/*
  bw_win1.set_win_subhndid(pp5,bw_win1.get_win_subhndpp(pp5),l);
  bw_win1.set_win_subhndpp(pp5,1+bw_win1.get_win_subhndpp(pp5));

  bw_win1.set_win_subhndpp(l+1,0);
*/
  return(0);
}
...
*/
    ...
    bw_menuvar1.w_menu_poin(str,SMG_SIZE,bw_main1.win_hnd_id);
    bw_menuvar1.w_menu_comm(bw_menuvar1.t5_m_pp3,1);

    if (bw_menuvar1.t5_m_pp2==26)
    {
      bw_bmp1.echo_bmp(hwnd,bw_menuvar1.t5_m_como,SMG_SIZE,bw_menuvar1.get_t5_m_mcnt(bw_menuvar1.t5_m_pp3),bw_main1.win_hnd_id);

      bw_menuvar1.set_t5_m_mcnt(bw_menuvar1.t5_m_pp3,bw_menuvar1.get_t5_m_mcnt(bw_menuvar1.t5_m_pp3)+1);
    }
  }
  if (str[0]=='[')
  {
    j=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
    SendMessage(bw_win1.get_win_hnd(j),BM_SETSTATE,11,(LPARAM)str);
  }
  if (str[0]=='(')
  {
    j=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
  ...
  {
    ShowCaret(hwnd);
    bw_dialog1.w_mv_get_read_g(str,SMG_SIZE,bw_main1.win_hnd_id,'g');
  }
  if ((str[0]==':')||(str[0]=='+'))
  {
    i=bw_buff1.get_linecol(bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield);
    ...
    if (l==0)
    {
      bw_fldstru1.w_tv_get_read_g(str,SMG_SIZE,bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,'g');
    }
    else
    {
...
//
      ...
      if ((str[0]==':')||(str[0]=='+'))
      {
	bw_mbase1.w_down_rec(k,bw_main1.win_hnd_id,bw_xbase1.get_win_recno(k,bw_main1.win_hnd_id,0)-1);
	bw_fldstru1.w_tv_get_read_g(str,SMG_SIZE,bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,'g');
      }
...
//  char sf1[SMG_SIZE];
  ...
  bw_win1.w_find_winfname(hnd_id,bw_win1.pline,bw_win1.pfield,str,SMG_SIZE);

  if (str[0]=='@')
  {
    bw_menuvar1.w_menu_poin(str,SMG_SIZE,hnd_id);
    bw_menuvar1.w_menu_comm(bw_menuvar1.t5_m_pp3,0);

    if (bw_menuvar1.t5_m_pp2==1)
    {
      bw_main1.get_view(bw_menuvar1.t5_m_como,SMG_SIZE,bw_main1.win_hwnd,bw_win1.get_win_pp_grp_pp(hnd_id,0));
      return(0);
    }
    ...
    if (bw_menuvar1.t5_m_pp2==18) bw_fldstru1.w_insert_rec(j,hnd_id);
    if (bw_menuvar1.t5_m_pp2==27) bw_fldstru1.w_add_rec(j,hnd_id);
    if (bw_menuvar1.t5_m_pp2==28) bw_fldstru1.w_up_line2(j,hnd_id);
    if (bw_menuvar1.t5_m_pp2==29) bw_fldstru1.w_down_line2(j,hnd_id);
  ...
  if (str[0]=='[')
  {
    ...
    i=SendMessage(bw_win1.get_win_hnd(j),BM_GETCHECK,11,(LPARAM)str);
    if (i==0) SendMessage(bw_win1.get_win_hnd(j),BM_SETCHECK,11,(LPARAM)str);
    else  SendMessage(bw_win1.get_win_hnd(j),BM_SETCHECK,0,(LPARAM)str);
...
int bw_main::get_ci_save(int hnd_id)
{
  ...
  if ((str1[0]=='?')||(str1[0]=='-'))
  {
    if (str1[0]=='?')
    {
      bw_getread1.get_smg_string(bw_win1.get_win_pp_get_rd(hnd_id),sf1,SMG_SIZE);
      bw_xbase1.set_win_scrnvar(hnd_id,sf1,SMG_SIZE);
      bw_dialog1.set_win_mv_getv(hnd_id,sf1,SMG_SIZE);
    }
    else
    {
  ...
  if ((str1[0]==':')||(str1[0]=='+'))
  {
    m=bw_buff1.get_linecol(hnd_id,bw_win1.get_win_linep(hnd_id,0),bw_win1.get_win_linep(hnd_id,1));
    if (m>=0) m=bw_win1.get_1fldbase(m);

    bw_inkey1.strpcut(str1,1,strlen(str1)-1,SMG_SIZE,fldname,SMG_SIZE);
...
*/
    ...
    {
      win_page_size=66;
      p2=bw_inkey1.str2int(tc5,strlen(tc5)+1);
    ...
    if (p1==6)
    {
      ...
      bw_inkey1.strpcut(tc5,0,i,SMG_SIZE,tc6,SMG_SIZE);
      bw_inkey1.strpcut(tc5,i+1,strlen(tc5)-i-1,SMG_SIZE,tc7,SMG_SIZE);
