...
int bw_win::w_dele_win_pp_wins_dbf(int wpp)
{
  int i,j;

  for (j=get_win_pp_grp_pp(get_win_pp_grp_id(wpp),0)
      ;j<=get_win_pp_grp_pp(get_win_pp_grp_id(wpp),2)
      ;j++)
  {
...
//  char str[SMG_SIZE];
  ...
  for (k=i+1;k<=l;k++)
  {
	...
	set_win_pp_wins_dbf(o,m-n,get_win_pp_wins_dbf(o,m));
      ...
      bw_dialog1.get_win_mv_getv(m,sm1,SMG_SIZE);
      bw_dialog1.set_win_mv_getv(m-n,sm1,SMG_SIZE);
...
//  char str[FN_SIZE];
  ...
  for (k=win_pp_new_win;k<WIN_NUM-1;k++)
  {
    ...
    {
      cmd=0;
      for (n=0;n<SMG_SIZE*2;n++) ts[n]=' ';
      fgets(ts,SMG_SIZE*2,fp1);
      if (feof(fp1))
      {
	ends=1;
	break;
	...
	continue;
      ...
      {
	if ((ts[j]<' ')&&(ts[j]>=0)) ts[j]=' ';
	bw_buff1.set_t_winfile(k,lcnt,j+1,ts[j]);
      }
  ...
  w_login_win(m,k,p_fn2,p_fn2_size,tmpc7,FN_SIZE);
  ...
  if (is_tab==1)  /* it is a table scrn */
  {
	...
	set_win_pp_wins_dbf(tmpc6[j],i,1);
	bw_xbase1.set_win_state_bff_in_use(tmpc6[j],1);
      }
    }
...
int bw_win::w_dele_xbase_state(int wpp)
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    if (get_win_pp_wins_dbf(i,wpp)==1)
    {
...
int bw_win::w_echo_win_pa(int p_pagecnt,int mark)  //paint win
{ 
...
int bw_win::w_echo_win_cr(int p_pagecnt,int mark) // create button ci
{ 
//  HDC  hdc;
//  HWND hnd;
	...
	if (bw_buff1.get_t_winfile(p_pagecnt,i,j)<=' ')
	{
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
		...
		if (s2[0]=='@')
		{
		  ...
		  for (l=t1+1;l<t2;l++)
		  {
		    s1[l-t1-1]=bw_buff1.get_t_winfile(p_pagecnt,i,l);
		    s1[l-t1]=0;
                  }

		  w_cre_ci_button(p_pagecnt,s1,SMG_SIZE,p6,p7,p8,i,s2,strlen(s2));
...
int bw_win::w_cre_ci_button(int p_pagecnt,char *p_s1,int p_s1_size,int p6,int p7,int p8,int p9,char *p_s2,int len2)
{
  HWND hnd;
  int  l,j;
  //char s2[SMG_SIZE];
  ...
  win_h_ci_total_pp++;
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5,win_h_ci_total_pp-1);
  set_win_h_ci_pp(win_h_ci_total_pp-1,0,p_pagecnt);
  set_win_h_ci_pp(win_h_ci_total_pp-1,1,p6);
  set_win_h_ci_pp(win_h_ci_total_pp-1,2,p9);
  set_win_h_ci_pp(win_h_ci_total_pp-1,3,win_h_ci_total_pp-1);
  set_win_h_ci_pp(win_h_ci_total_pp-1,4,p_pagecnt-get_win_pp_grp_pp(get_win_pp_grp_id(p_pagecnt),0)+get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),0));
...
int bw_win::w_cre_ci_radio(int p_pagecnt,int p6,int p7,int p8,int p9,char *p_str,int len)
{
  HWND hnd;
  char str[SMG_SIZE];
...
int bw_win::w_cre_ci_check(int p_pagecnt,int p6,int p7,int p8,int p9,char *p_str,int len)
{
  HWND hnd;
  char str[SMG_SIZE];
...
int bw_win::w_echo_win_edit(int p_pagecnt,int mark) // create edit ci
{
//  HDC  hdc;
//  HWND hnd;
...
int bw_win::w_echo_win_radio(int p_pagecnt,int mark) // create radio ci
{
//  HDC  hdc;
//  HWND hnd;
  int  p1,p2,p4,p5,p6,p7,p8;
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(s2)==1)
		{
		  w_cre_ci_radio(p_pagecnt,p6,p7,p8,i,s2,strlen(s2));
...
int bw_win::w_echo_win_check(int p_pagecnt,int mark) // create choose ci
{ 
//  HDC  hdc;
//  HWND hnd;
  int  p1,p2,p4,p5,p6,p7,p8;
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(s2)==1)
		{
		  w_cre_ci_check(p_pagecnt,p6,p7,p8,i,s2,strlen(s2));
...
int bw_win::w_fetch_fld_style(char *tfn,int tfn_size,char *tn,int tn_size,long *tr,char *fn,int fn_size,int wpp)
{
  int  i,j,k,l;
  int  p1;
  int  s_pp1[3][2];
  ...
  *tr=0;
  ...
  if (j==0)  /* 1 jie */
  {
    i=w_find_base_name(s_fld[0],SMG_SIZE,wpp);
    k=w_find_recno_name(s_fld[0],SMG_SIZE);
    l=w_find_fld_name(s_fld[0],SMG_SIZE,wpp);
    if (i==1)
    {
  ...
  if (j==1)  /* 2 jie */
  {
    ...
    i=w_find_base_name(s_fld[1],SMG_SIZE,wpp);
    k=w_find_recno_name(s_fld[1],SMG_SIZE);
    l=w_find_fld_name(s_fld[1],SMG_SIZE,wpp);
    if (i==1)
    {
      if (p1==1)
      {
  ...
  if (j==2)  /* 3 jie */
  {
    i=w_find_base_name(s_fld[0],SMG_SIZE,wpp);
    k=w_find_recno_name(s_fld[1],SMG_SIZE);
    l=w_find_fld_name(s_fld[2],SMG_SIZE,wpp);
    if (i==1)
    {
      if ((int)strlen(s_fld[0])<tn_size)
      {
...
int bw_win::w_find_base_name(char *p_str,int p_str_size,int wpp)
{
  int i,j;
  char stn[FN_SIZE];
  //char stm[SMG_SIZE];
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    if ((get_win_pp_wins_dbf(i,wpp)==1)&&(strcmp(stn,p_str)==0))
    {
      j=1;
      break;
    }
...
int bw_win::w_find_fld_name(char *p_str,int p_str_size,int wpp)
{
  int  i,j,k;
  char fn1[SMG_SIZE];
  //char stm[SMG_SIZE];
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    if (get_win_pp_wins_dbf(i,wpp)==1)
    {
      ...
      {
        bw_fldstru1.w_find_fldname(i,j,fn1,SMG_SIZE);
...
int bw_win::w_fld_set_base(char *p_fldtable,int p_fldtable_size,int pp1,int wpp)
{
  int  i,j,k;
  char stn[FN_SIZE];
  //char stm[SMG_SIZE];
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    if ((get_win_pp_wins_dbf(i,wpp)==1)&&(strcmp(p_fldtable,stn)==0))
    {
      k=1;
      break;
    }
...
//  char str2[SMG_SIZE];
  ...
  for (i=get_win_pp_fld_in_win(wpp,0);i<=get_win_pp_fld_in_win(wpp,1);i++)
  {
    set_1fldbase(i,0);
    set_1fldedt(i,0);
    set_1fldseri(i,0);
	...
	if ((s2[0]==':')||(s2[0]=='%')||(s2[0]=='+')||(s2[0]=='-'))
	{
	  ...
	  if ((s_tmpc2[0]=='%')||(s_tmpc2[0]=='-'))
	  {
	    ...
	    s_tmpn1=1; /* read field */
            bw_inkey1.strpcut(s_tmpc2,1,strlen(s_tmpc2)-1,SMG_SIZE,s_tmpc3,SMG_SIZE);
          }
	  else
	  {
	    str3[0]=s_tmpc2[0];
	  ...
	  w_fetch_fld_style(s_tmpc3,SMG_SIZE
			   ,s_fldtable,SMG_SIZE
			   ,&s_fldrecno
			   ,s_fldname,SMG_SIZE,wpp);
	  ...
	  pp1=w_fld_set_base(s_fldtable,SMG_SIZE,pp1,wpp);
	  set_1fldbase(n,pp1);
	  ...
	  l=0;
	  for (k=bw_buff1.get_linecol(m,i,j)-1;k>=get_win_pp_fld_in_win(wpp,0);k--)
	  {
...
int bw_win::w_adjust_edit(int wpp)
{
  HWND hnd;
...
*/
int bw_win::get_win_state_link_modi(int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_state_link_modi[wpp]);
}
int bw_win::set_win_state_link_modi(int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_state_link_modi[wpp]=val;
  return(0);
}
int bw_win::get_win_state_calc_modi(int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_state_calc_modi[wpp]);
}
int bw_win::set_win_state_calc_modi(int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_state_calc_modi[wpp]=val;
  return(0);
}
...
int bw_win::get_win_pp_fld_in_win(int wpp,int pp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>3)) return(0);
  return(win_pp_fld_in_win[wpp][pp]);
}
int bw_win::set_win_pp_fld_in_win(int wpp,int pp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>3)) return(0);
  win_pp_fld_in_win[wpp][pp]=val;
  return(0);
}
int bw_win::get_win_h_w_pp(int wpp,int pp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>8)) return(0);
  return(win_h_w_pp[wpp][pp]);
}
int bw_win::set_win_h_w_pp(int wpp,int pp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>8)) return(0);
  win_h_w_pp[wpp][pp]=val;
  return(0);
}
int bw_win::get_win_h_up_pp(int wpp,int pp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>1)) return(0);
  return(win_h_up_pp[wpp][pp]);
}
int bw_win::set_win_h_up_pp(int wpp,int pp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>1)) return(0);
  win_h_up_pp[wpp][pp]=val;
  return(0);
}
...
int bw_win::get_win_tabname(int p1,int p_fcnt,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_win::set_win_tabname(int p1,int p_fcnt,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_win::get_win_pp_win_mark(int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_pp_win_mark[wpp]);
}
int bw_win::set_win_pp_win_mark(int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_pp_win_mark[wpp]=val;
  return(0);
}
int bw_win::get_win_pp_grp_id(int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_pp_grp_id[wpp]);
}
int bw_win::set_win_pp_grp_id(int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_pp_grp_id[wpp]=val;
  return(0);
}
...
int bw_win::get_win_pp_grp_pp(int wpp,int p04)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((p04<0)||(p04>=5)) return(0);
  return(win_pp_grp_pp[wpp][p04]);
}
int bw_win::set_win_pp_grp_pp(int wpp,int p04,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((p04<0)||(p04>=5)) return(0);
  win_pp_grp_pp[wpp][p04]=val;
  return(0);
}
int bw_win::get_win_pp_wins_dbf(int p_fcnt,int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((p_fcnt<0)||(p_fcnt>=STRU_NUM)) return(0);
  return(win_pp_wins_dbf[p_fcnt][wpp]);
}
int bw_win::set_win_pp_wins_dbf(int p_fcnt,int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  if ((p_fcnt<0)||(p_fcnt>=STRU_NUM)) return(0);
  win_pp_wins_dbf[p_fcnt][wpp]=val;
  return(0);
}
...
int bw_win::get_win_pp_get_rd(int wpp)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_pp_get_rd[wpp]);
}
int bw_win::set_win_pp_get_rd(int wpp,int val)
{
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_pp_get_rd[wpp]=val;
  return(0);
}
...
int bw_win::get_win_window_name(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_win::set_win_window_name(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_win::get_win_find_name(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_win::set_win_find_name(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
