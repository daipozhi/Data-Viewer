...
{         
  ...
  set_win_rcnt(p_fcnt,0);    // total record number
  set_win_lcnt(p_fcnt,0,0);  // field number
  set_win_lcnt(p_fcnt,1,0);  // record length

  set_win_lnktab(p_fcnt,0);

  set_win_state_bff_in_use(p_fcnt,0);
  set_win_state_dbf_modi(p_fcnt,0);
...
int bw_xbase::w_open_xbase(char *p_fn1,int p_fn1_size,int backup,int p_fcnt)
{
  char          s_fn1[FN_SIZE];
  char          s_fn2[FN_SIZE];
  char          s_fn3[FN_SIZE];
  ...
  while(1)
  {
    win_fp[p_fcnt]=/*s*/open(s_fn1,O_RDWR/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
    if (win_fp[p_fcnt]>=0) break;           /* open success */
    else
    {
  ...
  if (backup==1)
  {
    ...
    win_fp[p_fcnt]=/*s*/open(s_fn2,O_RDWR/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
    if (win_fp[p_fcnt]<0) return(1);

  }

  set_win_state_bff_in_use(p_fcnt,1);

  getcwd(str,FN_SIZE);
  set_win_basedir(p_fcnt,str,FN_SIZE);
  set_win_basename(p_fcnt,p_fn1,p_fn1_size);
...
int bw_xbase::w_create_xbase(char *p_fn1,int p_fn1_size,int p_fcnt)
{
  int           i,j,k;
  int           s_fp1;
  char          s_tmpc1[16+1];
...
//  char          c1,c2;
  ...
  unsigned char s_tmpc1[SMG_SIZE];

  j=read(p_fp,s_tmpc1,16);
  if (/*eof(p_fp)*/j<16)
  {
    set_win_rcnt(p_fcnt,0);
    return(1);
...
int bw_xbase::w_loc_xbase_rec(long recno,int p_fcnt)
{
  ...
  if (recno<=0) return(1);
  //if (recno>get_win_rcnt(p_fcnt)) return(1);  //get_win_rcnt(p_fcnt) total record number

  i=(get_win_lcnt(p_fcnt,0)+1)*32+1+(recno-1)*(get_win_lcnt(p_fcnt,1)+1); //get_win_lcnt(p_fcnt,0) field number,get_win_lcnt(p_fcnt,1) record length

  lseek(win_fp[p_fcnt],i,0);

  j=read(win_fp[p_fcnt],str,2);
  lseek(win_fp[p_fcnt],i,0);
...
//  long recno1;
  ...
  {
    recno++;
    s_state1=w_read_xbase_rec(recno,p_fcnt);  // read current rec
    for (i=0;i<=get_win_lcnt(p_fcnt,1);i++) set_win_recvar2(i,1,get_win_recvar(i)); // put current rec to buffer 1
    for (i=0;i<=get_win_lcnt(p_fcnt,1);i++) set_win_recvar(i,get_win_recvar2(i,0)); // put buffer 0(prev rec) to current rec
    if (s_state1==0) w_save_xbase_rec(recno,p_fcnt); // write prev rec
    else
    {
      w_append_xbase_rec(recno,p_fcnt);
      set_win_rcnt(p_fcnt,get_win_rcnt(p_fcnt)+1);
    ...
    for (i=0;i<=get_win_lcnt(p_fcnt,1);i++) set_win_recvar2(i,0,get_win_recvar2(i,1)); // put buffer 1 to buffer 0
  }
}

int bw_xbase::w_insert_xbase_rec2(int p_fcnt) // for empty database ,add three empty rec
{
  long recno1;
  int  s_state1;
  int  s_state2;
  ...
  w_loc_xbase_rec(1,p_fcnt);
  write_win_recvar(win_fp[p_fcnt],get_win_lcnt(p_fcnt,1)+1);

  w_loc_xbase_rec(2,p_fcnt);
  write_win_recvar(win_fp[p_fcnt],get_win_lcnt(p_fcnt,1)+1);

  w_loc_xbase_rec(3,p_fcnt);
  write_win_recvar(win_fp[p_fcnt],get_win_lcnt(p_fcnt,1)+1);
...
//  char  c1;
  ...
  get_win_ext(p_fcnt,s_tmpc2,SMG_SIZE);

  strcpy(s_tmpc5,"tmp_pack");
  w_find_empty_file(s_tmpc5,SMG_SIZE,p_fcnt);
  get_win_ext(p_fcnt,s_tmpc4,SMG_SIZE);
  set_win_ext(p_fcnt,s_tmpc2,SMG_SIZE);
  ...
  while (1)
  {
    ...
    if (/*eof(win_fp[p_fcnt])*/j<get_win_lcnt(p_fcnt,1)+1) break;
  ...
  while (1)
  {
    ...
    s_fl=s_fl+s_fl2;
    if (/*eof(s_fh)*/s_recno1<REC_LEN) break;
  }
...
int bw_xbase::w_commit_xbase(int backup,int p_fcnt)
{
  int  i,j,k;
  int  s_state1;
  char s_fn1[FN_SIZE];
  ...
  if (get_win_state_dbf_modi(p_fcnt)==0) return(0);
  ...
  get_win_basename(p_fcnt,s_fn1,FN_SIZE);
...
int bw_xbase::w_rollback_xbase(int backup,int p_fcnt)
{
  long s_recno1;
  int  i,j,k;
  char s_fn1[FN_SIZE];
...
//  int  s_state1;
  ...
  bw_win1.get_win_tabname(1,p_fcnt,s_fn2,FN_SIZE);
  ...
  bw_win1.get_win_tabname(1,p_fcnt,s_fn1,FN_SIZE);
  bw_inkey1.strtrim(s_fn1,FN_SIZE);
  i=bw_inkey1.strlocc(s_fn1,FN_SIZE,'.');
  s_fn1[i]=0;
  strcat(s_fn1,".dbf");
...
int bw_xbase::w_append_sdf(char *p_fn,int p_fn_size,int p_fcnt)
{
  long recno;
  int  i,j;
...
int bw_xbase::w_copy_sdf(char *p_fn,int p_fn_size,int p_fcnt)
{
  long  i;
  int   j;
//  int   exist;
...
int bw_xbase::w_find_empty_file(char *p_str,int p_str_size,int p_fcnt)
{
  int   i,j,k,l;
  char  s_stc1[SMG_SIZE];
  char  s_stc2[FN_SIZE];
//  char  sc1[FN_SIZE];
  //char  s_tm[SMG_SIZE];
  char  c1,c2;
...
int bw_xbase::w_find_empty_ffile(char *p_str,int p_str_size,int wpp)
{
  int   i,j,k,l;
  char  s_stc1[SMG_SIZE];
  char  s_stc2[FN_SIZE];
...
int bw_xbase::w_find_empty_mfile(char *p_str,int p_str_size,int p_fcnt)
{
  int   i,j,k,l;
  char  s_stc1[SMG_SIZE];
  char  s_stc2[FN_SIZE];
...
int bw_xbase::w_close_xbase(int p_fcnt)
{
  ...
  //printf("modi=%d,backup=%d,\n",get_win_state_dbf_modi(p_fcnt),get_win_base_backup(p_fcnt));

  if ((get_win_state_dbf_modi(p_fcnt)==1)&&
      (get_win_base_backup(p_fcnt)==1))
  {
    get_win_basename(p_fcnt,s_fn1,FN_SIZE);

    i=bw_win1.w_atten9(s_fn1); // if to save
    if (i==2) { /* not save file*/ }
    else        // save file
    {
      /* make backup file name */
      strcpy(s_fn2,s_fn1);
  ...
  if (get_win_base_backup(p_fcnt)==1)
  {
...
//  char          str[SMG_SIZE];
  ...
  j2=read(win_use_fh,s_tmpc1,16);
  if (/*eof(win_use_fh)*/j2<16) return(1);
...
int bw_xbase::w_append_fox(int p_fcnt)
{
  ...
  int   recdel;
  char  s_stc1[SMG_SIZE];
  char  s_stc2[SMG_SIZE];
  char  s_stc3[SMG_SIZE];
  char  s_stc4[SMG_SIZE];
  char  s_stc5[SMG_SIZE];
  char  s_stc6[SMG_SIZE];
  long  r,s,t;
  float f1;
  ...
  while (1)
  {
    ...
    for (i=1;i<=get_t4_recnumb(1);i++)
    {
      ...
      get_t4_fldname(i,s_stc6,SMG_SIZE);

      bw_inkey1.lowers(s_stc6);

      l=bw_fldstru1.w_find_fldnum(s_stc6,SMG_SIZE,p_fcnt);
      if (l!=0) continue;

      if (get_t4_fldtype(i)=='n')
      {
		if ((bw_fldstru1.w_find_fldtype(s_stc6,SMG_SIZE,p_fcnt)=='c')
		  ||(bw_fldstru1.w_find_fldtype(s_stc6,SMG_SIZE,p_fcnt)=='l')
		  ||(bw_fldstru1.w_find_fldtype(s_stc6,SMG_SIZE,p_fcnt)=='m')
		  ||(bw_fldstru1.w_find_fldtype(s_stc6,SMG_SIZE,p_fcnt)=='b'))
		{
		  for (k=strlen(s_stc1);k<bw_fldstru1.w_find_fldlen(s_stc6,SMG_SIZE,p_fcnt);k++)
		    s_stc1[k]=' ';

		  k=bw_fldstru1.w_find_fldlen(s_stc6,SMG_SIZE,p_fcnt);
		  s_stc1[k]=0;
		  strcpy(s_stc4,s_stc1);
        }
	
		if (bw_fldstru1.w_find_fldtype(s_stc6,SMG_SIZE,p_fcnt)=='n')
		{
          ...
          {
		    f1=bw_inkey1.str2float(s_stc1,SMG_SIZE);
		    sprintf(s_stc2,"%25.10f",f1);

		    k=bw_fldstru1.w_find_fldlen(s_stc6,SMG_SIZE,p_fcnt);
		    l=bw_fldstru1.w_find_flddec(s_stc6,SMG_SIZE,p_fcnt);
		    ...
		    if (l!=0)
            {
		      ...
		      bw_inkey1.strpcut(s_stc2,14-m,m,SMG_SIZE,s_stc3,SMG_SIZE);
		      bw_inkey1.strpcut(s_stc2,14,n+1,SMG_SIZE,s_stc5,SMG_SIZE);
		    }
		    else
		    {
		      m=k;
      ...
      w_bsav_var(p_fcnt,s_stc6,SMG_SIZE,s_stc4,SMG_SIZE);
    }
...
int bw_xbase::w_bcpy_var(int p_fcnt,char *p_fld,int p_fld_size,char *p_stc4,int p_stc4_size)
{
  int i,j,k;

  i=bw_fldstru1.w_find_fldnum(p_fld,p_fld_size,p_fcnt);
  if (i!=0) return(1);

  j=bw_fldstru1.w_find_fldlen(p_fld,p_fld_size,p_fcnt);
  k=bw_fldstru1.w_find_fldposi(p_fld,p_fld_size,p_fcnt);
  for (i=1;i<=j;i++)
  {
    if (i-1<p_stc4_size)
    {
...
int bw_xbase::w_bsav_var(int p_fcnt,char *p_fld,int p_fld_size,char *p_stc4,int p_stc4_size)
{
  int  i,j,k;
  char c1;
  ...
  j=bw_fldstru1.w_find_fldposi(p_fld,p_fld_size,p_fcnt);
  k=strlen(p_stc4);
  if (k>=p_stc4_size) k=p_stc4_size-1;
  for (i=1;i<=bw_fldstru1.w_find_fldlen(p_fld,p_fld_size,p_fcnt);i++)
  {
    if (i>k) c1=' ';
    else
    {
...
int bw_xbase::get_win_state_bff_in_use(int spp)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  return(win_state_bff_in_use[spp]);
}
/*
...
*/
int bw_xbase::get_win_state_dbf_modi(int spp)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  return(win_state_dbf_modi[spp]);
}
/*
...
*/
int bw_xbase::get_win_state_in_srch(int wpp,int spp)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  return(win_state_in_srch[wpp][spp]);
}
/*
...
*/
int bw_xbase::set_win_state_bff_in_use(int spp,int val)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  win_state_bff_in_use[spp]=val;
  return(0);
}
/*
...
*/
int bw_xbase::set_win_state_dbf_modi(int spp,int val)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  win_state_dbf_modi[spp]=val;
  return(0);
}
/*
...
*/
int bw_xbase::set_win_state_in_srch(int wpp,int spp,int val)
{
  if ((spp<0)||(spp>=STRU_NUM)) return(0);
  if ((wpp<0)||(wpp>=WIN_NUM)) return(0);
  win_state_in_srch[wpp][spp]=val;
  return(0);
}
/*
...
int bw_xbase::get_win_scrnvar(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_scrnvar(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_memno(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_mem_ext(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_mem_fn(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_memno(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_mem_ext(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_mem_fn(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_basename(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_basename(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_basedir(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_basedir(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_ext(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::set_win_ext(int spp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_xbase::get_win_fext(int wpp,char *p_s1,int p_s1_size)
{
  int i,j;
