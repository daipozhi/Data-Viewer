...
int bw_bmp::echo_bmp(HWND hwnd,char *comm,int comm_size,int mcnt,int p_wpp)
{
  FILE *s_fp;
  int  i,j,k,l,m;
  int  exist;
  ...
  if (bw_win1.get_win_ci_mode(p_wpp)==0) k=(int)(k*bw_main1.win_edit_xchar);
  else k=(int)(k*bw_main1.win_xchar);

  if (bw_win1.get_win_ci_mode(p_wpp)==0) l=(int)(l*bw_main1.win_edit_ychar);
...
int bw_bmp::w_bmp_add(int p_fpp,int p_wpp,char *p_fldname,int p_fldname_size)
{
  int  i,j,k;
  char str[SMG_SIZE];
  char sfld[SMG_SIZE];
  ...
  bw_fldstru1.w_sav_tab_var(sfld,SMG_SIZE,p_fpp,str,SMG_SIZE);
