...
int bw_inkey::w_get_dir(char *p_s1,int p_s1_size)
{
  getcwd(p_s1,p_s1_size);
  return(0);
}
int bw_inkey::inkey()
{
...
int bw_inkey::strpcut(char *p_s1,int p1,int p2,int p_s1_size,char *p_s2,int p_s2_size)
{
  int  i,j;
  char c1;
...
int bw_inkey::float2str(float val,char *p_str,int p_str_size)
{
  ...
  strpcut(string,i,j-i+1,SMG_SIZE,str1,SMG_SIZE);
...
int bw_inkey::uppers(char *p_s1)
{
  int i;
  for (i=0;i<(int)strlen(p_s1);i++) p_s1[i]=upper(p_s1[i]);
  return(0);
}
int bw_inkey::lowers(char *p_s1)
{
  int i;
...
int bw_inkey::w_cpy_file(char *fn1,char *fn2)
{
  ...
  fp1=sopen(fn1,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  if (fp1<0) return(1);

  fp2=sopen(fn2,O_RDWR|O_CREAT|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  if (fp2<0)
  {
    close(fp1);
    return(1);
  ...
  while (1)
  {
    rn2=read(fp1,tmp_buff,CPY_BSIZE);
    rn1=rn2;
    write(fp2,tmp_buff,rn1);
    if (eof(fp1)) break;
  }
...
int bw_inkey::w_chs_string_cut(char *p_str,int pn)
{
  int p1,p2;
  if (pn<=0) return(0);
  for (p1=0;p1<pn;p1++) if ((p_str[p1]>=0)&&(p_str[p1]<' ')) p_str[p1]=' ';
  p1=0;
  p2=0;
  while(p2<=pn)
  {
