...
int bw_menuvar::w_read_mdc(char *p_fn1,int p_fn1_size,int p_win)
{
  ...
  while (!feof(fp1))
  {
    ...
    {
      if (tmps2[j]==' ') break;
      setc_t5_m_name(pp1,j-p1+0,tmps2[j]);
    ...
    {
      if (tmps2[j]==' ') break;
      setc_t5_m_button(pp1,j-p3+0,tmps2[j]);
    ...
    {
      if (tmps2[j]==' ') break;
      setc_t5_m_comm(pp1,j-p5+0,tmps2[j]);
    ...
    {
      if (tmps2[j]==' ') break;
      setc_t5_m_focus(pp1,j-p7,tmps2[j]);
...
int bw_choose::w_read_cdc(char *p_fn1,int p_fn1_size,int p_wpp)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
  int  pp1;
  ...
  while (!feof(fp1))
  {
    ...
    bw_inkey1.strpcut(tmps2,p1+1,p2-p1-1+1,SMG_SIZE,str2,SMG_SIZE);
    o=bw_inkey1.str2int(str2,SMG_SIZE);
    k=pp1;
    bw_choose1.set_win_choose(k,0,1);
    bw_choose1.set_win_choose(k,2,o);
    bw_choose1.set_win_choose(k,3,p_wpp);

    for (j=p3;j<=p4;j++)
    {
      if (tmps2[j]==' ') break;
      bw_choose1.setc_win_choose_name(k,j-p3+0,tmps2[j]);
      bw_choose1.setc_win_choose_name(k,j-p3+1,0);
    }

    bw_inkey1.strpcut(tmps2,p5,p6-p5+1,SMG_SIZE,str2,SMG_SIZE);
    l=bw_inkey1.str2int(str2,SMG_SIZE);
    bw_choose1.set_win_choose(k,1,l);

    bw_choose1.get_win_choose_name(k,str3,SMG_SIZE);

    if (bw_dialog1.w_mv_exist(str3,SMG_SIZE,p_wpp)==1)
    {
      bw_dialog1.w_mv_fet_val(str3,SMG_SIZE,0,str1,SMG_SIZE);
      //bw_inkey1.strpcut(str1,0,strlen(str1)-1-0+1,SMG_SIZE,str2,SMG_SIZE);
      bw_choose1.set_win_choose(k,1,bw_inkey1.str2int(str1,SMG_SIZE));
    }
...
int bw_choose::tst_choose()
{
  ...
  for (i=0;i<60;i++)
  {
    ...
    sprintf(str2,"%d,%d,%d,%d,%s,%d,",bw_choose1.get_win_choose(i,0)
...
int bw_choose::w_save_choose(int p_wpp)
{
  int  i,j;
  char str2[SMG_SIZE];
  char str1[SMG_SIZE];

  //for (j=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(p_wpp),0);j<=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(p_wpp),2);j++)
  //{
    for (i=0;i<win_choose_pp1;i++)
    {
      //if (bw_choose1.get_win_choose(i,3)==p_wpp)
      //{
	bw_inkey1.int2str(bw_choose1.get_win_choose(i,1),str2,SMG_SIZE);
        bw_choose1.get_win_choose_name(i,str1,SMG_SIZE);
	bw_dialog1.w_mv_save_mem(str1,SMG_SIZE,p_wpp,str2,strlen(str2));
      //}
    }
  //}
  return(0);
...
int bw_choose::w_echo_chs_change(int link,int p_wpp)
{
  int i;
  int exist;
  ...
  for (i=0;i<MENU_NUM_CHS;i++)
  {
    if ((bw_choose1.get_win_choose(i,2)==link)&&(bw_choose1.get_win_choose(i,3)==p_wpp))
    {
      exist=1;
      break;
    }
...
int bw_choose::w_clr_recvar_chs(int p_wpp)
{
  int i;
...
int bw_choose::w_echo_winrec_chs(int p_wpp)
{
  int i,j,k,l,n,o;
  char str1[SMG_SIZE];
  char str2[SMG_SIZE];
  ...
  while (1)
  {
    ...
    bw_win1.w_next_afield(p_wpp);

    if ((i==bw_win1.pline)&&(j==bw_win1.pfield)) break;

    bw_win1.w_find_winfname(p_wpp,bw_win1.pline,bw_win1.pfield,str1,SMG_SIZE);

    if (str1[0]=='[')
    {
      bw_inkey1.strpcut(str1,1,strlen(str1)-1-1+1,SMG_SIZE,str2,SMG_SIZE);
      n=bw_inkey1.str2int(str2,SMG_SIZE);

      for (k=0;k<MENU_NUM_CHS;k++)
      {
	if ((bw_choose1.get_win_choose(k,2)==n)&&(bw_choose1.get_win_choose(k,3)==p_wpp))
        {
	  l=bw_choose1.get_win_choose(k,1);

	  o=bw_win1.w_find_hnd(p_wpp,bw_win1.pfield,bw_win1.pline);

	  if (l>0)
	  {
	    SendMessage(bw_win1.get_win_hnd(o),BM_SETCHECK,11,(LPARAM)str1);
	  }
	  else
	  {
	    SendMessage(bw_win1.get_win_hnd(o),BM_SETCHECK,0,(LPARAM)str1);
	  }
	  break;
        }
      }
...
int bw_switch::w_read_sdc(char *p_fn1,int p_fn1_size,int p_wpp)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
  int  pp1;
  ...
  while (!feof(fp1))
  {
    ...
    set_win_switch(k,3,p_wpp);

    for (j=p3;j<=p4;j++)
    {
      if (tmps2[j]==' ') break;
      setc_win_switch_name(k,j-p3+0,tmps2[j]);
    ...
    if (bw_dialog1.w_mv_exist(st1,SMG_SIZE,p_wpp)==1)
    {
      bw_dialog1.w_mv_fet_val(st1,SMG_SIZE,0,str1,SMG_SIZE);
      //bw_inkey1.strpcut(str1,0,strlen(str1)-1-0+1,SMG_SIZE,str2,SMG_SIZE);
      set_win_switch(k,1,bw_inkey1.str2int(str1,SMG_SIZE));
    }
...
int bw_switch::w_save_switch(int p_wpp)
{
  int  i,j;
  char str2[SMG_SIZE];
  char str1[SMG_SIZE];
...
int bw_switch::tst_switch()
{
  ...
  for (i=0;i<60;i++)
  {
    ...
    sprintf(str2,"%d,%d,%d,%d,%s,%d,",get_win_switch(i,0)
				    ,get_win_switch(i,1)
				    ,get_win_switch(i,2)
				    ,get_win_switch(i,3)
				    ,str1
...
int bw_switch::w_set_radio_var(int l,int c,int p,int p_wpp)
{
  int  i,j,k,m,o,t1,t2;
  int  t3;
  char str1[SMG_SIZE];
	...
	if ((bw_win1.pline==l)&&(bw_win1.pfield==c))
        {
          ...
          {
	    if ((get_win_switch(m,2)==p)&&(get_win_switch(m,3)==p_wpp))
	    {
...
int bw_switch::w_echo_winrec_radio(int p_wpp)
{
  int  i,j,o,m;
  int  arr[MENU_NUM_RA];
  char str1[SMG_SIZE];
  ...
  while ((i!=bw_win1.pline)||(j!=bw_win1.pfield))
  {
	...
	if ((get_win_switch(m,2)==o)&&(get_win_switch(m,3)==p_wpp))
	{
...
int bw_switch::w_clr_recvar_radio(int p_wpp)
{
  int i;
...
int bw_dialog::w_read_ddc(char *p_fn1,int p_fn1_size,int p_wpp)
{
  int  i,j,k,l;
  ...
  while (!feof(fp1))
  {
	 ...
	 if (k>=0)
	 {
	   ...
	   bw_inkey1.strpcut(s_tmps5,p7,p8-p7+1,SMG_SIZE,str2,SMG_SIZE);
	   l=bw_inkey1.str2int(str2,SMG_SIZE);
	   set_win_mv_type(k,2,l);

	   bw_inkey1.strpcut(s_tmps5,p9,p10-p9+1,SMG_SIZE,str2,SMG_SIZE);
	   l=bw_inkey1.str2int(str2,SMG_SIZE);
	   ...
	   {
             s_tmps4[0]=0;
	     w_mv_add_val(s_tmps1,SMG_SIZE,wpp2,s_tmps4,SMG_SIZE);
	   }
	   else
	   {
		 ...
		 s_tmps2[j-p11+1]=0;
	       ...
	       bw_inkey1.strpcut(s_tmps2,1,strlen(s_tmps2)-2,SMG_SIZE,s_tmps3,SMG_SIZE);
	       w_mv_add_val(s_tmps1,SMG_SIZE,wpp2,s_tmps3,SMG_SIZE);
	     }
	     else
	     {
               s_tmps4[0]=0;
           ...
           if (w_mv_exist(s_tmps1,SMG_SIZE,wpp2)==1)
           {
             w_mv_fet_val(s_tmps1,SMG_SIZE,   0,str1,SMG_SIZE);
             w_mv_add_val(s_tmps1,SMG_SIZE,wpp2,str1,SMG_SIZE);
	   }
...
int bw_dialog::w_save_dialog(int p_wpp)
{
  int  i,j,k,l;
  char str[SMG_SIZE];
  char sv1[SMG_SIZE];
/*
  for (j=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(p_wpp),0);j<=bw_win1.get_win_pp_grp_pp(bw_win1.get_win_pp_grp_id(p_wpp),2);j++)
  {
	...
	if (k==0) w_mv_save_mem(sv1,SMG_SIZE,p_wpp,str,l);
      }
    }
  }
*/
...
int bw_dialog::w_mv_save_mem(char *p_str,int p_str_size,int p_wpp,char *val,int val_size)
{
  int  i,k;
  char sd1[1000];
  char sv1[SMG_SIZE];
  ...
  if (DEBUG_MV)
  {
    sprintf(sd1,"(((into w_mv_save_mem(),p_str=%s,p_str_size=%d,p_wpp=%d,val=%s,val_size=%d,",p_str,p_str_size,p_wpp,val,val_size);
  ...
  if (w_mv_exist(p_str,p_str_size,p_wpp)==0)
  {
    ...
    if (k>=0)
    {
      ...
      set_win_mv_type(k,4,p_wpp);          /* free window var */
    }
  }
  else
  {
    k=w_mv_seri(p_str,p_str_size,p_wpp);
    if (k>=0)
    {
      set_win_mv_type(k,2,val_size);         /* var len */      //remember
    }
  ...
  w_mv_add_val(p_str,p_str_size,p_wpp,sv1,val_size);
...
int bw_dialog::w_clr_recvar_dialog(int p_wpp)
{
  int i;
...
int bw_dialog::w_mv_add_val(char *p_s1,int p_s1_size,int p_wpp,char *p_s2,int p_s2_size)
{
  int  i,j,k,l;
  int  exist;
  char str3[SMG_SIZE];
  ...
  if (DEBUG_MV)
  {
    sprintf(sd1,"(((into w_mv_add_val(),p_s1=%s,p_s1_size=%d,p_wpp=%d,p_s2=%s,p_s2_size=%d,",p_s1,p_s1_size,p_wpp,p_s2,p_s2_size);
    bw_getread1.deb_record(sd1);
  }
  ...
  {
    get_win_mv_name(i,sv1,SMG_SIZE);
    if ((strcmp(p_s1,sv1)==0)&&(get_win_mv_type(i,4)==p_wpp))
    {
      exist=1;
      break;
    }
  ...
  if (l!=0) 
  {
    if (DEBUG_MV)
    {
      sprintf(sd1,")))return from w_mv_add_val(),move bef or aft error,exist=%d,,win_mv_pp1=%d,win_mv_pp2=%d,",exist,win_mv_pp1,win_mv_pp2);
      bw_getread1.deb_record(sd1);
    }
...
int bw_dialog::w_mv_fet_val(char *p_s1,int p_s1_size,int p_wpp,char *p_s2,int p_s2_size)
{
  int  i,j;
  int  exist;
  char sd1[1000];
  ...
  if (DEBUG_MV)
  {
    sprintf(sd1,"(((into w_mv_fet_val(),p_s1=%s,p_s1_size=%d,p_wpp=%d,",p_s1,p_s1_size,p_wpp);
    bw_getread1.deb_record(sd1);
  }
...
int bw_dialog::w_mv_exist(char *p_s1,int p_s1_size,int p_wpp)
{
  int  i;
  int  exist;
  char sd1[1000];
  ...
  if (DEBUG_MV)
  {
    sprintf(sd1,"(((into w_mv_exist(),p_s1=%s,p_s1_size=%d,p_wpp=%d,",p_s1,p_s1_size,p_wpp);
    bw_getread1.deb_record(sd1);
  }
...
int bw_dialog::w_mv_seri(char *p_s1,int p_s1_size,int p_wpp)
{
  int  i,j;
  int  exist;
  char sd1[1000];
  ...
  if (DEBUG_MV)
  {
    sprintf(sd1,"(((into w_mv_seri(),p_s1=%s,p_s1_size=%d,p_wpp=%d,",p_s1,p_s1_size,p_wpp);
    bw_getread1.deb_record(sd1);
  }
...
int bw_dialog::w_mv_sav_var(char *p_s1,int p_s1_size,int p_wpp)
{
  char   sm1[SMG_SIZE];
  int    i,j;

  if (w_mv_exist(p_s1,p_s1_size,p_wpp)!=1) return(1);

  i=w_mv_seri(p_s1,p_s1_size,p_wpp);

  j=get_win_mv_type(i,2);

  setc_win_mv_getv(p_wpp,j,0);

  get_win_mv_getv(p_wpp,sm1,SMG_SIZE);

  w_mv_add_val(p_s1,p_s1_size,p_wpp,sm1,SMG_SIZE);

  return(0);
}

int bw_dialog::w_mv_get_var(char *p_s1,int p_s1_size,int p_wpp)
{
  char   sm1[SMG_SIZE];

  w_mv_fet_val(p_s1,p_s1_size,p_wpp,sm1,SMG_SIZE);

  set_win_mv_getv(p_wpp,sm1,SMG_SIZE);

  return(0);
}

int bw_dialog::w_echo_winrec_var(int p_wpp)
{
  HDC   hdc;
  ...
  if (bw_win1.get_win_ci_mode(p_wpp)==0)
  {
  ...
  while (1)
  {
    ...
    if ((str1[0]=='?')||(str1[0]=='-'))
    {
      if (w_mv_exist(str1,SMG_SIZE,p_wpp)!=1) continue;

      k=w_mv_seri(str1,SMG_SIZE,p_wpp);
      if (k>=0)
      {
	if (get_win_mv_type(k,0)!=2)
	  bw_win1.w_set_winfedit(p_wpp,bw_win1.pline,bw_win1.pfield,1);

        w_mv_fet_val(str1,SMG_SIZE,p_wpp,str2,SMG_SIZE);

	len =get_win_mv_type(k,2);

	len2=bw_win1.w_find_winflen(p_wpp,bw_win1.pline,bw_win1.pfield);
	...
	if (str1[0]=='?')
	  ...
	  if (bw_win1.get_win_ci_mode(p_wpp)==0)
          {
	    TextOut(hdc
		   ,(int)((bw_win1.pfield-bw_win1.get_win_con(p_wpp,0))*bw_main1.win_edit_xchar)
		   ,(int)((bw_win1.pline-bw_win1.get_win_con(p_wpp,1))*bw_main1.win_edit_ychar)
		   ,str2,l);
	  }
	  else
	  {
	    TextOut(hdc
		   ,(int)((bw_win1.pfield-bw_win1.get_win_con(p_wpp,0))*bw_main1.win_xchar)
		   ,(int)((bw_win1.pline-bw_win1.get_win_con(p_wpp,1))*bw_main1.win_ychar)
		   ,str2,l);
	  }
	}
	else
...
int bw_dialog::w_mv_get_read_paint(char *p_str,int p_str_size,int p_wpp,char comm)
{
  HWND hnd;
  int  i,j,q;
  char sm1[SMG_SIZE];
  ...
  if (w_mv_exist(p_str,p_str_size,p_wpp)!=1) return(1);

  w_mv_get_var(p_str,p_str_size,p_wpp);

  i=w_mv_seri(p_str,p_str_size,p_wpp);

  get_win_mv_getv(p_wpp,sm1,SMG_SIZE);

  if (p_str[0]=='?')
  {
    j=bw_getread1.get_read(bw_win1.pline-bw_win1.get_win_con(p_wpp,1)+1,
		       bw_win1.pfield-bw_win1.get_win_con(p_wpp,0)+1,
		       "",
		       sm1,
		       get_win_mv_type(i,5),
		       get_win_mv_type(i,1),
...
int bw_dialog::w_mv_get_read_g(char *p_str,int p_str_size,int p_wpp,char comm)
{
  HWND hnd;
  int  i,j,q;
  char sm1[SMG_SIZE];
...
int bw_dialog::w_mv_get_read_r(char *p_str,int p_str_size,int p_wpp,char comm)
{
  int  i,j;
  char sm1[SMG_SIZE];
...
int bw_dialog::w_echo_winrec_read(int p_fpp,int p_wpp,int mark)
{
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	if (str[0]=='*')
	{
	  ...
	  if (m==8)  bw_inkey1.long2str(bw_xbase1.get_win_recno(p_fpp,p_wpp,0),s_tmps2,SMG_SIZE);
	  if (m==9)  bw_inkey1.long2str(bw_xbase1.get_win_rcnt(p_fpp),s_tmps2,SMG_SIZE);
	  ...
	  if (bw_win1.get_win_ci_mode(p_wpp)==0)
	      TextOut(hdc
		     ,(int)((j-bw_win1.get_win_con(p_wpp,0))*bw_main1.win_edit_xchar)
			 ,(int)((i-bw_win1.get_win_con(p_wpp,1))*bw_main1.win_edit_ychar)
			 ,s_tmps2,n);
	  else
	      TextOut(hdc
		     ,(int)((j-bw_win1.get_win_con(p_wpp,0))*bw_main1.win_xchar)
			 ,(int)((i-bw_win1.get_win_con(p_wpp,1))*bw_main1.win_ychar)
			 ,s_tmps2,n);
...
int * bw_dialog::get_win_mv_getp(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(&win_mv_getp[p_wpp]);
}
int bw_dialog::getv_win_mv_getp(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_mv_getp[p_wpp]);
}
...
int bw_dialog::set_win_mv_getp(int p_wpp ,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_mv_getp[p_wpp]=val;
  return(0);
}
...
int bw_dialog::get_win_mv_getv(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s1[j+0]=win_mv_getv[p_wpp][j];
    p_s1[j+1]=0;
  }

  return(0);
}
int bw_dialog::set_win_mv_getv(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  win_mv_getv[p_wpp][0]=0;

  for (j=0;j<i-1;j++)
  {
    win_mv_getv[p_wpp][j+0]=p_s1[j];
    win_mv_getv[p_wpp][j+1]=0;
  }

  return(0);
}
int bw_dialog::setc_win_mv_getv(int p_wpp,int pp1,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp1<0)||(pp1>=SMG_SIZE)) return(0);
  win_mv_getv[p_wpp][pp1]=val;
  return(0);
}
