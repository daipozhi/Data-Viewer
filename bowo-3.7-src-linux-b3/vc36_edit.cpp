...
int bw_edit::get_edit(char *p_s1,int p_s1_size,int p_wpp,HWND hwnd)
{
  HWND    hnd;
  int     s_hp1;
  int     i,j,k,l;
  ...
  set_win_edit_dirn(win_edit_pp1,str1,FN_SIZE);

  set_win_edit_file(win_edit_pp1,p_s1,p_s1_size);

  set_win_edit_inuse(win_edit_pp1,1);

  s_hp1=/*s*/open(p_s1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
  if (s_hp1<0)
  {
  ...
  SetWindowText(get_win_edit_hnd2(win_edit_pp1),bw_inkey1.tmp_buff);

  set_win_edit_modi(win_edit_pp1,0);

  if (p_wpp>0) set_win_edit_wpp(win_edit_pp1,p_wpp);
...
int bw_edit::get_win_edit_dirn(int epp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_edit::set_win_edit_dirn(int epp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_edit::get_win_edit_file(int epp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_edit::set_win_edit_file(int epp,char *p_s1,int p_s1_size)
{
  int i,j;
