...
int bw_link::tst_link1()
{
  ...
  for (i=0;i<60;i++)
  {
    sprintf(str1,"lkpoin1,2=%d,%d,modi=%d,seri=%d,base=%d,recn=%d,wind=%d,"
    ,get_win_lkpoin(i,0)
    ,get_win_lkpoin(i,1)
    ,get_win_lkmodi(i)
    ,get_win_lkseri(i)
...
int bw_link::tst_link2()
{
  ...
  for (i=0;i<60;i++)
  {
    ...
    sprintf(str3,"bslink0,1,2=%d,%d,%d,bspoin0,1=%d,%d,%s,%s,"
    ,get_win_bslink(i,0)
    ,get_win_bslink(i,1)
    ,get_win_bslink(i,2)
    ,get_win_bspoin(i,0)
    ...
    TextOut(hdc,1,i*20,str3,strlen(str3));
  }
...
int bw_link::tst_link3()
{
  ...
  for (i=0;i<60;i++)
  {
    ...
    sprintf(str3,"svpoin1,2=%d,%d,used=%d,seri=%d,base=%d,recn=%d,wind=%d,%s,"
...
int bw_link::tst_link4()
{
  ...
  for (i=0;i<60;i++)
  {
    ...
    sprintf(str1,"win_bb231=%d,%d,%s,",get_win_bb2(i),get_win_bb3(i),str2);
...
/*
  sprintf(str,"p_wpp=%d,%d,lvpp=%d,%d,%d,svpp=%d,%d,%d,"
	     ,wp1,wp2
	     ,i,j,n
	     ,l,m,p);
  MessageBox(bw_main1.win_hwnd1,str,"aa",MB_OK);
...
int bw_link::w_read_link(char *p_fn,int p_fn_size,int p_wpp,int num)
{
  FILE *fp1;
  int   i,j,k,l;
  ...
  while (1)
  {
    ...
    if (s_tmps1[0]=='&') break;
    k=w_find_link_tn(s_tmps1,SMG_SIZE,p_wpp,i);
    if (k==1)
    {
      fclose(fp1);
      return(1);
...
int bw_link::w_find_link_tn(char *p_s1,int p_s1_size,int p_wpp,int i)
{
  int  j,l,m,n;
  ...
  if (j>=p_s1_size) return(1);
  p6=j-1;
  bw_inkey1.strpcut(p_s1,p1,p2-p1+1,p_s1_size,tn1,FN_SIZE);
  bw_inkey1.strpcut(p_s1,p3,p4-p3+1,p_s1_size,tn2,FN_SIZE);
  bw_inkey1.strpcut(p_s1,p5,p6-p5+1,p_s1_size,tn3,FN_SIZE);

  for (m=0;m<STRU_NUM;m++)
  {
    if (bw_win1.get_win_pp_wins_dbf(m,p_wpp)==1)
    {
  ...
  {
    if (bw_win1.get_win_pp_wins_dbf(n,p_wpp)==1)
    {
  ...
  set_win_bslink(i,2,p_wpp);
  bw_xbase1.set_win_lnktab(n,l);

  return(0);
}

int bw_link::w_find_link_cd(char *p_s1,int p_s1_size,int i,int k)
{
  int  l,m;
  int  p1,p2;
  char s_tmps1[SMG_SIZE];
  ...
  if (l>=p_s1_size) return(-1);
  p2=l-1;
  bw_inkey1.strpcut(p_s1,p1,p2-p1+1,p_s1_size,s_tmps1,SMG_SIZE);
  l=k;
...
int bw_link::w_link_modi(int p_wpp)  // fetch link var in cond // and link var if modi // prepare for link
{
  char str1[SMG_SIZE];
  char tn[SMG_SIZE];
  char fn[SMG_SIZE];
  ...
  for (i=0;i<COND_NUM;i++)
  {
    if (get_win_bslink(i,2)!=p_wpp) continue;
	...
	if (bw_win1.get_win_pp_wins_dbf(j,p_wpp)==1)
      ...
      w_link_modi_set(tn,SMG_SIZE,fn,SMG_SIZE,p1,p_wpp);
...
int bw_link::w_link_modi_set(char *p_tn,int p_tn_size,char *p_fn,int p_fn_size,int p1,int p_wpp)  // prepare for link
{
  int  i,j;
  char stn[FN_SIZE];
  char sfn1[20];
  ...
  set_win_lkseri(p1,j);
  set_win_lkbase(p1,i);
  set_win_lkwind(p1,p_wpp);
  set_win_lkrecn(p1,0);

  return(0);
}

int bw_link::w_link_modi_mark(int p_wpp,int mark,int seri,int recn)   // if changed ,mark it /* echo win rec ,link rec should change */
{
  int i,j,k;
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    {
      if ((get_win_bslink(j,0)==i)&&(get_win_bslink(j,2)==p_wpp))
      {
...
int bw_link::w_link_modi_unmark(int p_wpp,int mark,int seri,int recn)   /* echo win rec ,link rec should change */
{
  int i,j,k;
...
int bw_link::w_echo_winrec_lnk(int p_wpp) /* echo win rec where modified */
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
	...
	{
	  if (get_win_continue(i,p_wpp)==1)
	  {
	    s_recno[i]=bw_xbase1.get_win_recno(i,p_wpp,1)-1;
	    set_win_continue(i,p_wpp,0);
	  }
	  else
	  {
	    s_recno[i]=0;
      ...
      //}
      //else s_recno[i]=bw_xbase1.get_win_recno(i,p_wpp,1)-1;
      s_seri[i]=0;
      bw_xbase1.set_win_enseri(i,p_wpp,0);
      bw_xbase1.set_win_cuseri(i,p_wpp,0);
      exist=1;
    }
  }
  ...
  {
    if (get_win_bslink(r,2)!=p_wpp) continue;
    ...
    if (w_link_modified(p_wpp,s)!=1) continue;

    w_link_modi_unmark(p_wpp,s,-1,-1);

    w_link_modi_mark(p_wpp,k,-1,-1);
...
//	      win_tst1=0;
		...
		{
		  u=bw_buff1.get_linecol(p_wpp,bw_win1.pline,bw_win1.pfield);
		  if(u>=0) u=bw_win1.get_1fldseri(u);
		  else u=1;
		}
		else u=bw_win1.get_1fldseri(m);

		if (w_link_cond_ok(r,p_wpp,k,u,1)==1) break;
		...
		{
		  bw_xbase1.set_win_recno(k,p_wpp,0,s_recno[k]);
		}
                exist=1;
	      }
	    }
	    ...
	    str1[bw_win1.get_1fldlen(m)]=0;
            ...
            if (bw_win1.get_1fldlen(m)<q) x=bw_win1.get_1fldlen(m);
            else x=q;
...
int bw_link::w_link_modified(int p_wpp,int basep)
{
  int i,j;

  for (i=0;i<COND_NUM;i++)
  {
    if ((get_win_bslink(i,0)==basep)&&(get_win_bslink(i,2)==p_wpp))
    {
...
int bw_link::w_link_cond_ok(int condnum,int p_wpp,int basep,int recn,int page)
{
  int  i,j,k;
  int  exist;
  int  p2;
  ...
  if ((get_win_bslink(i,1)==basep)&&(get_win_bslink(i,2)==p_wpp))
  {
    exist=1;
  }
  ...
  while (1)
  {
    ...
    else
    {
      ...
      {
	if ((get_win_svwind(j)==p_wpp)&&(get_win_svbase(j)==pp1)&&(get_win_svseri(j)==pp2))
        {
...
int bw_link::w_scan_sv(int p_wpp)
{
  int  i,j,k,n;
//  int  s_tmpn1,s_tmpn2;
  ...
  {
    if (get_win_svwind(i)==p_wpp) break;
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	{
	  bw_win1.get_1fldname(n,str2,FN_SIZE);
...
int bw_link::w_save_sv(int p_wpp,int base,int seri,int recn,int posi,int len)  /*save scrn var to arr*/
{
  int i,j;
  int exist;
  ...
  for (i=0;i<WIN_VAR_NUM;i++)
  {
    if ((get_win_svwind(i)==p_wpp)&&(get_win_svbase(i)==base)&&(get_win_svseri(i)==seri)&&(get_win_svrecn(i)==recn))
    {
      exist=1;
      break;
    }
...
int bw_link::w_get_sv(int p_wpp,int base,int seri,int recn,char *p_str,int p_str_size)  /*save scrn var to arr*/
{
  int i,j;
  int exist;
...
int bw_link::w_link_type(int p_wpp,int basep)
{
  int i,j,k;
  ...
  for (i=0;i<COND_NUM;i++)
  {
    if ((get_win_bslink(i,2)==p_wpp)&&(get_win_bslink(i,0)==basep))
    {
...
int bw_link::w_link_number(int p_wpp,int basep)
{
  int i,j;
  ...
  for (i=0;i<COND_NUM;i++)
  {
    if ((get_win_bslink(i,2)==p_wpp)&&(get_win_bslink(i,1)==basep))
    {
      j=i;
    }
  }
...
int bw_link::w_link_base_num(int p_wpp,int basep)
{
  int i,j;
...
int bw_link::w_link_set_continue(int p_wpp,int p_fpp)
{
  if ((p_wpp<1)||(p_wpp>=WIN_NUM)) return(0);
  if ((p_fpp<0)||(p_fpp>=STRU_NUM)) return(0);
  set_win_continue(p_fpp,p_wpp,1);
  return(0);
}

int bw_link::w_keep_linkfld(int p_wpp,int p_fpp,int dir)
{
  int  i,j,k,l,m,n;
  int  exist;
  char str4[SMG_SIZE];
  ...
  for (i=0;i<COND_NUM;i++)   
  {
    if ((get_win_bslink(i,1)==p_fpp)&&(get_win_bslink(i,2)==p_wpp))
    {
      j=get_win_bslink(i,0);
      exist=1;
      break;
  ...
  l=get_win_lkseri(k);
  w_get_sv(p_wpp,j,l,1,str4,SMG_SIZE);
  ...
  bw_inkey1.strpcut(str1,n+1,strlen(str1)-n-1,SMG_SIZE,str2,SMG_SIZE);
  n=bw_inkey1.strlocc(str2,SMG_SIZE,'.');
  bw_inkey1.strpcut(str2,n+1,strlen(str2)-n-1,SMG_SIZE,str3,SMG_SIZE);

  bw_xbase1.w_clr_recvar(p_fpp);
  bw_fldstru1.w_sav_tab_var(str3,SMG_SIZE,p_fpp,str4,SMG_SIZE);
  bw_mbase1.w_save_rec(p_fpp,p_wpp,bw_xbase1.get_win_recno(p_fpp,p_wpp,0)+dir);
...
int bw_link::get_win_svar(int vpp)
{
  if ((vpp<0)||(vpp>=VAR_IN_SCRN)) return(0);
  return(win_svar[vpp]);
}
int bw_link::set_win_svar(int vpp,int val)
{
  if ((vpp<0)||(vpp>=VAR_IN_SCRN)) return(0);
  win_svar[vpp]=val;
  return(0);
...
int bw_link::get_win_continue(int p_fpp,int p_wpp)
{
  if ((p_fpp<0)||(p_fpp>=STRU_NUM)) return(0);
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_continue[p_fpp][p_wpp]);
}
int bw_link::set_win_continue(int p_fpp,int p_wpp,int val)
{
  if ((p_fpp<0)||(p_fpp>=STRU_NUM)) return(0);
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_continue[p_fpp][p_wpp]=val;
  return(0);
}
...
int bw_link::get_win_bb1(int vpp,char *p_s1,int p_s1_size)
{
  int i,j;
...
int bw_link::set_win_bb1(int vpp,char *p_s1,int p_s1_size)
{
  int i,j;
