...
*/
	...
	bw_choose1.w_echo_winrec_chs(bw_main1.win_hnd_id);   // paint window ...
...
//	hdc=GetDC(win_hwnd);
//	sprintf(str2,"%d,%d,%s,",bw_win1.pline,bw_win1.pfield,str1);
//	TextOut(hdc,1,1,str2,strlen(str2));
//	ReleaseDC(win_hwnd,hdc);

	if (str1[0]=='?') bw_dialog1.w_mv_get_read_paint(str1,SMG_SIZE,bw_main1.win_hnd_id,'g');  // init get_read
	else
        {
	  if (str1[0]==':')
	  {
	      ...
	      bw_fldstru1.w_tv_get_read_paint(str1,SMG_SIZE,bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield,'g');
            }
	  }
	  else
	  {
...
/*	  return(0);*/
	...
	if (str1[0]=='[') // choose ci
	{
	  ...
	  SendMessage(bw_win1.get_win_hnd(j),BM_SETSTATE,0,(LPARAM)str1);
...
/*	  return(0);*/
	...
	if ((str1[0]=='?')||(str1[0]=='-')) // memory var field
	{
	  ...
	  if (k==1)
	  {
	    if (str1[0]=='?')
            {
	      bw_getread1.get_smg_string(bw_win1.get_win_pp_get_rd(bw_main1.win_hnd_id),str4,SMG_SIZE);
	    }
	    else
	    {
...
//	    MessageBox(hwnd,str2,"aa",MB_OK);
	    ...
	    if (bw_main1.win_key==CTRL_E)
	    {
	      if (str1[0]==':') bw_xbase1.setc_win_scrnvar(bw_main1.win_hnd_id,0,0);
	      else
	      {
		q=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
	        if (q!=0)
...
/*	    return(0);*/
	...
	if (bw_main1.win_key>=5000)
	{
	  SendMessage(bw_main1.win_hwnd,WM_CHAR,1002,(LPARAM)str1);
	} 
      }
...
long  /*PASCAL*/ w_menu_ci_wp(HWND hwnd,WORD msg,WORD wp,LONG lp)
{
  ...
  switch(msg)
  {
	    ...
	    (bw_main1.win_key<=32))
	{
	  ...
	  SendMessage(bw_win1.get_win_hnd(k),WM_CHAR,1002,(LPARAM)str1);
	}
      }
...
*/
  ...
  {
    j=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
    SendMessage(bw_win1.get_win_hnd(j),BM_SETSTATE,11,(LPARAM)str1);
  }
  if (str1[0]=='(')
  {
    j=bw_win1.w_find_hnd(bw_main1.win_hnd_id,bw_win1.pfield,bw_win1.pline);
...
int bw_main::w_set_lbutton(HWND hwnd,int hnd_id)
{
  ...
  if (str1[0]=='[')
  {
    ...
    i=SendMessage(bw_win1.get_win_hnd(j),BM_GETCHECK,11,(LPARAM)str1);
    if (i==0) SendMessage(bw_win1.get_win_hnd(j),BM_SETCHECK,11,(LPARAM)str1);
    else  SendMessage(bw_win1.get_win_hnd(j),BM_SETCHECK,0,(LPARAM)str1);
...
int bw_main::get_ci_save(int hnd_id)
{
  ...
  if ((str1[0]=='?')||(str1[0]=='-'))
  {
    if (str1[0]=='?')
    {
      bw_getread1.get_smg_string(bw_win1.get_win_pp_get_rd(hnd_id),str2,SMG_SIZE);
