...
int bw_mbase::w_ins_rec(int p_fpp,int p_wpp,long recno)
{
  ...
  if (bw_xbase1.get_win_base_type(p_fpp)==0)
  {
    ...
    else
    {
      ...
      j=read(bw_xbase1.win_fp2[p_wpp],str1,5);
      ...
      if (/*eof(bw_xbase1.win_fp2[p_wpp])*/j<5)
      {
...
/*    w_find_recno_ins(recno);*/
	...
	j=read(bw_xbase1.win_fp2[p_wpp],str2,5);

	if (/*eof(bw_xbase1.win_fp2[p_wpp])*/j<5) i=1;
  ...
  if (bw_xbase1.get_win_base_type(p_fpp)==0)
  {
    ...
    else
    {
      ...
      while(1)
      {
        ...
        if (/*eof(bw_xbase1.win_fp2[p_wpp])*/j<5) break;
        if (str1[4]=='*') continue;
        break;
      }
