...
int bw_win::w_ini_win(void)
{
  ...
  win_pp_new_win=1; /* can not be zero (0) */

  for (i=0;i<WIN_NUM;i++)
  {
    set_win_pp_fld_in_win(i,0,0);
    set_win_pp_fld_in_win(i,1,0);
    set_win_pp_fld_in_win(i,2,0);
    set_win_pp_fld_in_win(i,3,0);
    set_win_pp_win_mark(i,0);
    set_win_pp_grp_id(i,0);
    set_win_pp_grp_pp(i,0,0);
    set_win_pp_grp_pp(i,1,0);
    set_win_pp_grp_pp(i,2,0);
...
int bw_win::w_login_win(int p1,int p2,char *p_win_name,int p_win_name_size,char *p_find_name,int p_find_name_size)
{
  ...
  set_win_pp_grp_pp(i,0,p1);
  set_win_pp_grp_pp(i,1,p1);
  set_win_pp_grp_pp(i,2,p2);

  for (j=p1;j<=p2;j++)
  {
    set_win_pp_grp_id(j,i);
    set_win_pp_win_mark(j,j);
...
int bw_win::w_dele_win_pp(int p_wpp)
{
  int i,j,k;

  i=get_win_pp_grp_id(p_wpp);

  for (j=get_win_pp_grp_pp(i,0);j<=get_win_pp_grp_pp(i,2);j++)
  {
    set_win_pp_grp_id(j,0);
    set_win_pp_win_mark(j,0);
    for (k=0;k<STRU_NUM;k++)
    {
      set_win_pp_wins_dbf(k,j,0);
    }
  }
...
int bw_win::w_dele_win_pp_wins_dbf(int p_wpp)
{
  int i,j;

  for (j=get_win_pp_grp_pp(get_win_pp_grp_id(p_wpp),0)
      ;j<=get_win_pp_grp_pp(get_win_pp_grp_id(p_wpp),2)
      ;j++)
  {
    for (i=0;i<STRU_NUM;i++)
    {
      if (get_win_pp_wins_dbf(i,j)==1)
      {
	set_win_tabname(0,i,"",0);
	set_win_tabname(1,i,"",0);
      }
      set_win_pp_wins_dbf(i,j,0);
    }
  }
...
{
  win_h_pp=get_win_pp_grp_pp(get_win_pp_grp_id(p_pagecnt),2)-get_win_pp_grp_pp(get_win_pp_grp_id(p_pagecnt),0);

  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),0,win_h_w_total_pp);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),1,win_h_w_total_pp+win_h_pp);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),2,win_h_w_total_pp);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),3,get_win_pp_grp_id(p_pagecnt)-1);

  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),4,win_h_w_total_pp+win_h_pp+1);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5,0);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),6,win_h_w_total_pp+win_h_pp+1);
  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),7,get_win_pp_grp_id(p_pagecnt));

  win_h_ci_total_pp=win_h_w_total_pp+win_h_pp+1;
  win_h_w_total_pp=win_h_ci_total_pp;
...
int bw_win::w_mouse_infield(int p_wpp,int y,int x)
{
  int  exist;
  int  m,n;
  char str[SMG_SIZE];
  ...
  for (m=1;m<COL_NUM;m++)
  {
    n=bw_buff1.get_linecol(p_wpp,y,m);
    if ((n>0)&&(get_1fldedt(n)==0))
    {
      if ((m<=x)&&(x<=m+get_1fldlen(n)-1))
      {
	get_1fldname(n,str,SMG_SIZE);
...
int bw_win::tst_win1()
{
  ...
  for (i=1;i<60;i++)
  {
    ...
    sprintf(str1,"%s,t=%d,b=%d,s=%d,l=%d,d=%d,e=%d,"
                                       ,str2
				       ,get_1fldtype(i)
				       ,get_1fldbase(i)
				       ,get_1fldseri(i)
				       ...
				       ,get_1flddec(i)
				       ,get_1fldedt(i));
    TextOut(hdc,1,(i-1)*20,str1,strlen(str1));
  }
  ...
  for (l=1;l<WIN_NUM;l++)
  {
	...
	if (bw_buff1.get_linecol(l,i,j)!=0)
	{
	  ...
	  sprintf(str1,"w=%d,l=%d,c=%d,n=%d,seri=%d,base=%d,%s,",l,i,j,m,get_1fldseri(m),get_1fldbase(m),str2);
          TextOut(hdc,300,k*20,str1,strlen(str1));
	  k++;
	  if (k>=60) break;
        }
      }
...
int bw_win::tst_win2()
{
  ...
  for (i=0;i<WIN_NUM;i++)
  {
    sprintf(str1,"20=%d,21=%d,4=%d,5=%d,60=%d,61=%d,62=%d,90=%d,91=%d,92=%d,93=%d,pp1=%d,subwub=%d,"
                                       ,get_win_pp_fld_in_win(i,0)
				       ,get_win_pp_fld_in_win(i,1)
				       ,get_win_pp_win_mark(i)   
				       ,get_win_pp_grp_id(i)
				       ,get_win_pp_grp_pp(i,0)
				       ,get_win_pp_grp_pp(i,1)
				       ,get_win_pp_grp_pp(i,2)
				       ,get_win_pp_wins_dbf(0,i)
				       ,get_win_pp_wins_dbf(1,i)
				       ,get_win_pp_wins_dbf(2,i)
				       ,get_win_pp_wins_dbf(3,i)
				       ,win_pp_new_win
				       ,0/*get_win_subhndpp(i)*/);
    TextOut(hdc,1,i*20,str1,strlen(str1));
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    sprintf(str1,"tab=%s,stru=%s,",stn1,stn2);
    TextOut(hdc,600,i*20,str1,strlen(str1));
  }
...
int bw_win::tst_win3()
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    sprintf(str1,"wn=%s,fn=%s,stru=%s,base=%s,"
                                       ,stn3
				       ,stn4
				       ,stn1
				       ,stn2);
...
int bw_win::tst_win4()
{
  ...
  for (i=1;i<24;i++)
  {
    for (j=1;j<=80;j++)
    {
      str[j-1]=bw_buff1.get_t_winfile(1,i,j);
    }
    str[80]=0;
...
int bw_win::tst_win5()
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    sprintf(str1,"base=%s,stru=%s,",stn1,stn2);
...
int bw_win::w_close_win(int p1)
{
  ...
  for (i=WIN_NUM-1;i>=1;i--)
  {
    if (id[i]==1)
    {
      ...
      {
	if ((get_win_pp_wins_dbf(m,i)==1)&&(bw_xbase1.get_win_mem_inuse(m)==1))
	{
...
//      sprintf(str,"%d,%d,",i,win_pp_grp_pp[i][0]);
//      MessageBox(bw_main1.win_hwnd1,str,"aa",MB_OK);

      w_close_subwin(get_win_pp_grp_pp(i,0));
...
int bw_win::w_close_subwin(int p1)
{
  ...
  i=get_win_pp_grp_id(p1);



  /* close database */  
  // lock msg
  for (k=get_win_pp_grp_pp(i,0);k<=get_win_pp_grp_pp(i,2);k++)
  {
  ...
  bw_menuvar1.w_dele_menu(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));

  //bw_choose1.w_dele_choose(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));
  //bw_switch1.w_dele_switch(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));
  //bw_dialog1.w_dele_dialog(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));

  bw_choose1.w_clr_recvar_chs(get_win_pp_grp_pp(i,0));
  bw_switch1.w_clr_recvar_radio(get_win_pp_grp_pp(i,0));
  //bw_dialog1.w_clr_recvar_dialog(get_win_pp_grp_pp(i,0));

  bw_link1.w_dele_svar(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));

  bw_link1.w_dele_link(get_win_pp_grp_pp(i,0),get_win_pp_grp_pp(i,2));

  for (k=get_win_pp_grp_pp(i,0);k<=get_win_pp_grp_pp(i,2);k++)
  {
    j=get_win_pp_get_rd(k);
    bw_getread1.get_read_logout(j);
  }

  w_dele_win_pp_wins_dbf(p1);

  for (j=get_win_pp_grp_pp(i,0);j<=get_win_pp_grp_pp(i,2);j++)
  {
    SetFocus(bw_main1.win_main_hnd);

    k=get_win_h_w_pp(i,0)+j-get_win_pp_grp_pp(i,0);
  ...
  l=get_win_pp_grp_id(j);

  n=get_win_pp_grp_pp(i,2)-get_win_pp_grp_pp(i,0)+1;

  q=0;

  for (k=get_win_pp_grp_pp(i,0);k<=get_win_pp_grp_pp(i,2);k++)
  {
    q=q+get_win_pp_fld_in_win(k,1)-get_win_pp_fld_in_win(k,0)+1;
  }

  r=get_win_pp_fld_in_win(get_win_pp_grp_pp(l,2),1);

  s=get_win_pp_fld_in_win(get_win_pp_grp_pp(i,2),1)+1;

  for (k=s;k<=r;k++)
  {
    get_1fldname(k,sfn1,FLD_NAME_LEN);
    set_1fldname(k-q,sfn1,FLD_NAME_LEN);
  ...
  t=get_win_h_w_pp(i,1)-get_win_h_w_pp(i,0)+1;
  if (get_win_h_w_pp(i,5)==0)
     t=t;
  else
     t=t+get_win_h_w_pp(i,5)-get_win_h_w_pp(i,4)+1;

  p10=get_win_pp_grp_pp(l,2);

  for (k=i+1;k<=l;k++)
  {
    for (m=get_win_pp_grp_pp(k,0);m<=get_win_pp_grp_pp(k,2);m++)
    {
      set_win_pp_fld_in_win(m-n,0,get_win_pp_fld_in_win(m,0)-q);
      set_win_pp_fld_in_win(m-n,1,get_win_pp_fld_in_win(m,1)-q);
      set_win_pp_grp_id(m-n,get_win_pp_grp_id(m)-1);
      set_win_pp_win_mark(m-n,m-n);

      for (o=0;o<STRU_NUM;o++)
      {
	set_win_pp_wins_dbf(o,m-n,get_win_pp_wins_dbf(o,m));
      }

      set_win_pp_get_rd(m-n,get_win_pp_get_rd(m));
      ...
      bw_dialog1.setc_win_mv_getv(m,SMG_SIZE-1,0);
      ...
      set_win_h_up_pp(m-n,0,get_win_h_up_pp(m,0)-t);
      set_win_h_up_pp(m-n,1,get_win_h_up_pp(m,1)-t);
	...
	for (p=1;p<COL_NUM;p++)
	{
	  bw_buff1.set_t_winfile(m-n,o,p,bw_buff1.get_t_winfile(m,o,p));
    ...
    set_win_pp_grp_pp(k-1,0,get_win_pp_grp_pp(k,0)-n);
    set_win_pp_grp_pp(k-1,1,get_win_pp_grp_pp(k,1)-n);
    set_win_pp_grp_pp(k-1,2,get_win_pp_grp_pp(k,2)-n);

    if (k==l)
    {
      set_win_pp_grp_pp(l,0,0);
      set_win_pp_grp_pp(l,1,0);
      set_win_pp_grp_pp(l,2,0);
    }
  }

  for (k=p10-n+1;k<=p10;k++)
  {
    set_win_pp_fld_in_win(k,0,0);
    set_win_pp_fld_in_win(k,1,0);
    set_win_pp_win_mark(k,0);
    set_win_pp_grp_id(k,0);

    for (o=0;o<STRU_NUM;o++)
    {
      set_win_pp_wins_dbf(o,k,0);
    }

    set_win_pp_get_rd(k,0);
  }
  ...
  j=get_win_h_w_pp(i,1)-get_win_h_w_pp(i,0)+1;
  if (get_win_h_w_pp(i,5)==0)
    j=j;
  else
    j=j+get_win_h_w_pp(i,5)-get_win_h_w_pp(i,4)+1;

  if (get_win_h_w_pp(i,5)==0) m=get_win_h_w_pp(i,4);
  else m=get_win_h_w_pp(i,5)+1;

  if (get_win_h_w_pp(l,5)==0) o=get_win_h_w_pp(l,1);
  else o=get_win_h_w_pp(l,5);
/*
...
*/
  for (k=i+1;k<=l;k++)
  {
    if (get_win_h_w_pp(k,0)>=j) set_win_h_w_pp(k-1,0,get_win_h_w_pp(k,0)-j);
    else                  set_win_h_w_pp(k-1,0,0);
    if (get_win_h_w_pp(k,1)>=j) set_win_h_w_pp(k-1,1,get_win_h_w_pp(k,1)-j);
    else                  set_win_h_w_pp(k-1,1,0);
    if (get_win_h_w_pp(k,2)>=j) set_win_h_w_pp(k-1,2,get_win_h_w_pp(k,2)-j);
    else                  set_win_h_w_pp(k-1,2,0);
    if (get_win_h_w_pp(k,4)>=j) set_win_h_w_pp(k-1,4,get_win_h_w_pp(k,4)-j);
    else                  set_win_h_w_pp(k-1,4,0);
    if (get_win_h_w_pp(k,5)>=j) set_win_h_w_pp(k-1,5,get_win_h_w_pp(k,5)-j);
    else                  set_win_h_w_pp(k-1,5,0);
    if (get_win_h_w_pp(k,6)>=j) set_win_h_w_pp(k-1,6,get_win_h_w_pp(k,6)-j);
    else                  set_win_h_w_pp(k-1,6,0);

    if (k==l)
    {
      set_win_h_w_pp(k,0,0);
      set_win_h_w_pp(k,1,0);
      set_win_h_w_pp(k,2,0);
      set_win_h_w_pp(k,3,0);
      set_win_h_w_pp(k,4,0);
      set_win_h_w_pp(k,5,0);
      set_win_h_w_pp(k,6,0);
    }
  }

  for (k=m;k<=o;k++)
  {
    if (get_win_h_ci_pp(k,4)!=0)
    {
      //SetWindowLong(get_win_hnd(k),GWL_ID,k-j);
    }

    if (get_win_h_ci_pp(k,0)>=n) set_win_h_ci_pp(k-j,0,get_win_h_ci_pp(k,0)-n);  /* win num,x,y,hnd num,main hnd */
    set_win_h_ci_pp(k-j,1,get_win_h_ci_pp(k,1));
    set_win_h_ci_pp(k-j,2,get_win_h_ci_pp(k,2));
    set_win_h_ci_pp(k-j,3,k-j);

    if (get_win_h_ci_pp(k,4)==0) set_win_h_ci_pp(k-j,4,0);
    else set_win_h_ci_pp(k-j,4,get_win_h_ci_pp(k,4)-j);
  ...
  for (k=o-j+1;k<=o;k++)
  {
    set_win_h_ci_pp(k,0,0);  /* win num,x,y,hnd num,main hnd */
    set_win_h_ci_pp(k,1,0);
    set_win_h_ci_pp(k,2,0);
    set_win_h_ci_pp(k,3,0);
    set_win_h_ci_pp(k,4,0);
  ...
  if (i==l)
  {
    set_win_h_w_pp(i,0,0);
    set_win_h_w_pp(i,1,0);
    set_win_h_w_pp(i,2,0);
    set_win_h_w_pp(i,3,0);
    set_win_h_w_pp(i,4,0);
    set_win_h_w_pp(i,5,0);
    set_win_h_w_pp(i,6,0);
  }

  /* end mem var move */

  win_pp_new_win=win_pp_new_win-n;

  win_h_w_total_pp=win_h_w_total_pp-j;
...
int bw_win::w_read_winfile(char *p_fn2,int p_fn2_size)
{
  int  p1,p2,same,ends;
  ...
  for (k=win_pp_new_win;k<WIN_NUM-1;k++)
  {
    set_win_timer(k,0);
	...
	continue;
	  ...
	  if (s_firstline==0)
	  {
	    if (s_lastline==1)
	      ...
	      if ((lcnt-1>0)&&(lcnt-1<LIN_NUM))
	      {
	        for (j=1;j<COL_NUM;j++)
	        {
	          bw_buff1.set_t_winfile(k,lcnt-1,j,' ');
	        }
	      }
	      s_lastline=0;

	      bw_buff1.set_t_winfile(k,lcnt-1,s_beginposi+1,'.');
	      bw_buff1.set_t_winfile(k,lcnt-1,s_endposi+1,'.');
      ...
      for (j=0;j<COL_NUM;j++)
      {
	if ((str1[j]<' ')&&(str1[j]>=0)) str1[j]=' ';
	bw_buff1.set_t_winfile(k,lcnt,j+1,str1[j]);
      }
  ...
  w_login_win(m,k,p_fn2,p_fn2_size,tmps7,FN_SIZE);
  ...
  if (is_tab==1)  /* it is a table scrn */
  {
    ...
    for (i=m;i<=k;i++)  /* mark win base pp */
    {
      ...
      {
	set_win_pp_wins_dbf(j,i,0);
      }

      for (j=0;j<=s_ps;j++)
      {
        if (tmps6[j]>=STRU_NUM) continue;
	set_win_pp_wins_dbf(tmps6[j],i,1);
	...
	return(1);
      ...
      {
	bw_inkey1.strpcut(tmps5[s_ps1],0,i,FN_SIZE,stn2,FN_SIZE);
	...
	else bw_inkey1.strpcut(tmps5[s_ps1],p1+5,i-p1-5,FN_SIZE,stn2,FN_SIZE);
	...
	if (i-p1-5<=0) w_atten60();
	else bw_inkey1.strpcut(tmps5[s_ps1],p1+9,i-p1-9,FN_SIZE,str2,FN_SIZE);
      }
      ...
      {
	if ((tmps5[s_ps1][i]==' ')||(tmps5[s_ps1][i]==','))
	{
	  j=1;
	  break;
	}
	...
	else bw_inkey1.strpcut(tmps5[s_ps1],p1+7,i-p1-7,FN_SIZE,str3,FN_SIZE);
      }
	...
	if (i-p1>0)
	{
	  bw_inkey1.strpcut(tmps5[s_ps1],p1,i-p1,FN_SIZE,str4,FN_SIZE);
  ...
  {
/*
    bw_inkey1.strtrim(tmps8,FN_SIZE);
    i=bw_inkey1.strlocc(tmps8,FN_SIZE,'.');
    tmps8[i]=0;
    strcat(tmps8,".dbf");
*/
/*
    for (j=0;j<STRU_NUM;j++)
    {
      if (bw_xbase1.get_win_state_bff_in_use(j)==1)
      {
        bw_xbase1.get_win_basename(j,stn1,FN_SIZE);

	if (strncmp(tmps8,stn1,strlen(tmps8))==0)
	{
...
int bw_win::w_dele_xbase_state(int p_wpp)
{
  int  i;

  for (i=0;i<STRU_NUM;i++)
  {
    if (get_win_pp_wins_dbf(i,p_wpp)==1)
    {
...
{ 
  ...
  if (get_win_pp_fld_in_win(p_pagecnt,0)==0)
  {
    i=p_pagecnt-1;
    while ((get_win_pp_fld_in_win(i,1)==0)&&(i>0))
    {
      i--;
    }
    set_win_pp_fld_in_win(p_pagecnt,0,get_win_pp_fld_in_win(i,1)+1);
    pp1=get_win_pp_fld_in_win(i,1);
  }
  else pp1=get_win_pp_fld_in_win(p_pagecnt,0)-1;
  ...
  for (i=1;i<LIN_NUM;i++)
  {
    ...
    {                                                                                    
      ...
      for (j=1;j<COL_NUM;j++)  
      {
        if ((bw_buff1.get_t_winfile(p_pagecnt,i,j)>' ')||(bw_buff1.get_t_winfile(p_pagecnt,i,j)<0))
        {
	  p1=j;
	  break;
        }
    ...
    else
    {
      ...
      for (k=p1;k<=p2;k++)
      {
	if ((bw_buff1.get_t_winfile(p_pagecnt,i,k)>' ')||(bw_buff1.get_t_winfile(p_pagecnt,i,k)<0))
	{
	  j=1;
	  break;
	}
    ...
    for (j=p1;j<=p2;j++)  
    {
      if (bw_buff1.get_t_winfile(p_pagecnt,i,j)=='&')
      {
        pp1++;
	p4=0;      
	p5=1;      
	...
	if (bw_buff1.get_t_winfile(p_pagecnt,i,j)<=' ')
	{
	  ...
	  if (mark==1)
	  {
	    if (bw_buff1.get_t_winfile(p_pagecnt,i,j)<0)
	    {
	      if (pp3==1)
	      {
		pp3=0;
		continue;
	      ...
	      {
                pp3=1;
		str[0]=bw_buff1.get_t_winfile(p_pagecnt,i,j);
		if (bw_buff1.get_t_winfile(p_pagecnt,i,j+1)!='&')
                {
		  str[1]=bw_buff1.get_t_winfile(p_pagecnt,i,j+1);
                  str[2]=0;
                }
                else str[1]=0;
		if (get_win_ci_mode(p_pagecnt)==0)
	...
	else
	{
	  ...
	  {
	    if ((bw_buff1.get_t_winfile(p_pagecnt,i,j)=='(')
	      &&(bw_buff1.get_t_winfile(p_pagecnt,i,j-1)!='&'))
	    {
	      state7=1;
              state1=0;
              continue;
            }
	    if ((p4==0)&&(bw_buff1.get_t_winfile(p_pagecnt,i,j-1)=='%'))
	    {
	      p4++;
	    }
	    p4++;
...
{ 
  ...
  win_h_ci_total_pp=win_h_w_total_pp;

  pp3=win_h_ci_total_pp;

  if (win_h_ci_total_pp>=HND_NUM) return(0);
  ...
  for (i=1;i<LIN_NUM;i++)
  {
    for (j=1;j<COL_NUM;j++)
    {
      bw_buff1.set_linecol(p_pagecnt,i,j,0);
    }
  }
  ...
  {                                                                                       
	...
	if (p5==1)
	{
	  ...
	  get_1fldname(pp1,str2,FLD_NAME_LEN);
	...
	pp1++;
	if (bw_buff1.get_t_winfile(p_pagecnt,i,j+1)=='#') bw_buff1.set_linecol(p_pagecnt,i,j+1,pp1);
...
/*
		if (str2[0]=='@')
		{
		  ...
		  {
		    if (bw_buff1.get_t_winfile(p_pagecnt,i,t1)=='(') break;
                  }

		  for (t2=p6;t2<COL_NUM;t2++)
		  {
		    if (bw_buff1.get_t_winfile(p_pagecnt,i,t2)==')') break;
		  }
		  ...
		  for (l=t1+1;l<t2;l++)
		  {
		    str1[l-t1-1]=bw_buff1.get_t_winfile(p_pagecnt,i,l);
		    str1[l-t1]=0;
                  }
...
*/
	    ...
	    set_1fldname1(pp1,p4+0,bw_buff1.get_t_winfile(p_pagecnt,i,j));
...
/*
  ...
  for (i=pp3;i<=get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5);i++)
  {
    set_win_fpp(i,(FARPROC)GetWindowLong(get_win_hnd(i),GWL_WNDPROC));
    SetWindowLong(get_win_hnd(i),GWL_WNDPROC,(LPARAM)win_tfpp);
  }
*/
  set_win_h_up_pp(p_pagecnt,0,get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),6));
  set_win_h_up_pp(p_pagecnt,1,get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5));

  win_h_w_total_pp=win_h_ci_total_pp;

  set_win_pp_fld_in_win(p_pagecnt,1,pp1);

  // adjust field length for 800*600 256color mode
  /*
  for (i=get_win_pp_fld_in_win(p_pagecnt,0);i<=get_win_pp_fld_in_win(p_pagecnt,1);i++)
  {
    if (get_1fldlen(i)>18)
    {
...
int bw_win::w_cre_ci_button(int p_pagecnt,char *p_s1,int p_s1_size,int p6,int p7,int p8,int p9,char *p_s2,int p_s2_size)
{
  HWND hnd;
  int  l,j;
  char s3[SMG_SIZE];
  ...
  if (get_win_ci_mode(p_pagecnt)==0)
  {
    hnd=CreateWindow("button",s3
		      ...
		      ,bw_main1.win_hwnd,(HMENU)win_h_ci_total_pp
		      ,bw_main1.win_ins,0);
  }
  else
  {
  ...
  set_win_hnd(win_h_ci_total_pp,hnd);
  if (get_win_hnd(win_h_ci_total_pp)==0) bw_main1.win_cre_ok=0;

  set_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5,win_h_ci_total_pp);
  set_win_h_ci_pp(win_h_ci_total_pp,0,p_pagecnt);
  set_win_h_ci_pp(win_h_ci_total_pp,1,p6);
  set_win_h_ci_pp(win_h_ci_total_pp,2,p9);
  set_win_h_ci_pp(win_h_ci_total_pp,3,win_h_ci_total_pp);
  set_win_h_ci_pp(win_h_ci_total_pp,4,p_pagecnt-get_win_pp_grp_pp(get_win_pp_grp_id(p_pagecnt),0)+get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),0));
...
int bw_win::w_cre_ci_radio(int p_pagecnt,int p6,int p7,int p8,int p9,char *p_s1,int p_s1_size)
{
  HWND hnd;
  char str1[SMG_SIZE];
...
int bw_win::w_cre_ci_check(int p_pagecnt,int p6,int p7,int p8,int p9,char *p_s1,int p_s1_size)
{
  HWND hnd;
  char str1[SMG_SIZE];
	...
	if (bw_buff1.get_t_winfile(p_pagecnt,i,j)<=' ')
	{
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
	    if (p5==1)
	    {
		  ...
		  w_cre_ci_button(p_pagecnt,str1,SMG_SIZE,p6,p7,p8,i,str2,strlen(str2)+1);
...
int bw_win::w_cre_win_edit(int p_pagecnt,int mark) // create edit ci
{
  ...
  for (i=hpp1;i<=get_win_h_w_pp(get_win_pp_grp_id(p_pagecnt),5);i++)
  {
    //set_win_fpp(i,(FARPROC)GetWindowLong(get_win_hnd(i),GWL_WNDPROC));
    //SetWindowLong(get_win_hnd(i),GWL_WNDPROC,(LPARAM)win_tfpp);
  }
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(str2)==1)
		{
		  w_cre_ci_radio(p_pagecnt,p6,p7,p8,i,str2,strlen(str2)+1);
	  ...
	  if (bw_buff1.get_t_winfile(p_pagecnt,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(str2)==1)
		{
		  w_cre_ci_check(p_pagecnt,p6,p7,p8,i,str2,strlen(str2)+1);
...
int bw_win::w_find_hnd(int p1,int p2,int p3)
{
  ...
  for (i=0;i<=win_h_w_total_pp-1;i++)
  {
    if ((get_win_h_ci_pp(i,0)==p1)&&
	(get_win_h_ci_pp(i,1)==p2)&&
	(get_win_h_ci_pp(i,2)==p3))
    {
      j=get_win_h_ci_pp(i,3);
      break;
    }
  }
...
int bw_win::w_find_winfname(int w,int l,int c,char *p_s1,int p_s1_size)
{
  ...
  if (i>0)
  {
    ...
    if (FLD_NAME_LEN>p_s1_size) k=p_s1_size;
    else k=FLD_NAME_LEN;
...
int bw_win::w_field_info3(int w)
{
  ...
  for (i=1;i<LIN_NUM;i++) /* find first screen field */
  {
	...
	get_1fldname(k,str2,FLD_NAME_LEN);

	if (str2[0]=='*') set_1fldedt(k,1);
	else
	{
	  if (strlen(str2)<=1) set_1fldedt(k,1);
	  else
	  {
	    if (str2[0]=='%') set_1fldedt(k,1);
...
int bw_win::w_down_line(int p_pp,int p1)                      
{
  ...
  for (i=1;i<LIN_NUM;i++) /* find first screen field */
  {
    for (j=1;j<COL_NUM;j++)
    {
      ...
      get_1fldname(n,str2,FLD_NAME_LEN);
...
int bw_win::w_field_type(char *str)
{
  ...
  if (str[0]=='$') return(1);  /* sound */
  if (str[0]=='(') return(2);  /* (X)    ci */ // ci : control item
  if (str[0]=='[') return(3);  /* [X]    ci */ // ci : control item
  if (str[0]=='@') return(4);  /* button ci */ // ci : control item
  if (str[0]=='#') return(5);  /* bitmap */
  if (str[0]=='%') return(6);  /* read database var */
  if (str[0]==':') return(7);  /* edit database var */
  if (str[0]=='?') return(8);  /* memory var */
  if (str[0]=='+') return(9);  /* edit database ci */ // ci : control item
  if (str[0]=='-') return(10); /* edit var      ci */ // ci : control item
  return(0);
}

int bw_win::w_fetch_fld_style(char *tfn,int tfn_size,char *tn,int tn_size,long *tr,char *fn,int fn_size,int p_wpp)
{
  int  i,j,k,l;
  int  p1;
  int  s_pp1[3][2];
  ...
  *tr=0;
  ...
  if (j==0)  /* 1 jie */
  {
    i=w_find_base_name(s_fld[0],SMG_SIZE,p_wpp);
    k=w_find_recno_name(s_fld[0],SMG_SIZE);
    l=w_find_fld_name(s_fld[0],SMG_SIZE,p_wpp);
    if (i==1)
    {
  ...
  if (j==1)  /* 2 jie */
  {
    ...
    i=w_find_base_name(s_fld[1],SMG_SIZE,p_wpp);
    k=w_find_recno_name(s_fld[1],SMG_SIZE);
    l=w_find_fld_name(s_fld[1],SMG_SIZE,p_wpp);
    if (i==1)
    {
      if (p1==1)
      {
  ...
  if (j==2)  /* 3 jie */
  {
    i=w_find_base_name(s_fld[0],SMG_SIZE,p_wpp);
    k=w_find_recno_name(s_fld[1],SMG_SIZE);
    l=w_find_fld_name(s_fld[2],SMG_SIZE,p_wpp);
    if (i==1)
    {
      if ((int)strlen(s_fld[0])<tn_size)
      {
...
int bw_win::w_find_base_name(char *p_str,int p_str_size,int p_wpp)
{
  int i,j;
  char stn[FN_SIZE];
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    if ((get_win_pp_wins_dbf(i,p_wpp)==1)&&(strcmp(stn,p_str)==0))
    {
      j=1;
      break;
    }
...
int bw_win::w_find_fld_name(char *p_str,int p_str_size,int p_wpp)
{
  int  i,j,k;
  char s_fn1[SMG_SIZE];
...
int bw_win::w_fld_set_base(char *p_fldtable,int p_fldtable_size,int pp1,int p_wpp)
{
  int  i,j,k;
  char stn[FN_SIZE];
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    if ((get_win_pp_wins_dbf(i,p_wpp)==1)&&(strcmp(p_fldtable,stn)==0))
    {
      k=1;
      break;
    }
...
int bw_win::w_field_info2(int p_wpp)
{
  int  i,j,k,l,m,n;
  int  exist;
  int  pp1;
  ...
  for (i=get_win_pp_fld_in_win(p_wpp,0);i<=get_win_pp_fld_in_win(p_wpp,1);i++)
  {
    set_1fldbase(i,0);
    set_1fldedt(i,0);
    set_1fldseri(i,0);
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	if ((str2[0]==':')||(str2[0]=='%')||(str2[0]=='+')||(str2[0]=='-'))
	{
	  ...
	  pp1=w_fld_set_base(s_fldtable,SMG_SIZE,pp1,p_wpp);
	  set_1fldbase(n,pp1);
	  ...
	  l=0;
	  for (k=bw_buff1.get_linecol(m,i,j)-1;k>=get_win_pp_fld_in_win(p_wpp,0);k--)
	  {
	    get_field_tmp(k,str1,FLD_NAME_LEN);
	  ...
	  if (s_tmpn1==1)
	    ...
	    set_1fldedt(n,w_fld_set_fld(s_fldname,SMG_SIZE));
...
int bw_win::w_adjust_edit(int p_wpp)
{
  HWND hnd;
  ...
  for (i=1;i<LIN_NUM;i++)
  {
    for (j=1;j<COL_NUM;j++)
    {
      if (bw_buff1.get_linecol(p_wpp,i,j)>0)
      {
        n=bw_buff1.get_linecol(p_wpp,i,j);
	...
	if (str2[0]=='+')
	  ...
	  bw_inkey1.strpcut(str2,1,strlen(str2)-1,FLD_NAME_LEN,str1,FLD_NAME_LEN);

	  o=bw_fldstru1.w_find_fldlen(str1,FLD_NAME_LEN,l);
	  ...
	  q=w_find_hnd(p_wpp,j,i);

	  hnd=get_win_hnd(q);

	  if (get_win_ci_mode(p_wpp)==0)
          {
	    MoveWindow(hnd
			,(int)((j-get_win_con(p_wpp,0))*bw_main1.win_edit_xchar-3)
			,(int)((i-get_win_con(p_wpp,1))*bw_main1.win_edit_ychar-6)
	  ...
	  {
	    MoveWindow(hnd
			,(int)((j-get_win_con(p_wpp,0))*bw_main1.win_xchar-3)
			,(int)((i-get_win_con(p_wpp,1))*bw_main1.win_ychar-6)
			,(int)(p*bw_main1.win_xchar+6)
			,(int)(bw_main1.win_ychar+12)
			,FALSE);
	  }

	  SendMessage(hnd,EM_LIMITTEXT,o,(LPARAM)str1);
	}
	...
	if (str2[0]=='-')
	  ...
	  r=bw_dialog1.w_mv_seri(str2,FLD_NAME_LEN,p_wpp);
...
int bw_win::w_cnt_winline(int basep,int p_wpp,int ln)
{
  int i,j,k,l;
  ...
  for (i=1;i<ln;i++) /* find first screen field */
  {
    for (j=1;j<COL_NUM;j++)
    {
      l=bw_buff1.get_linecol(p_wpp,i,j);
...
long bw_win::get_win_timer(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_timer[p_wpp]);
}
int bw_win::set_win_timer(int p_wpp,long val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_timer[p_wpp]=val;
  return(0);
...
int bw_win::get_1fldname(int p1,char *p_val,int p_val_size)
{
  int i,j;
  ...
  if (p_val_size>FLD_NAME_LEN) j=FLD_NAME_LEN;
  else j=p_val_size;
...
int bw_win::set_1fldname(int p1,char *p_val,int p_val_size)
{
  int i,j;
...
int bw_win::get_field_tmp(int p1,char *p_val,int p_val_size)
{
  int i,j;
...
int bw_win::set_field_tmp(int p1,char *p_val,int p_val_size)
{
  int i,j;
...
/*
int bw_win::get_win_subhndid(int p_wpp,int pp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>9)) return(0);
  return(win_subhndid[p_wpp][pp]);
}
int bw_win::set_win_subhndid(int p_wpp,int pp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>9)) return(0);
  win_subhndid[p_wpp][pp]=val;
  return(0);
}
int bw_win::get_win_subhndpp(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_subhndpp[p_wpp]);
}
int bw_win::set_win_subhndpp(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_subhndpp[p_wpp]=val;
  return(0);
}
*/
int bw_win::get_win_state_link_modi(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_state_link_modi[p_wpp]);
}
int bw_win::set_win_state_link_modi(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_state_link_modi[p_wpp]=val;
  return(0);
}
int bw_win::get_win_state_calc_modi(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_state_calc_modi[p_wpp]);
}
int bw_win::set_win_state_calc_modi(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_state_calc_modi[p_wpp]=val;
  return(0);
}
int bw_win::get_win_switch_bmp(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_switch_bmp[p_wpp]);
}
int bw_win::set_win_switch_bmp(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_switch_bmp[p_wpp]=val;
  return(0);
}

int bw_win::get_win_pp_fld_in_win(int p_wpp,int pp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>3)) return(0);
  return(win_pp_fld_in_win[p_wpp][pp]);
}
int bw_win::set_win_pp_fld_in_win(int p_wpp,int pp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>3)) return(0);
  win_pp_fld_in_win[p_wpp][pp]=val;
  return(0);
}
int bw_win::get_win_h_w_pp(int p_wpp,int pp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>8)) return(0);
  return(win_h_w_pp[p_wpp][pp]);
}
int bw_win::set_win_h_w_pp(int p_wpp,int pp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>8)) return(0);
  win_h_w_pp[p_wpp][pp]=val;
  return(0);
}
int bw_win::get_win_h_up_pp(int p_wpp,int pp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>1)) return(0);
  return(win_h_up_pp[p_wpp][pp]);
}
int bw_win::set_win_h_up_pp(int p_wpp,int pp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>1)) return(0);
  win_h_up_pp[p_wpp][pp]=val;
  return(0);
}

HWND bw_win::get_win_phh(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_phh[p_wpp]);
}

int bw_win::set_win_phh(int p_wpp,HWND hwnd)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_phh[p_wpp]=hwnd;
  return(0);
...
int bw_win::get_win_tabname(int p1,int p_fpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s1[j+0]=win_tabname[p1][p_fpp][j];
    p_s1[j+1]=0;
  }

  return(0);
}
int bw_win::set_win_tabname(int p1,int p_fpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  win_tabname[p1][p_fpp][0]=0;

  for (j=0;j<i-1;j++)
  {
    win_tabname[p1][p_fpp][j+0]=p_s1[j];
    win_tabname[p1][p_fpp][j+1]=0;
  }

  return(0);
}
int bw_win::get_win_pp_win_mark(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_pp_win_mark[p_wpp]);
}
int bw_win::set_win_pp_win_mark(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_pp_win_mark[p_wpp]=val;
  return(0);
}
int bw_win::get_win_pp_grp_id(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_pp_grp_id[p_wpp]);
}
int bw_win::set_win_pp_grp_id(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_pp_grp_id[p_wpp]=val;
  return(0);
}


int bw_win::get_win_msgboxlock(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_msgboxlock[p_wpp]);
}
int bw_win::set_win_msgboxlock(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_msgboxlock[p_wpp]=val;
  return(0);
}


int bw_win::get_win_pp_grp_pp(int p_wpp,int p04)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p04<0)||(p04>=5)) return(0);
  return(win_pp_grp_pp[p_wpp][p04]);
}
int bw_win::set_win_pp_grp_pp(int p_wpp,int p04,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p04<0)||(p04>=5)) return(0);
  win_pp_grp_pp[p_wpp][p04]=val;
  return(0);
}
int bw_win::get_win_pp_wins_dbf(int p_fpp,int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p_fpp<0)||(p_fpp>=STRU_NUM)) return(0);
  return(win_pp_wins_dbf[p_fpp][p_wpp]);
}
int bw_win::set_win_pp_wins_dbf(int p_fpp,int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p_fpp<0)||(p_fpp>=STRU_NUM)) return(0);
  win_pp_wins_dbf[p_fpp][p_wpp]=val;
  return(0);
}
int bw_win::get_win_con(int p_wpp,int p03)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p03<0)||(p03>3)) return(0);
  return(win_con[p_wpp][p03]);
}
int bw_win::set_win_con(int p_wpp,int p03,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p03<0)||(p03>3)) return(0);
  win_con[p_wpp][p03]=val;
  return(0);
}
int bw_win::get_win_style(int p_wpp,int pp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>=WIN_STYLE_NUM)) return(0);
  return(win_style[p_wpp][pp]);
}
int bw_win::set_win_style(int p_wpp,int pp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((pp<0)||(pp>=WIN_STYLE_NUM)) return(0);
  win_style[p_wpp][pp]=val;
  return(0);
}
...
int bw_win::get_win_h_ci_pp(int hpp,int p04)
{
  if ((hpp<0)||(hpp>=HND_NUM)) return(0);
  if ((p04<0)||(p04>4)) return(0);
  return(win_h_ci_pp[hpp][p04]);
}
int bw_win::set_win_h_ci_pp(int hpp,int p04,int val)
{
  if ((hpp<0)||(hpp>=HND_NUM)) return(0);
  if ((p04<0)||(p04>4)) return(0);
  win_h_ci_pp[hpp][p04]=val;
  return(0);
}
int bw_win::get_win_pp_get_rd(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_pp_get_rd[p_wpp]);
}
int bw_win::set_win_pp_get_rd(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_pp_get_rd[p_wpp]=val;
  return(0);
}
int bw_win::get_win_create_menu(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_create_menu[p_wpp]);
}
int bw_win::set_win_create_menu(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_create_menu[p_wpp]=val;
  return(0);
}
int bw_win::get_win_linep(int p_wpp,int p01)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(win_linep[p_wpp][p01]);
}
int bw_win::set_win_linep(int p_wpp,int p01,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  win_linep[p_wpp][p01]=val;
  return(0);
}
int bw_win::get_win_window_name(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s1[j+0]=win_window_name[p_wpp][j];
    p_s1[j+1]=0;
  }

  return(0);
}
int bw_win::set_win_window_name(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  win_window_name[p_wpp][0]=0;

  for (j=0;j<i-1;j++)
  {
    win_window_name[p_wpp][j+0]=p_s1[j];
    win_window_name[p_wpp][j+1]=0;
  }

  return(0);
}
int bw_win::get_win_find_name(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s1[j+0]=win_find_name[p_wpp][j];
    p_s1[j+1]=0;
  }

  return(0);
}
int bw_win::set_win_find_name(int p_wpp,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  win_find_name[p_wpp][0]=0;

  for (j=0;j<i-1;j++)
  {
    win_find_name[p_wpp][j+0]=p_s1[j];
    win_find_name[p_wpp][j+1]=0;
  }

  return(0);
}
int bw_win::get_win_is_link(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_is_link[p_wpp]);
}
int bw_win::set_win_is_link(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_is_link[p_wpp]=val;
  return(0);
}
int bw_win::get_win_is_calc(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_is_calc[p_wpp]);
}
int bw_win::set_win_is_calc(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_is_calc[p_wpp]=val;
  return(0);
}
int bw_win::get_win_memo_basep(int p_wpp)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  return(win_memo_basep[p_wpp]);
}
int bw_win::set_win_memo_basep(int p_wpp,int val)
{
  if ((p_wpp<0)||(p_wpp>=WIN_NUM)) return(0);
  win_memo_basep[p_wpp]=val;
  return(0);
}
...
int bw_win::get_win_ci_mode(int p_wpp)
{
  if ((p_wpp<1)||(p_wpp>=WIN_NUM)) return(0);
  return(win_ci_mode[p_wpp]);
}
int bw_win::set_win_ci_mode(int p_wpp,int val)
{
  if ((p_wpp<1)||(p_wpp>=WIN_NUM)) return(0);
  win_ci_mode[p_wpp]=val;
  return(0);
}

int bw_win::get_win_ci_posi(int p_wpp,int p01)
{
  if ((p_wpp<1)||(p_wpp>=WIN_NUM)) return(0);
  if ((p01<0)||(p01>1))       return(0);
  return(win_ci_posi[p_wpp][p01]);
}
int bw_win::set_win_ci_posi(int p_wpp,int p01,int val)
{
  if ((p_wpp<1)||(p_wpp>=WIN_NUM)) return(0);
  if ((p01<0)||(p01>1))       return(0);
  win_ci_posi[p_wpp][p01]=val;
  return(0);
}
