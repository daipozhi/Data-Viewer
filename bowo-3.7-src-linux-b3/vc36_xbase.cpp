...
int bw_xbase::w_open_xbase(char *p_fn1,int p_fn1_size,int backup,int p_fpp)
{
  ...
  while(1)
  {
    win_fp[p_fpp]=/*s*/open(s_fn1,O_RDWR/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
  ...
  if (s_state1==1)              // differant database
  {
    ...
    else			        // update stru
    {
      ...
      win_fp[p_fpp] = /*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY, SH_DENYNO, S_IREAD|S_IWRITE*/);
  ...
  if (backup==1)
  {
    ...
    win_fp[p_fpp]=/*s*/open(s_fn2,O_RDWR/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
...
//  char  c1;
  ...
  while (1)
  {
    ...
    if (/*eof(win_fp[p_fpp])*/j<get_win_lcnt(p_fpp,1)+1) break;
...
int bw_xbase::w_use_fox(char *p_fn1,int p_fn1_size)
{
  ...
  l=read(win_use_fh,s_tmps1,16);      /* calculate record number */
  if (/*eof(win_use_fh)*/l<16) return(1);
