...
int bw_link::w_read_link(char *p_fn,int p_fn_size,int p_wptr,int num)
{
  FILE *fp1;
  int   i,j,k,l;
...
int bw_link::w_find_link_tn(char *p_s1,int p_s1_size,int p_wptr,int i)
{
  int  j,l,m,n;
...
//	      win_tst1=0;
		...
		{
		  u=bw_buff1.get_linecol(p_wptr,bw_win1.pline,bw_win1.pfield);
...
int bw_link::w_link_set_continue(int p_wptr,int p_fptr)
{
  if ((p_wptr<1)||(p_wptr>=WIN_NUM)) return(0);
  if ((p_fptr<0)||(p_fptr>=STRU_NUM)) return(0);
  set_win_continue(p_fptr,p_wptr,1);
  return(0);
}

int bw_link::w_keep_linkfld(int p_wptr,int p_fptr,int dir)
{
  int  i,j,k,l,m,n;
  int  exist;
  char str4[SMG_SIZE];
  ...
  for (i=0;i<COND_NUM;i++)   
  {
    if ((get_win_bslink(i,1)==p_fptr)&&(get_win_bslink(i,2)==p_wptr))
    {
      j=get_win_bslink(i,0);
      exist=1;
      break;
  ...
  bw_fldstru1.w_sav_tab_var(str3,SMG_SIZE,p_fptr,str4,SMG_SIZE);
  bw_mbase1.w_save_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0)+dir);
...
int bw_link::get_win_continue(int p_fptr,int p_wptr)
{
  if ((p_fptr<0)||(p_fptr>=STRU_NUM)) return(0);
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  return(win_continue[p_fptr][p_wptr]);
}
int bw_link::set_win_continue(int p_fptr,int p_wptr,int val)
{
  if ((p_fptr<0)||(p_fptr>=STRU_NUM)) return(0);
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  win_continue[p_fptr][p_wptr]=val;
  return(0);
}
