...
int bw_xbase::w_open_xbase(char *p_fn1,int p_fn1_size,int backup,int p_fptr)
{
  ...
  while(1)
  {
    win_fp[p_fptr]=sopen(s_fn1,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  ...
  if (s_state1==1)              // differant database
  {
    ...
    else			        // update stru
    {
      ...
      win_fp[p_fptr] = sopen(s_fn1,O_RDWR|O_CREAT|O_BINARY, SH_DENYNO, S_IREAD|S_IWRITE);
  ...
  if (backup==1)
  {
    ...
    win_fp[p_fptr]=sopen(s_fn2,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
