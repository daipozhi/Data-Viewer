...
int bw_fldstru::w_cnt_tabfield(int p_fptr)
{
//  int i,j,k;
  return(get_t2_fldpoin(p_fptr,1)-get_t2_fldpoin(p_fptr,0));
}
int bw_fldstru::w_cnt_reclen(int p_fptr)
{
  int i,j;
  j=0;
  for (i=get_t2_fldpoin(p_fptr,0);i<get_t2_fldpoin(p_fptr,1);i++)
  {
    j=j+get_t2_fldlen(i);
  }
  return(j);
}
int bw_fldstru::w_cnt_fldposi(int p_fptr)
{
  int i,j;
  j=1;
  for (i=get_t2_fldpoin(p_fptr,0);i<get_t2_fldpoin(p_fptr,1);i++)
...
int bw_fldstru::w_read_stru(char *p_fn1,int p_fn1_size,int p_fptr)
{
  int  exist;
  int  i,j,k,l;
...
//    k++;
  ...
  set_t2_fldpoin(p_fptr,1,pp1+k);

  fclose(fp1);

  bw_xbase1.set_win_fld_cnt(p_fptr/*,0*/,w_cnt_tabfield(p_fptr));   /* field number */
  bw_xbase1.set_win_len_cnt(p_fptr/*,1*/,  w_cnt_reclen(p_fptr));   /* record length */
  w_cnt_fldposi(p_fptr);
...
int bw_fldstru::w_dele_stru(int p_fptr)
{
  int  i,j;
  int  p1,p3,p4;
  int  n1;
  ...
  n1=get_t2_fldpoin(p_fptr,1)-get_t2_fldpoin(p_fptr,0);
  p1=get_t2_fldpoin(p_fptr,1);
  p4=0;
  ...
  if (p4==0) p4=get_t2_fldpoin(p_fptr,1);
  ...
  set_t2_fldpoin(p_fptr,0,0);
  set_t2_fldpoin(p_fptr,1,0);

  bw_xbase1.set_win_fld_cnt(p_fptr/*,0*/,0);   /* field number */
  bw_xbase1.set_win_len_cnt(p_fptr/*,1*/,0);   /* record length */

  return(0);
}

int bw_fldstru::w_find_fldnum(char *p_str,int p_str_size,int p_fptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldlen(char *p_str,int p_str_size,int p_fptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldtype(char *p_str,int p_str_size,int p_fptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_flddec(char *p_str,int p_str_size,int p_fptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldposi(char *p_str,int p_str_size,int p_fptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_echo_winrec(int p_wptr)
{
  ...
  for (k=0;k<STRU_NUM;k++)
  {
	...
	for (j=1;j<COL_NUM;j++)
	{
	  if (bw_buff1.get_linecol(p_wptr,i,j)!=0)
	  {
	    m=bw_buff1.get_linecol(p_wptr,i,j);
...
int bw_fldstru::w_tv_get_read_paint(char *p_str,int p_str_size,int p_wptr,int l,int c,char comm)
{
  ...
  i=bw_buff1.get_linecol(p_wptr,l,c);
  ...
  else
  {
	...
	bw_xbase1.set_c_win_scrnvar(p_wptr,bw_win1.get_1fldlen(i),0);
...
int bw_fldstru::w_get_tab_var(char *p_fn,int p_fn_size,int p_fptr,char *p_ca,int p_ca_size)
{
  int  i,j,k;

  p_ca[0]=0;

  i=w_find_fldnum(p_fn,p_fn_size,p_fptr);
  if (i!=0) return(1);
  j=w_find_fldlen(p_fn,p_fn_size,p_fptr);
  k=w_find_fldposi(p_fn,p_fn_size,p_fptr);
...
int bw_fldstru::w_cmp_tab_var(char *p_fn,int p_fn_size,int p_fptr,char *p_ca,int p_ca_size)
{
  int  exist;
  int  i,j,k,l;
  ...
  if (i!=0) return(1);
  j=strlen(p_ca);
  l=w_find_fldlen(p_fn,p_fn_size,p_fptr);
  k=w_find_fldposi(p_fn,p_fn_size,p_fptr);
  if (j!=l) return(1);
...
int bw_fldstru::w_sav_tab_var(char *p_fn,int p_fn_size,int p_fptr,char *p_ca,int p_ca_size)
{
  int  i,j,k,l;
  char c1;
...
int bw_fldstru::w_fetch_rec(int p_fptr,int p_wptr,int seri)
{
  int  i,j,l;
  int  r1,r2;
  long recno;

  if (bw_xbase1.get_win_cur_seri(p_fptr,p_wptr)==0)
  {
    bw_xbase1.set_win_cur_seri(p_fptr,p_wptr,1);
    bw_xbase1.set_win_recno(p_fptr,p_wptr,0,0);

    recno=0;

    if (bw_xbase1.get_win_lnktab(p_fptr)==0)
    {
      recno=bw_mbase1.w_down_rec(p_fptr,p_wptr,recno);
    }
    else
    {
      while (1)
      {
        recno=bw_mbase1.w_down_rec(p_fptr,p_wptr,recno);
	j=bw_link1.w_link_number(p_wptr,p_fptr);
	if (bw_link1.w_link_cond_ok(j,p_wptr,p_fptr,seri,1)==1) break;
	if (bw_xbase1.win_state_dbf_err==1) break;
      }
    }

    bw_xbase1.set_win_recno(p_fptr,p_wptr,0,recno);
  }

  if (bw_xbase1.get_win_rec_cnt(p_fptr)<1)
  {
    bw_xbase1.w_clr_recvar(p_fptr);

    return(1);
  }
/*
  if (bw_xbase1.get_win_cur_seri(p_fptr,p_wptr)==seri)
  {
    recno=bw_xbase1.get_win_recno(p_fptr,p_wptr,0);
    recno--;
    ...
    {
      if (bw_xbase1.get_win_lnktab(p_fptr)>1) recno=0;
    ...
    {
      bw_xbase1.set_win_cur_seri(p_fptr,p_wptr,seri);
      bw_xbase1.set_win_recno(p_fptr,p_wptr,0,recno);
      return(0);
    }
  }
*/
  if (1/*bw_xbase1.get_win_cur_seri(p_fptr,p_wptr)!=seri*/)
  {
    if (bw_xbase1.get_win_lnktab(p_fptr)==0) // not linked table (type 0, not type 1,2,3)
    {
      r1=bw_xbase1.get_win_recno(p_fptr,bw_main1.win_hnd_id,1)-1;
      r2=0;

      //s_tmpn1=bw_win1.w_cnt_winline(p_fptr,p_wptr,LIN_NUM);
      for (i=1;i<=seri;i++)
      {
        r2=bw_mbase1.w_down_rec(p_fptr,bw_main1.win_hnd_id,r1);
        if (bw_xbase1.win_state_dbf_err==1) break;
        r1=r2;
      }
      ...
      bw_xbase1.set_win_cur_seri(p_fptr,bw_main1.win_hnd_id,seri);
      bw_xbase1.set_win_recno(p_fptr,bw_main1.win_hnd_id,0,r2);
    ...
    else  // linked tab
    {
      ...
      if (bw_xbase1.get_win_lnktab(p_fptr)>1) r1=0;
      ...
      for (i=1;i<=seri;i++)
      {
        while (1)
        {
          ...
          l=bw_link1.w_link_number(bw_main1.win_hnd_id,p_fptr);
          r1=r2;
          if (bw_link1.w_link_cond_ok(l,bw_main1.win_hnd_id,p_fptr,seri,1)==1) break;
          if (bw_xbase1.win_state_dbf_err==1) break;
        }
        if (bw_xbase1.win_state_dbf_err==1) break;
      }
...
/*
        while (1)
        {
          r2=bw_mbase1.w_up_rec(p_fptr,bw_main1.win_hnd_id,r1);
          *//*m=bw_buff1.get_linecol(bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pfield);
          if (m>0) n=bw_win1.get_1fldseri(m);
          else n=1;*//*
          l=bw_link1.w_link_number(bw_main1.win_hnd_id,p_fptr);
        ...
        if (bw_xbase1.win_state_dbf_err==1)
        {
          //bw_xbase1.set_win_recno(p_fptr,bw_main1.win_hnd_id,0,1);
          //r2=1;
        }*/
        return(1);
      }
...
int bw_fldstru::w_page_up(int p_fptr,int p_wptr)
{
  int  i,j,k,l;
  int  s_tmpn1;

  if (bw_xbase1.get_win_lnktab(p_fptr)>1) return(0);
  ...
  if (bw_xbase1.get_win_lnktab(p_fptr)==0)
  {
    ...
    for (i=1;i<=s_tmpn1;i++)
    {
      bw_xbase1.set_win_recno(p_fptr,p_wptr,1,bw_mbase1.w_up_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,1)));
      if (bw_xbase1.win_state_dbf_err==1) break;
    }

    w_echo_winrec(p_wptr);
    w_first_editposi(p_fptr,p_wptr);
    bw_xbase1.set_win_cur_seri(p_fptr,p_wptr,1);
    bw_xbase1.set_win_recno(p_fptr,p_wptr,0,bw_xbase1.get_win_recno(p_fptr,p_wptr,1));
  ...
  else
  {
	...
	if (bw_link1.w_link_cond_ok(l,p_wptr,p_fptr,k,1)==1) break;
	if (bw_xbase1.win_state_dbf_err==1) break;
      }
      if (bw_xbase1.win_state_dbf_err==1) break;
    }
    ...
    if (bw_xbase1.win_state_dbf_err==1)
    {
	bw_xbase1.set_win_recno(p_fptr,p_wptr,1,bw_xbase1.get_win_recno(p_fptr,p_wptr,1)-1);
	while (1)
        {
	  bw_xbase1.set_win_recno(p_fptr,p_wptr,1,bw_mbase1.w_down_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,1)));
	  j=bw_buff1.get_linecol(p_wptr,bw_win1.pline,bw_win1.pfield);
	  if (j>0) k=bw_win1.get_1fldseri(j);
	  else k=1;
	  l=bw_link1.w_link_number(p_wptr,p_fptr);
	...
	if (bw_xbase1.win_state_dbf_err==1) bw_xbase1.set_win_recno(p_fptr,p_wptr,1,1);
    }

    i=bw_link1.w_link_base_num(p_wptr,p_fptr);
    bw_link1.w_link_modi_mark(p_wptr,i,-1,-1);
    bw_link1.w_link_set_continue(p_wptr,p_fptr);
...
int bw_fldstru::w_page_down(int p_fptr,int p_wptr)
{
  int  i,j,k,l;
  int  s_tmpn1,s_tmpn2;
  int  r1,r2;
  ...
  if (bw_xbase1.get_win_lnktab(p_fptr)==0)
  {
    ...
    r1=bw_xbase1.get_win_recno(p_fptr,p_wptr,1)-1; // try if there are n lines in after
    r2=0;

    for (i=1;i<=s_tmpn2;i++)
    {
      r2=bw_mbase1.w_down_rec(p_fptr,p_wptr,r1);
      r1=r2;
      if (bw_xbase1.win_state_dbf_err==1) break;
    }
...
int bw_fldstru::w_first_editposi(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  k=0;
  for (i=1;i<LIN_NUM;i++) /* find first screen field */
  {
    for (j=1;j<COL_NUM;j++)
    {
      l=bw_buff1.get_linecol(p_wptr,i,j);
      if ((l!=0)&&(bw_win1.get_1fldedt(l)==0)&&(bw_win1.get_1fldbase(l)==p_fptr))
      {
	bw_win1.pline=i;
...
int bw_fldstru::w_last_editposi(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  k=0;
...
int bw_fldstru::w_insert_rec(int p_fptr,int p_wptr)
{
  int s_state1;
  int i;
  ...
  s_state1=bw_mbase1.w_ins_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0));
  ...
  if (bw_xbase1.get_win_lnktab(p_fptr)==1)
  {
    if ((bw_xbase1.get_win_carry_on(p_fptr)!=1)||(bw_xbase1.get_win_enseri(p_fptr,p_wptr)<=0))
    {
      bw_link1.w_keep_linkfld(p_wptr,p_fptr,bw_xbase1.win_insert_dir);
    }
...
int bw_fldstru::w_add_rec(int p_fptr,int p_wptr)
{
  int s_state1;
  int i,j,k;
  ...
  s_state1=bw_mbase1.w_add_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0));
  ...
  j=bw_win1.w_cnt_winline(p_fptr,p_wptr,LIN_NUM);
  k=w_last_line(p_fptr,p_wptr);
    
  if ((j>1)&&(k==0)) bw_win1.w_down_field(p_wptr);  // multi line windows and not at last line
  else bw_fldstru1.w_down_line2(p_fptr,p_wptr); // single line window or in last line
...
int bw_fldstru::w_delete_rec(int p_fptr,int p_wptr)
{
  int s_state1;
  int i,j,k;
  long recno;
  ...
  s_state1=bw_mbase1.w_del_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0));
  ...
  if (w_have_next_rec2(p_fptr,p_wptr,recno))
  { 
  }  // adjust cursor
  else
  {
    j=bw_win1.w_cnt_winline(p_fptr,p_wptr,LIN_NUM);
    k=w_first_line(p_fptr,p_wptr);
    
    if ((j>1)&&(k==0)) bw_win1.w_up_field(p_wptr);  // multi line windows and not at first line
    else
    {
      if (w_have_prev_rec2(p_fptr,p_wptr,recno)) bw_fldstru1.w_up_line2(p_fptr,p_wptr); // single line window or in first line
    }
  }
...
int bw_fldstru::w_have_next_rec2(int p_fptr,int p_wptr,int recno)
{
  int i;
  i=bw_mbase1.w_down_rec(p_fptr,p_wptr,recno-1);
  if (bw_xbase1.win_state_dbf_err==0) return(1);
  else return(0);
}


int bw_fldstru::w_have_prev_rec2(int p_fptr,int p_wptr,int recno)
{
  int i;
  i=bw_mbase1.w_up_rec(p_fptr,p_wptr,recno);
  if (bw_xbase1.win_state_dbf_err==0) return(1);
  else return(0);
}

int bw_fldstru::w_rollback_base(int p_fptr,int p_wptr)
{
  int  i,k;
  int  p1;
  char s_fn1[FN_SIZE];
  ...
  if (bw_xbase1.get_win_state_dbf_modi(p_fptr)==0) return(0);

  if (bw_xbase1.get_win_state_in_srch(p_wptr,p_fptr)==1)
  {
    close(bw_xbase1.win_fp2[p_wptr]);
    ...
    bw_xbase1.get_win_basedir(p_fptr,str1,FN_SIZE);

    bw_inkey1.w_set_dir(str1);

    bw_inkey1.w_del_file(s_fn1);

    bw_xbase1.set_win_state_in_srch(p_wptr,p_fptr,0);
  }

  bw_win1.get_win_tabname(1,p_fptr,s_fn3,FN_SIZE);

  bw_xbase1.w_rollback_xbase(/*s_fn3,FN_SIZE,*/bw_xbase1.get_win_base_backup(p_fptr),p_fptr);

  bw_xbase1.set_win_state_dbf_modi(p_fptr,0);
  ...
  for (p1=bw_win1.get_win_ptr_grp_ptr(bw_win1.get_win_ptr_grp_id(p_wptr),0)
      ...
      ;p1++)
  {
    if (bw_xbase1.get_win_state_in_srch(p1,p_fptr)==1)
    {
      close(bw_xbase1.win_fp2[p1]);
      bw_xbase1.set_win_state_in_srch(p1,p_fptr,0);
  ...
  w_first_editposi(p_fptr,p_wptr);
  bw_xbase1.set_win_cur_seri(p_fptr,p_wptr,1);
  bw_xbase1.set_win_recno(p_fptr,p_wptr,0,1);
  bw_xbase1.set_win_recno(p_fptr,p_wptr,1,1);
...
int bw_fldstru::w_commit_base(int p_fptr,int p_wptr)
{
  ...
  bw_xbase1.w_commit_xbase(bw_xbase1.get_win_base_backup(p_fptr),p_fptr);
...
int bw_fldstru::w_link_window(int p_fptr,int p_wptr)
{
  int i,j;

  if (bw_xbase1.get_win_state_in_srch(p_wptr,p_fptr)==1) return(1);

  i=w_is_card(p_fptr,p_wptr);

  j=bw_win1.get_win_ptr_grp_ptr(bw_win1.get_win_ptr_grp_id(p_wptr),0);

  if (i==1) bw_xbase1.set_win_recno(p_fptr,p_wptr,1,bw_xbase1.get_win_recno(p_fptr,j,0));
  else bw_xbase1.set_win_recno(p_fptr,p_wptr,1,bw_xbase1.get_win_recno(p_fptr,j,1));
...
int bw_fldstru::w_is_card(int p_fptr,int p_wptr)  // single record window
{
  int i,j,k,l;
...
int bw_fldstru::w_find_cond(int p_fptr,int p_wptr)
{
  char  sfn1[FN_SIZE];
  char  str1[FN_SIZE];
...
int bw_fldstru::w_find_key(int p_fptr,int p_wptr)
{
  int i,j;

  i=w_find_recno(p_fptr,p_wptr);
  if (i!=0) return(1);

  for (j=0;j<=2;j++) bw_xbase1.set_win_recno(p_fptr,p_wptr,j,1);
  bw_xbase1.set_win_cur_seri(p_fptr,p_wptr,1);
  w_echo_winrec(p_wptr);
  w_first_editposi(p_fptr,p_wptr);
...
int bw_fldstru::w_find_recno(int p_fptr,int p_wptr)
{
  int           i;
  int           exist;
  long          lj;
  ...
  bw_xbase1.win_fp2[p_wptr]=/*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE*/);
  if (bw_xbase1.win_fp2[p_wptr]<0) return(1);

  bw_xbase1.set_win_recno(p_fptr,p_wptr,0,0);
  lj=0;
  while (1)
  {
    bw_xbase1.set_win_recno(p_fptr,p_wptr,0,bw_mbase1.w_down_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0)));
    if (bw_xbase1.win_state_dbf_err!=0) break;
    if (w_find_cond_ok(p_fptr)!=1) continue;

    c1=(unsigned char)(bw_xbase1.get_win_recno(p_fptr,p_wptr,0)/256/256/256);
    c2=(unsigned char)((bw_xbase1.get_win_recno(p_fptr,p_wptr,0)-c1*256*256*256)/256/256);
    c3=(unsigned char)((bw_xbase1.get_win_recno(p_fptr,p_wptr,0)-c1*256*256*256-c2*256*256)/256);
    c4=(unsigned char)(bw_xbase1.get_win_recno(p_fptr,p_wptr,0)-c1*256*256*256-c2*256*256-c3*256);
    ...
    lj++;
    lseek(bw_xbase1.win_fp2[p_wptr],(lj-1)*5,0);
    write(bw_xbase1.win_fp2[p_wptr],str2,7);

  }
  
  bw_xbase1.set_win_state_in_srch(p_wptr,p_fptr,1);

  return(0);
}

int bw_fldstru::w_clr_condition(int p_fptr)
{
  int   i;
  int   zero=0;
  char  str1[SMG_SIZE];
  ...
  for (i=w_find_fldpoin0(p_fptr)
      ;i<w_find_fldpoin1(p_fptr)
      ;i++)
  {
    w_find_fldname(p_fptr,i,str3,SMG_SIZE);
...
int bw_fldstru::w_find_end(int p_fptr,int p_wptr)
{
  int  i;
  char s_fn1[FN_SIZE];
  char str1[FN_SIZE];
...
int bw_fldstru::w_find_cond_ok(int p_fptr)
{
  float f1,f2;
  int   i,j=1,k,l,m,n,o;
  int   exist;
  ...
  for (i=w_find_fldpoin0(p_fptr)
      ...
      ;i++)
  {
    w_find_fldname(p_fptr,i,str7,SMG_SIZE);
    ...
    if ((bw_dialog1.w_mv_exist(str6,SMG_SIZE,zero)==1)||(bw_dialog1.w_mv_exist(str4,SMG_SIZE,zero)==1))
    {
      ...
      w_get_tab_var(str7,SMG_SIZE,p_fptr,str2,SMG_SIZE);

      //MessageBox(0,"cc str2",str2,MB_OK);

      if (w_find_fldtype(str7,SMG_SIZE,p_fptr)=='c')
      {
	...
	else
	{
      ...
      if (w_find_fldtype(str7,SMG_SIZE,p_fptr)=='n')
      {
	...
	while (1)
	{
      ...
      if (w_find_fldtype(str7,SMG_SIZE,p_fptr)=='l')
      {
	if (str1[0]==' ')
	{
          break;
...
//      if (w_find_fldtype(str7,p_fptr)=='m')
//      {
...
int bw_fldstru::w_edit_memo(int p_fptr,int p_wptr,char *p_fldname,int p_fldname_size)
{                           
  char fn1[FN_SIZE];
  char str[SMG_SIZE];
  char str1[SMG_SIZE];
...
//  char stra[SMG_SIZE];
  ...
  if (bw_xbase1.get_win_mem_inuse(p_fptr)==1) return(0);
  ...
  // make file name
  bw_xbase1.get_win_basename(p_fptr,st2,FN_SIZE);
  ...
  //file exist
  bw_xbase1.get_win_basedir(p_fptr,st1,FN_SIZE);
  ...
  if (bw_inkey1.w_find_file(fn1)==0)
  {
    ...
    bw_xbase1.get_win_memno(p_fptr,str9,SMG_SIZE);
...
//    fclose(fh);
    ...
    bw_xbase1.w_find_empty_mfile(str2,FN_SIZE,p_fptr);

    strcpy(str3,str2);
    bw_xbase1.get_win_mem_ext(p_fptr,str8,SMG_SIZE);
    strcat(str3,str8);
...
//      MessageBox(bw_main1.win_hwnd1,str4,str5,MB_OK);
    ...
    if (exist==1)
    {
      ...
      strcpy(str3,str2);
      bw_xbase1.get_win_mem_ext(p_fptr,str8,FN_SIZE);
      strcat(str3,str8);
      bw_inkey1.strtrim(str3,FN_SIZE);
...
//      fclose(fh);
  ...
  if (mark==1)
  {
    ...
    w_sav_tab_var(str6,SMG_SIZE,p_fptr,str9,SMG_SIZE);
  ...
  bw_xbase1.set_win_mem_inuse(p_fptr,1);
  bw_xbase1.set_win_mem_wptr(p_fptr,p_wptr);
  bw_xbase1.set_win_mem_fn(p_fptr,str3,FN_SIZE);
  bw_win1.set_win_memo_basep(p_wptr,p_fptr);
...
//  long memp;
  ...
  p_fptr=bw_win1.get_win_memo_basep(p_wptr);
  bw_xbase1.set_win_mem_inuse(p_fptr,0);

  // save mem ext name
  bw_xbase1.get_win_mem_ext(p_fptr,str0,10);
...
//  MessageBox(bw_main1.win_hwnd1,fn1,"aa",MB_OK);
  ...
  else
  {
    ...
    bw_xbase1.w_find_empty_mfile(str1,FN_SIZE,p_fptr);

    bw_xbase1.get_win_mem_ext(p_fptr,str7,10);
    strcpy(str2,str1);
    strcat(str2,str7);
...
//    MessageBox(bw_main1.win_hwnd1,"copy & open ok","bb",MB_OK);
    ...
    bw_xbase1.get_win_memno(p_fptr,str8,FN_SIZE);
    strcat(str4,str8);
...
int bw_fldstru::w_edit_memo_add(int p_fptr,int p_wptr,char *p_fldname,int p_fldname_size)
{
/*
  int  i,j,k,l,m,n;
  char s_tc0[SMG_SIZE];
...
//  FILE *fh1;
  ...
  bw_xbase1.w_find_empty_mfile(s_tc7,SMG_SIZE,p_fptr);
  ...
  bw_xbase1.get_win_memno(p_fptr,str8,SMG_SIZE);
  ...
  if (bw_inkey1.w_find_file(fn1)==1)
  {
    ...
    if (exist!=1)
    {
      ...
      bw_xbase1.set_win_memno(p_fptr,str8,strlen(str8)+1);
    }
    else  bw_xbase1.set_win_memno(p_fptr,str8,strlen(str8)+1);
  }
  else return(1);
  ...
  if (mark==1)
  {
    ...
    w_sav_tab_var(str6,SMG_SIZE,p_fptr,str8,SMG_SIZE);
    bw_mbase1.w_save_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,0));
    bw_xbase1.set_win_state_dbf_modi(p_fptr,1);
  ...
  bw_xbase1.set_win_mem_inuse(p_fptr,1);
  bw_xbase1.set_win_mem_fn(p_fptr,s_tc8,SMG_SIZE);
  bw_win1.set_win_memo_basep(p_wptr,p_fptr);
...
int bw_fldstru::get_t2_fldpoin(int fptr,int p01)
{
  if ((fptr<0)||(fptr>=STRU_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(t2_fldpoin[fptr][p01]);
}
...
int bw_fldstru::set_t2_fldpoin(int fptr,int p01,int val)
{
  if ((fptr<0)||(fptr>=STRU_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  t2_fldpoin[fptr][p01]=val;
  return(0);
}
...
int bw_fldstru::w_down_line(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  int s_tmpn1;
  int r1,r2;

  i=w_last_line(p_fptr,p_wptr);
  if (i==0) { /*MessageBox(0,"aa","aa",MB_OK);*/ bw_win1.w_down_field(p_wptr);}
  else
  {
    j=w_be_linked(p_fptr,p_wptr);  // type 0,1 not link,type 2,3 linked
    if (j==1) { /*MessageBox(0,"bb","bb",MB_OK);*/ bw_win1.w_down_field(p_wptr);}
    else
    {
      k=w_have_next_rec(p_fptr,p_wptr);
...
int bw_fldstru::w_down_line2(int p_fptr,int p_wptr) // button command
{
  int i,j,k,l;
  int r1,r2;
...
int bw_fldstru::w_last_line(int p_fptr,int p_wptr)
{
  int  i,j,k,l,m;
  int  exist;
  char str[SMG_SIZE];
...
/*
      if (bw_buff1.get_linecol(p_wptr,i,j)>0)
      {
	if ((bw_win1.get_1fldedt(bw_buff1.get_linecol(p_wptr,i,j))==0)&&
            (bw_win1.get_1fldbase(bw_buff1.get_linecol(p_wptr,i,j))==p_fptr))
	{
	  exist=1;
	  break;
	}
...
int bw_fldstru::w_be_linked(int p_fptr,int p_wptr)
{
  if (bw_xbase1.get_win_lnktab(p_fptr)>1) return(1);
  else return(0);
}

int bw_fldstru::w_have_next_rec(int p_fptr,int p_wptr)
{
  int  i,j,k,l;
  int  s_tmpn1;
  int  r1,r2;

  r1=bw_xbase1.get_win_recno(p_fptr,p_wptr,1)-1;
  r2=0;

  if (bw_xbase1.get_win_lnktab(p_fptr)==0)
  {
    s_tmpn1=1+bw_win1.w_cnt_winline(p_fptr,p_wptr,LIN_NUM);
    for (i=1;i<=s_tmpn1;i++)
    {
      r2=bw_mbase1.w_down_rec(p_fptr,p_wptr,r1);
      r1=r2;
...
int bw_fldstru::w_up_line(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  int r1,r2;
  int s_tmpn1;

  i=w_first_line(p_fptr,p_wptr);
  if (i==0) { /*MessageBox(0,"aa","aa",MB_OK);*/ bw_win1.w_up_field(p_wptr); }
  else
  {
    j=w_be_linked(p_fptr,p_wptr);
    if (j==1) { /*MessageBox(0,"bb","bb",MB_OK);*/ bw_win1.w_up_field(p_wptr); }
    else
    {
      k=w_have_prev_rec(p_fptr,p_wptr);
...
int bw_fldstru::w_up_line2(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  int r1,r2;
...
int bw_fldstru::w_first_line(int p_fptr,int p_wptr)
{
  int  i,j,k,l,m;
  int  exist;
  char str[SMG_SIZE];
...
int bw_fldstru::w_have_prev_rec(int p_fptr,int p_wptr)
{
  int i,j,k,l;
  int s_tmpn1;
  int r1,r2;
  ...
  if (bw_xbase1.get_win_lnktab(p_fptr)==0)
  {
    ...
    for (i=1;i<=s_tmpn1;i++)
    {
      bw_mbase1.w_up_rec(p_fptr,p_wptr,bw_xbase1.get_win_recno(p_fptr,p_wptr,1));
      if (bw_xbase1.win_state_dbf_err==1) return(0);
    }
    return(1);
  }
  ...
  {
      r1=bw_xbase1.get_win_recno(p_fptr,p_wptr,1);
      r2=0;

      while (1)
      {
	r2=bw_mbase1.w_up_rec(p_fptr,p_wptr,r1);
	j=bw_buff1.get_linecol(p_wptr,bw_win1.pline,bw_win1.pfield);
	if (j>0) k=bw_win1.get_1fldseri(j);
	else k=1;
	l=bw_link1.w_link_number(p_wptr,p_fptr);
