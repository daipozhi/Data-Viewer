...
int bw_choose::w_read_cdc(char *p_fn1,int p_fn1_size,int p_wptr)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
  int  ptr1;
...
int bw_choose::w_save_choose(int p_wptr)
{
  ...
  //{
    for (i=0;i<win_choose_ptr1;i++)
    {
      //if (bw_choose1.get_win_choose(i,3)==p_wptr)
      //{
...
int bw_switch::w_read_sdc(char *p_fn1,int p_fn1_size,int p_wptr)
{
  int  i,j,k,l,o;
  int  p1,p2,p3,p4,p5,p6;
  int  ptr1;
...
int bw_dialog::w_read_ddc(char *p_fn1,int p_fn1_size,int p_wptr)
{
  int  i,j,k,l;
  ...
  //win_mv_ptr1=ptr1;  //bug , can't modify win_mv_ptr1 
...
int bw_dialog::w_echo_winrec_read(int p_fptr,int p_wptr,int mark)
{
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	bw_win1.get_1fldname(k,str,FLD_NAME_SIZE);
	...
	if (str[0]=='*')
	{
	  ...
	  if (m== 8)  bw_inkey1.long2str(bw_xbase1.get_win_recno(p_fptr,p_wptr,0),s_tmps2,SMG_SIZE);
	  if (m== 9)  bw_inkey1.long2str(bw_xbase1.get_win_rec_cnt(p_fptr),s_tmps2,SMG_SIZE);
...
*/
  ...
  sprintf(str1,"%4d",1900+tm->tm_year);
  p_s1[0]=str1[2];
  p_s1[1]=str1[3];
  p_s1[2]=0;
  return(0);
...
*/
  ...
  gettimeofday(&tv,&tz);
  tm=localtime(&(tv.tv_sec));
  sprintf(str1,"%4d-%2d-%2d",1900+tm->tm_year,1+tm->tm_mon,tm->tm_mday);
...
*/
  ...
  gettimeofday(&tv,&tz);
  tm=localtime(&(tv.tv_sec));
  sprintf(str1,"%2d:%2d:%2d",tm->tm_hour,tm->tm_min,tm->tm_sec);
