...
int bw_win::w_mouse_infield(int p_wptr,int y,int x)
{
  ...
  for (m=1;m<COL_NUM;m++)
  {
    n=bw_buff1.get_linecol(p_wptr,y,m);

    if ((n>0)&&(get_1fldedt(n)==0))
    {
      if ((m<=x)&&(x<=m+get_1fldlen(n)-1))
      {
	get_1fldname(n,str1,SMG_SIZE);
...
int bw_win::tst_win1()
{
  ...
  for (i=1;i<60;i++)
  {
    get_1fldname(i,str2,FLD_NAME_SIZE);
  ...
  for (l=1;l<WIN_NUM;l++)
  {
	...
	if (bw_buff1.get_linecol(l,i,j)!=0)
	{
	  ...
	  get_1fldname(m,str2,FLD_NAME_SIZE);
...
int bw_win::tst_win2()
{
  ...
  for (i=0;i<WIN_NUM;i++)
  {
    sprintf(str1,"20=%d,21=%d,4=%d,5=%d,60=%d,61=%d,62=%d,90=%d,91=%d,92=%d,93=%d,ptr1=%d,subwub=%d,"
...
int bw_win::tst_win4()
{
  ...
  for (i=1;i<24;i++)
  {
    for (j=1;j<=80;j++)
    {
      str1[j-1]=bw_buff1.get_t_winfile(1,i,j);
    }
    str1[80]=0;
	...
	if ((get_win_ptr_wins_dbf(m,i)==1)&&(bw_xbase1.get_win_mem_inuse(m)==1))
	{
	  for (n=0;n<EDIT_NUM;n++)
	  {
	    if (bw_edit1.get_win_edit_wptr(n)==i)
	    {
	      ...
	      SendMessage(edit_hnd,WM_COMMAND,1803,(LPARAM)str1);
	    }
          }
	}
      }
...
int bw_win::w_close_subwin(int p1)
{
  ...
  for (k=s;k<=r;k++)
  {
    get_1fldname(k,sfn1,FLD_NAME_SIZE);
    set_1fldname(k-q,sfn1,FLD_NAME_SIZE);
...
*/
  ...
  for (k=m;k<=o;k++)
  {
    ...
    set_win_fptr(k-j,get_win_fptr(k));
...
{ 
  ...
  {                                                                                       
	...
	if (p5==1)
	{
	  set_1fldlen(ptr1,j-1-p6);

	  set_win_style(p_pagecnt,1,1);

	  get_1fldname(ptr1,str2,FLD_NAME_SIZE);
	...
	ptr1++;
	if (bw_buff1.get_t_winfile(p_pagecnt,i,j+1)=='#') bw_buff1.set_linecol(p_pagecnt,i,j+1,ptr1);
	else bw_buff1.set_linecol(p_pagecnt,i,j,ptr1);
	p4=0;      
	p5=1;      
	state1=1;
        p6=j;      
	...
	else
	{
	  if (state1==1)
	  {
	    ...
	    if ((p4==0)&&(bw_buff1.get_t_winfile(p_pagecnt,i,j-1)=='%'))
	    {
	      set_1fldname1(ptr1,p4,'%');
	      p4++;
	    }
	    set_1fldname1(ptr1,p4+0,bw_buff1.get_t_winfile(p_pagecnt,i,j));
	    set_1fldname1(ptr1,p4+1,0);
	    set_1fldtype(ptr1,'c');
	    p4++;
	  }
	  else
...
/*
  win_tfptr=(FARPROC)MakeProcInstance((FARPROC)w_menu_ci_wp,bw_main1.win_ins);
  for (i=ptr3;i<=get_win_h_w_ptr(get_win_ptr_grp_id(p_pagecnt),5);i++)
  {
    set_win_fptr(i,(FARPROC)GetWindowLong(get_win_hnd(i),GWL_WNDPROC));
    SetWindowLong(get_win_hnd(i),GWL_WNDPROC,(LPARAM)win_tfptr);
  }
*/
...
int bw_win::w_cre_win_edit(int p_pagecnt,int mark) // create edit ci
{
  ...
  //win_tfptr=(FARPROC)MakeProcInstance((FARPROC)w_tab_ci_wp,bw_main1.win_ins);
...
int bw_win::w_field_info3(int w)
{
  ...
  for (i=1;i<LIN_NUM;i++) /* find first screen field */
  {
	...
	get_1fldname(k,str2,FLD_NAME_SIZE);
...
int bw_win::w_down_line(int p_ptr,int p1)                      
{
  ...
  for (i=1;i<LIN_NUM;i++) // find first screen field 
  {
    for (j=1;j<COL_NUM;j++)
    {
      ...
      get_1fldname(n,str2,FLD_NAME_SIZE);
      
      if ((n!=0)&&(str2[0]!='#')&&(str2[0]!='%')&&(str2[0]!='@'))
      {
	if (get_1fldseri(n)==p_ptr+1)
	{
	  pline=i;
          pfield=j;
	  exist=1;
...
int bw_win::w_fetch_fld_style(char *tfn,int tfn_size,char *tn,int tn_size,long *tr,char *fn,int fn_size,int p_wptr)
{
  ...
  for (i=0;i<=j;i++)   /* make str */
  {
    for (k=s_ptr1[i][0];k<=s_ptr1[i][1];k++)
      s_fld[i][k-s_ptr1[i][0]]=tfn[k];
    s_fld[i][s_ptr1[i][1]-s_ptr1[i][0]+1]=0;
  }
...
int bw_win::w_fld_set_recno(long s_fldrecno,int ptr2)
{
...
int bw_win::w_field_info2(int p_wptr)
{
  ...
  for (i=1;i<LIN_NUM;i++)
  {
	...
	if ((str2[0]==':')||(str2[0]=='%')||(str2[0]=='+')||(str2[0]=='-'))
	{
	  ...
	  l=0;
	  for (k=bw_buff1.get_linecol(m,i,j)-1;k>=get_win_ptr_fld_in_win(p_wptr,0);k--)
	  {
	    get_field_tmp(k,str1,FLD_NAME_SIZE);
...
int bw_win::w_adjust_edit(int p_wptr)
{
  ...
  for (i=1;i<LIN_NUM;i++)
  {
    for (j=1;j<COL_NUM;j++)
    {
      ...
      {
        n=bw_buff1.get_linecol(p_wptr,i,j);
...
int bw_win::w_cnt_winline(int basep,int p_wptr,int ln)
{
  int i,j,k,l;
  ...
  for (i=1;i<ln;i++) /* find first screen field */
  {
    for (j=1;j<COL_NUM;j++)
    {
      ...
      {
	if (k<get_1fldseri(l)) k=get_1fldseri(l);
      }
    }
  }
...
int bw_win::tst_hptr3()
{
  ...
  for (i=0;i<WIN_NUM;i++)
  {
    sprintf(str,"hptr30123456=%d,%d,%d,%d,%d,%d,%d,"
			      ,get_win_h_w_ptr(i,0)
			      ,get_win_h_w_ptr(i,1)
			      ,get_win_h_w_ptr(i,2)
			      ,get_win_h_w_ptr(i,3)
...
int bw_win::tst_hptr4()
{
  ...
  for (i=0;i<60;i++)
  {
    sprintf(str,"hptr401234=%d,%d,%d,%d,%d,"
...
int bw_win::get_win_tabname(int p1,int p_fptr,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s1[j+0]=win_tabname[p1][p_fptr][j];
    p_s1[j+1]=0;
  }

  return(0);
}
int bw_win::set_win_tabname(int p1,int p_fptr,char *p_s1,int p_s1_size)
{
  int i,j;
  ...
  win_tabname[p1][p_fptr][0]=0;

  for (j=0;j<i-1;j++)
  {
    win_tabname[p1][p_fptr][j+0]=p_s1[j];
    win_tabname[p1][p_fptr][j+1]=0;
  }
...
int bw_win::get_win_ptr_wins_dbf(int p_fptr,int p_wptr)
{
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  if ((p_fptr<0)||(p_fptr>=STRU_NUM)) return(0);
  return(win_ptr_wins_dbf[p_fptr][p_wptr]);
}
int bw_win::set_win_ptr_wins_dbf(int p_fptr,int p_wptr,int val)
{
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  if ((p_fptr<0)||(p_fptr>=STRU_NUM)) return(0);
  win_ptr_wins_dbf[p_fptr][p_wptr]=val;
  return(0);
}
...
FARPROC bw_win::get_win_fptr(int ptr)
{
  if ((ptr<0)||(ptr>=HND_NUM)) return(0);
  return(win_fptr[ptr]);
}  
int bw_win::set_win_fptr(int ptr,FARPROC fproc)
{
  if ((ptr<0)||(ptr>=HND_NUM)) return(0);
  win_fptr[ptr]=fproc;
  return(0);
