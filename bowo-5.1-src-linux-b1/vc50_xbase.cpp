...
int bw_xbase::w_create_xbase(char *p_fn1,int p_fn1_size,int p_dbptr)
{
  ...
  s_fh1=/*s*/open(p_fn1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  if (s_fh1<0) return(1);

  j=get_win_fld_cnt(p_dbptr/*,0*/);   /* write head line 1 */
  s_tmpn1=(j+1)*32+1;
  c1=s_tmpn1/256;
  c2=s_tmpn1-c1*256;
  s_tmpn1=get_win_len_cnt(p_dbptr/*,1*/);
  s_tmpn1++;
  c3=s_tmpn1/256;
  c4=s_tmpn1-c3*256;
...
int bw_xbase::w_fox_append(int p_dbptr)
{
  ...
  while (1)
  {
	...
	s_str1[j-get_t4_fldposi(i)+1]=0;
      ...
      l=bw_fldstru1.w_find_fldnum(s_str6,SMG_SIZE,p_dbptr);
      if (l!=0) continue;
