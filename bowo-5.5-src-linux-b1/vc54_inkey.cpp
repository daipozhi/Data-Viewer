...
int bw_inkey::strfind(char *p1,char *p2)
{
  ...
  for (i=len2;i>=1;i--) if ((p2[i-1]<0)||(p2[i-1]>' ')) break;
...
*/
int bw_inkey::zero2sp(char *p)  //for int's string
{
  int i,j;
...
int bw_inkey::w_cpy_file(char *fn1,char *fn2)
{
  ...
  fh1=/*s*/open(fn1,O_RDWR/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  if (fh1<0) return(1);

  fh2=/*s*/open(fn2,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  if (fh2<0)
  {
    close(fh1);
    return(1);
