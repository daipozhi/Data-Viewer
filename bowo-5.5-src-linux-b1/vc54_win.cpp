...
{
  set_win_h_w_ptr(p_pagecnt,0,win_h_ci_total_ptr);
  set_win_h_w_ptr(p_pagecnt,5,0);
...
{ 
  ...
  for (i=1;i<LIN_NUM;i++)
  {
    ...
    p4=0;
    p5=0;
    if (p7==0) p7=i;  //corner y
    if (p8==0) p8=p1; //corner x
    ptr3=0;
...
int bw_win::w_cre_ci_button(int p_pagecnt,char *p_s1,int p_s1_size,int p6,int p7,int p8,int p9,char *p_s2,int p_s2_size)
{
  ...
  if (get_win_hnd(win_h_ci_total_ptr)==0) bw_main1.win_cre_ok=0;
