...
int bw_fldstru::w_read_stru(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  while (!feof(fp1))
  {
    ...
    {
      if (get_t2_fldtype(pp1+k)=='m') set_t2_fldlen(pp1+k,254);
      else
      {
	if (get_t2_fldtype(pp1+k)=='b') set_t2_fldlen(pp1+k,254);
...
int bw_fldstru::w_find_fldnum(char *p_fn,int p_fn_size,int p_dbptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldlen(char *p_fn,int p_fn_size,int p_dbptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldtype(char *p_fn,int p_fn_size,int p_dbptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_flddec(char *p_fn,int p_fn_size,int p_dbptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
int bw_fldstru::w_find_fldposi(char *p_fn,int p_fn_size,int p_dbptr)
{
  int  i;
  int  exist;
  char sfn1[SMG_SIZE];
...
//		bw_xbase1.set_win_recno(k,p_wptr,0,s_seri[k]);
	    ...
	    if (s=='m') continue;
            ...
            bw_xbase1.w_fld_cpy(k,s_fldname,SMG_SIZE,str1,SMG_SIZE);  //read from dbf
...
int bw_fldstru::w_tv_get_read_g(char *p_str,int p_str_size,int p_wptr,int l,int c,char comm)  // 'tv'(table variant)
{
  ...
  i=bw_inkey1.str_has_null(p_str,p_str_size);
  if (i!=1) return(0);
...
int bw_fldstru::w_get_tab_var(char *p_fn,int p_fn_size,int p_dbptr,char *p_val,int p_val_size)
{
  int  i,j,k;
  ...
  for (i=1;i<=j;i++)
  {
    if (i-1+1<p_val_size)
    {
      p_val[i-1+0]=bw_xbase1.get_win_recvar(i-1+k);
      p_val[i-1+1]=0;
    }
  }
...
int bw_fldstru::w_cmp_tab_var(char *p_fn,int p_fn_size,int p_dbptr,char *p_val,int p_val_size)
{
  int  exist;
  int  i,j,k,l;
  ...
  for (i=1;i<=l;i++)
  {
    if (i-1<p_val_size)
    {
      if (p_val[i-1]!=bw_xbase1.get_win_recvar(i-1+k))
      {
        exist=1;
	break;
      }
...
int bw_fldstru::w_sav_tab_var(char *p_fn,int p_fn_size,int p_dbptr,char *p_val,int p_val_size)
{
  int  i,j,k,l;
  char c1;
...
int bw_fldstru::w_find_cond_ok(int p_dbptr)
{
      ...
      ;i++)
  {
    ...
    if ((bw_dialog1.w_mv_exist(str6,SMG_SIZE,zero)==1)||(bw_dialog1.w_mv_exist(str4,SMG_SIZE,zero)==1))
    {
      ...
      {
	k=bw_inkey1.str_seek_char_first(str1,SMG_SIZE,';');

	if (str1[k]!=';')
        {
	  if (bw_inkey1.strfind(str2,SMG_SIZE,str1,SMG_SIZE)<0)
	  {
	    j=0;
	    break;
	  }
	...
	else
	{
	  ...
	  while (1)
	  {
            ...
            else
	    {
	      ...
	      if (bw_inkey1.strfind(str2,SMG_SIZE,str3,SMG_SIZE)>=0)
	      {
		m=1;
		break;
              }
