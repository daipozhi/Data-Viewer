...
int bw_chkbox::w_read_chkdc(char *p_fn,int p_fn_size,int p_wptr)
{
  ...
  while (!feof(fp1))
  {
    ...
    for (i=i;i<=strlen(tmps2);i++) if ((tmps2[i]>=0)&&(tmps2[i]<=' ')) break; /* scan for menuname*/
    if (i>strlen(tmps2))
    {
      fclose(fp1);
    ...
    bw_inkey1.cut_string(tmps2,p1+1,p2-p1-1,SMG_SIZE,str2,SMG_SIZE);
    ...
    bw_inkey1.cut_string(tmps2,p5,p6-p5,SMG_SIZE,str2,SMG_SIZE);
...
int bw_dialog::w_echo_winrec_read(int p_dbptr,int p_wptr,int mark)
{
  ...
  for (i=0;i<LIN_NUM;i++)
  {
	...
	if (str[0]=='*')
	{
	  ...
	  if (m== 9)  bw_inkey1.long2str(bw_xbase1.get_win_rec_cntr(p_dbptr),s_tmps2,SMG_SIZE);
