...
int bw_print::w_read_page(char *p_fn,int p_fn_size)
{
  ...
  if (s_fp1==NULL)
  {
    bw_win1.w_atten1(bw_main1.win_hwnd1,str3);
    return(1);
  }
  ...
  for (i=0;i<PAGE_LINE+10;i++)
  {
    ...
    for (j=0;j<PAGE_COLU;j++)
    {
      if (  (j>=strlen(s_tmps2))  ||  ((s_tmps2[j]>=0)&&(s_tmps2[j]<' '))  ) c1=' ';
      else c1=s_tmps2[j];
...
int bw_print::w_out_win_print()
{
  ...
  i=bw_inkey1.str_seek_char_last(str2,SMG_SIZE,'.');
  str2[i]=0;
  strcat(str2,".txt");
  ...
  for (k=0;k<STRU_NUM;k++)
  {
	...
	if ((get_c_t6_fldname(m,0)==':')||(get_c_t6_fldname(m,0)=='%'))
	{
	  ...
	  s=bw_fldstru1.w_find_fldtype(s_fldname,SMG_SIZE,k);
          ...
          bw_xbase1.w_fld_cpy(k,s_fldname,SMG_SIZE,str,SMG_SIZE);  //read from dbf
	  ...
	  if ((s=='c')||(s=='n')||(s=='l'))
	  {
	    ...
	    {
	      if (n+1>(int)strlen(str)) set_t6_tmps2(i,j+n,' ');
	      else  set_t6_tmps2(i,j+n,str[n]);
            }
	  }
	  else
	  {
...
int bw_print::w_print_lnk(int p_wptr) /* echo win rec where modified */
{
  ...
  for (r=0;r<COND_NUM;r++)
  {
	...
	if (get_t6_fldedit(v)==1)
	{
	  ...
	  for (o=0;o<get_t6_fldlen(m);o++)
	  {
	    if (o+1>(int)strlen(str1)) set_t6_tmps2(i,j+o,' ');
	    else  set_t6_tmps2(i,j+o,str1[o]);
          }
...
int bw_print::w_save_pv(int p_wptr,int p_dbptr,int seri,int recn,int posi,int len)  /*save scrn var to arr*/
{
  ...
  for (j=0;j<len;j++) set_win_pv(get_win_pvpoin(i,0)+j,bw_xbase1.get_win_recvar(posi+j));
