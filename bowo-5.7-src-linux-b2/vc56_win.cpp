...
int bw_win::w_login_wwin(int p_pagecntr)
{
  set_win_h_w_ptr(p_pagecntr,0,win_h_ci_total_ptr);
  set_win_h_w_ptr(p_pagecntr,5,0);
...
int bw_win::tst_win7_focus_buf()
{
  ...
  sprintf(str1,"win_focus_ptr1=%d,win_focus_ptr2=%d,",win_focus_ptr1,win_focus_ptr2);
  bw_getread1.deb_record(str1);
  ...
  while(1)
  {
    if (i==win_focus_ptr2) break;

    sprintf(str1,"ptr=%d,win_focus_buf=%d,",i,get_win_focus_buf(i));
    bw_getread1.deb_record(str1);
...
int bw_win::w_close_subwin(int p1)
{
  ...
  {
    w_atten66(bw_main1.get_win_hwnd(p1));
    return(1);
  }
  ...
  if (q>0)
  {
    for (j=i+1;j<WIN_NUM;j++)
    {
      ...
      {
        s=get_win_ptr_fld_in_win(j,0);
        r=get_win_ptr_fld_in_win(j,1);
  ...
  for (k=i+1;k<WIN_NUM;k++)
  {
	...
	set_win_ptr_wins_dbf(o,k-1,get_win_ptr_wins_dbf(o,k));
      ...
      bw_main1.set_win_hwnd(k-1,bw_main1.get_win_hwnd(k));
      
      set_win_is_link(k-1,get_win_is_link(k));
  ...
  j=win_focus_ptr2;    // delete focus which is closing
  ...
  while(1)
  {
    if (j==win_focus_ptr1) break;
    j--;
    if (j<0) j=BW_WIN_FOCUS_NUM-1;
    if (get_win_focus_buf(j)==p10) delete_win_focus(j);

    //sprintf(str1,"step1 j=%d,win_focus_ptr1=%d,win_focus_ptr2=%d,",j,win_focus_ptr1,win_focus_ptr2);
    //bw_getread1.deb_record(str1);
  }

  //bw_getread1.deb_record("step1 ok");

  j=win_focus_ptr2;    // delete focus which is repeated
  k=0;
  ...
  while(1)
  {
    ...
    j--;
    if (j<0) j=BW_WIN_FOCUS_NUM-1;
    if (get_win_focus_buf(j)==k) delete_win_focus(j);
    else k=get_win_focus_buf(j);

    //sprintf(str1,"step2 j=%d,k=%d,win_focus_ptr1=%d,win_focus_ptr2=%d,",j,k,win_focus_ptr1,win_focus_ptr2);
    //bw_getread1.deb_record(str1);
  }

  //bw_getread1.deb_record("step2 ok");

  j=win_focus_ptr2;    // change focus windows id
  ...
  while(1)
  {
    ...
    j--;
    if (j<0) j=BW_WIN_FOCUS_NUM-1;
    if (get_win_focus_buf(j)>p10) set_win_focus_buf(j,get_win_focus_buf(j)-1);
  }

  //bw_getread1.deb_record("step3 ok");

  if (win_focus_ptr2!=win_focus_ptr1)  // set focus
  {
    j=win_focus_ptr2-1;
    if (j<0) j=BW_WIN_FOCUS_NUM-1;
    k=get_win_focus_buf(j);
    SetFocus(get_win_phh(k));
  }
  else if (win_ptr_new_win-1>0) SetFocus(get_win_phh(win_ptr_new_win-1));
...
int bw_win::delete_win_focus(int ptr)
{
  ...
  while(1)
  {
    if (j==win_focus_ptr2) break;
    set_win_focus_buf(i,get_win_focus_buf(j));
    i++;
    if (i>=BW_WIN_FOCUS_NUM) i=0;
    j++;
    if (j>=BW_WIN_FOCUS_NUM) j=0;
  ...
  win_focus_ptr2--;
  if (win_focus_ptr2<0) win_focus_ptr2=BW_WIN_FOCUS_NUM-1;
...
int bw_win::w_read_winfile(char *p_fn,int p_fn_size)
{
  ...
  if (fp1==NULL)
  {
    w_atten1(bw_main1.win_hwnd1,p_fn);
    return(1);
  }
  ...
  for (i=0;i<LIN_NUM+10;i++)
  {
    ...
    {
      cmd=1; // timer
      li=bw_inkey1.str2int(tmps9,FN_SIZE);
      set_win_timer(k,li);
    }
  ...
  if (is_tab==1)  /* it is a table scrn */
  {
	...
	return(1);
      ...
      {
	if (i-p1-5<=0) w_atten6(bw_main1.win_hwnd1);
	...
	return(1);
      ...
      {
	if (i-p1-5<=0) w_atten60(bw_main1.win_hwnd1);
	...
	return(1);
      ...
      {
	if (i-p1-5<=0) w_atten62(bw_main1.win_hwnd1);
...
int bw_win::w_echo_winrec_paint(int p_pagecntr,int mark)  //paint win
{ 
  ...
  if (get_win_ptr_fld_in_win(p_pagecntr,0)==0)
  {
    i=p_pagecntr-1;
    ...
    set_win_ptr_fld_in_win(p_pagecntr,0,get_win_ptr_fld_in_win(i,1)+1);
    ptr1=get_win_ptr_fld_in_win(i,1);
  }
  else ptr1=get_win_ptr_fld_in_win(p_pagecntr,0)-1;

  hdc=GetDC(bw_main1.get_win_hwnd(p_pagecntr));

  if (get_win_ci_mode(p_pagecntr)==0)
  {
  ...
  for (i=0;i<LIN_NUM;i++)
  {
    ...
    {                                                                                    
      ...
      for (j=0;j<COL_NUM;j++)  
      {
        if ((bw_buff1.get_t3_winfile(p_pagecntr,i,j)>' ')||(bw_buff1.get_t3_winfile(p_pagecntr,i,j)<0))
        {
	  p1=j;
	  break;
        }
    ...
    {
      p1=get_win_ci_posi(p_pagecntr,0);
      p2=get_win_ci_posi(p_pagecntr,1);

      j=0;

      for (k=p1;k<=p2;k++)
      {
	if ((bw_buff1.get_t3_winfile(p_pagecntr,i,k)>' ')||(bw_buff1.get_t3_winfile(p_pagecntr,i,k)<0))
	{
	  j=1;
	  break;
	}
    ...
    for (j=p1;j<=p2;j++)  
    {
      if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)=='&')
      {
        ptr1++;
	p4=0;      
	p5=1;      
	...
	if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)<=' ')
	{
	  ...
	  if (mark==1)
	  {
	    if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)<0)
	    {
	      if (ptr3==1)
	      {
		ptr3=0;
		continue;
	      ...
	      {
                ptr3=1;
		str[0]=bw_buff1.get_t3_winfile(p_pagecntr,i,j);
		if (bw_buff1.get_t3_winfile(p_pagecntr,i,j+1)!='&')
                {
		  str[1]=bw_buff1.get_t3_winfile(p_pagecntr,i,j+1);
                  str[2]=0;
                }
                else str[1]=0;
		if (get_win_ci_mode(p_pagecntr)==0)
	...
	else
	{
	  ...
	  {
	    if ((bw_buff1.get_t3_winfile(p_pagecntr,i,j)=='(')
	      &&(bw_buff1.get_t3_winfile(p_pagecntr,i,j-1)!='&'))
	    {
	      state7=1;
              state1=0;
              continue;
            }
	    if ((p4==0)&&(bw_buff1.get_t3_winfile(p_pagecntr,i,j-1)=='%'))
	    {
	      p4++;
	    }
	    p4++;
  ...
  ReleaseDC(bw_main1.get_win_hwnd(p_pagecntr),hdc);

  return(0);
}

int bw_win::w_field_info1(int p_pagecntr,int mark) // get field info
{ 
  ...
  for (i=0;i<WIN_STYLE_NUM;i++) set_win_style(p_pagecntr,i,0);
  ...
  for (i=0;i<LIN_NUM;i++)
  {
    for (j=0;j<COL_NUM;j++)
    {
      bw_buff1.set_t3_linecol(p_pagecntr,i,j,0);
    }
  }
  ...
  {                                                                                       
    ...
    if (ini==0)
    {
      p7=i;
      p8=p1;
      set_win_con(p_pagecntr,1,p7);
      set_win_con(p_pagecntr,0,p8);
      p9=p2-p1+2;
      set_win_con(p_pagecntr,2,p9);
      p10=i;
      ini=1;
    }
	...
	if (p5==1)
	{
	  ...
	  set_win_style(p_pagecntr,1,1);

	  get_t_fldname(ptr1,str2,FLD_NAME_SIZE);

	  if (str2[0]=='(') set_win_style(p_pagecntr,2,1);
	  if (str2[0]=='[') set_win_style(p_pagecntr,3,1);
	  if (str2[0]=='@') set_win_style(p_pagecntr,4,1);
	  if (str2[0]=='#') set_win_style(p_pagecntr,5,1);
	  if (str2[0]=='%') set_win_style(p_pagecntr,6,1);
	  if (str2[0]==':') set_win_style(p_pagecntr,7,1);
	  if (str2[0]=='?') set_win_style(p_pagecntr,8,1);
	  if (str2[0]=='+') set_win_style(p_pagecntr,9,1);
	  if (str2[0]=='-') set_win_style(p_pagecntr,9,1);

	}

	ptr1++;
	if (bw_buff1.get_t3_winfile(p_pagecntr,i,j+1)=='#') bw_buff1.set_t3_linecol(p_pagecntr,i,j+1,ptr1);
	else bw_buff1.set_t3_linecol(p_pagecntr,i,j,ptr1);
	p4=0;      
	p5=1;      
	state1=1;
        p6=j;      
	...
	else
	{
	  if (state1==1)
	  {
	    ...
	    set_c_t_fldname(ptr1,p4+0,bw_buff1.get_t3_winfile(p_pagecntr,i,j));
  ...
  set_win_ptr_fld_in_win(p_pagecntr,1,ptr1);

  // adjust field length for 800*600 256 color mode
  /*
  for (i=get_win_ptr_fld_in_win(p_pagecntr,0);i<=get_win_ptr_fld_in_win(p_pagecntr,1);i++)
  {
...
//int bw_win::w_cre_ci_button(int p_pagecntr,char *p_s1,int p_s1_len,int p6,int p7,int p8,int i)
int bw_win::w_cre_ci_button(int p_pagecntr,char *p_s1,int p_s1_size,int p6,int p7,int p8,int p9,char *p_s2,int p_s2_size)
{
  HWND hnd;
  int  l,j;
  ...
  if (get_win_ci_mode(p_pagecntr)==0)
  {
    hnd=CreateWindow("button",s3
		      ...
		      ,bw_main1.get_win_hwnd(p_pagecntr),(HMENU)win_h_ci_total_ptr
  ...
  set_win_h_w_ptr(p_pagecntr,5,win_h_ci_total_ptr);

  set_win_h_ci_ptr(win_h_ci_total_ptr,0,p_pagecntr);
  ...
  set_win_h_ci_ptr(win_h_ci_total_ptr,4,get_win_h_w_ptr(p_pagecntr,0));
...
//int bw_win::w_cre_ci_edit(int p_pagecntr,int p6,int p7,int p8,int i)
int bw_win::w_cre_ci_edit(int p_pagecntr,int p6,int p7,int p8,int p9)
{
  HWND hnd;
...
//int bw_win::w_cre_ci_radio(int p_pagecntr,int p6,int p7,int p8,int i)
int bw_win::w_cre_ci_radio(int p_pagecntr,int p6,int p7,int p8,int p9,char *p_s1,int p_s1_size)
{
  HWND hnd;
  char str1[SMG_SIZE];
...
//int bw_win::w_cre_ci_check(int p_pagecntr,int p6,int p7,int p8,int i)
int bw_win::w_cre_ci_check(int p_pagecntr,int p6,int p7,int p8,int p9,char *p_s1,int p_s1_size)
{
  HWND hnd;
  char str1[SMG_SIZE];
...
int bw_win::w_cre_win_button(int p_pagecntr,int mark) // create button ci
{ 
  int  p1,p2,p4,p5,p6,p7,p8,p9,p10,p10a;
  int  state1;
  int  state7;
	...
	if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)<=' ')
	{
	  ...
	  if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)!=' ')
	  {
	    if (p5==1)
	    {
		  ...
		  {
		    if (bw_buff1.get_t3_winfile(p_pagecntr,i,t1)=='(') break;
                  }

		  for (t2=p6;t2<COL_NUM;t2++)
		  {
		    if (bw_buff1.get_t3_winfile(p_pagecntr,i,t2)==')') break;
		  }
		  ...
		  for (l=t1+1;l<t2;l++)
		  {
		    str1[l-t1-1]=bw_buff1.get_t3_winfile(p_pagecntr,i,l);
		    str1[l-t1]=0;
                  }

		  w_cre_ci_button(p_pagecntr,str1,SMG_SIZE,p6,p7,p8,i,str2,strlen(str2)+1);
...
/*
  ...
  for (i=ptr3;i<=get_win_h_w_ptr(p_pagecntr,5);i++)
  {
...
int bw_win::w_cre_win_edit(int p_pagecntr,int mark) // create edit ci
{
  ...
  {                                                                                       
	...
	if (p5==1)
	{
	  ...
	  else
	  {
	    ...
	    {
	      w_cre_ci_edit(p_pagecntr,p6,p7,p8,i);
...
/*
  ...
  for (i=hptr1;i<=get_win_h_w_ptr(p_pagecntr,5);i++)
  {
    set_win_proc(i,(FARPROC)GetWindowLong(get_win_hnd(i),GWL_WNDPROC));
    SetWindowLong(get_win_hnd(i),GWL_WNDPROC,(LPARAM)win_tproc);
  }
...
int bw_win::w_cre_win_radio(int p_pagecntr,int mark) // create radio ci
{
  int  p1,p2,p4,p5,p6,p7,p8;
  int  state1;
  int  i,j;
	  ...
	  if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(str2,FLD_NAME_SIZE)==1)
		{
		  w_cre_ci_radio(p_pagecntr,p6,p7,p8,i,str2,strlen(str2)+1);
...
int bw_win::w_cre_win_check(int p_pagecntr,int mark) // create chkbox ci
{ 
  int  p1,p2,p4,p5,p6,p7,p8;
  int  state1;
  int  i,j;
	  ...
	  if (bw_buff1.get_t3_winfile(p_pagecntr,i,j)!=' ')
	  {
	    if (p5==1)
	    {
	      ...
	      else
	      {
	        if (bw_inkey1.strempty(str2,FLD_NAME_SIZE)==1)
		{
		  w_cre_ci_check(p_pagecntr,p6,p7,p8,i,str2,strlen(str2)+1);
...
int bw_win::w_find_hnd(int p1,int p2,int p3)
{
  ...
  for (i=0;i<win_h_ci_total_ptr;i++)
  {
...
int bw_win::w_field_type(char *p_str)
{
  ...
  if (p_str[0]=='%') return(6);  /* readonly database var */
  if (p_str[0]==':') return(7);  /* editable database var */
  if (p_str[0]=='?') return(8);  /* memory var */
  if (p_str[0]=='+') return(9);  /* editable database ci */ // ci : control item
  if (p_str[0]=='-') return(10); /* editable var      ci */ // ci : control item
  return(0);
}
...
int bw_win::w_find_base_name(char *p_tab,int p_tab_size,int p_wptr)
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    if ((get_win_ptr_wins_dbf(i,p_wptr)==1)&&(strcmp(stn,p_tab)==0))
    {
      j=1;
      break;
    }
...
int bw_win::w_atten1(HWND hwnd,char *p_fn1)
{
  ...
  if (bw_main1.win_lang==1) MessageBox(hwnd,str1,"ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,str2,"错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten2(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error in table structure.","ERROR MESSGAE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"表格结构错误","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten3(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Windows too much.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"窗口数量太多","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten4(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Screen format define error,table name need ','.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Screen format define error,structure name too long.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Screen format define error,need structure name.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when insert record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when add record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when delete record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten8(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) i=MessageBox(hwnd,s3,"ERROR MESSAGE",MB_OKCANCEL|MB_ICONQUESTION);
...
int bw_win::w_atten9(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) i=MessageBox(hwnd,s3,"INFORMATION BOX",MB_YESNO|MB_ICONQUESTION);
  else i=MessageBox(hwnd
            ,s2 
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"const LINK_CHAR too small.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"常量LINK_CHAR太小.","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten11(HWND hwnd)
{      
  if (bw_main1.win_lang==1) MessageBox(hwnd,"const WIN_VAR_NUM too small.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"常量WIN_VAR_NUM太小."
	    ,"错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten12(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"const VAR_IN_SCRN too small.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten13(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) MessageBox(hwnd,s3,"ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten15(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) i=MessageBox(hwnd,s3,"ERROR MESSAGE",MB_RETRYCANCEL|MB_ICONQUESTION);
...
int bw_win::w_atten16(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) i=MessageBox(hwnd,s3,"ERROR MESSAGE",MB_YESNO|MB_ICONQUESTION);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Too much edit windows.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"编辑窗口太多","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten22(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Database buffer number is too small.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten28(HWND hwnd,char *s1)
{
  ...
  if (bw_main1.win_lang==1) MessageBox(hwnd,s3,"EEROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,s2,"错误信息",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten39(HWND hwnd)
{
  ...
  if (bw_main1.win_lang==1) i=MessageBox(hwnd,s3,"INFORMATION BOX",MB_YESNOCANCEL|MB_ICONQUESTION);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Only support XBASE,please choose database type.","INFORMATION BOX",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Please clear if to backup database defore open it.","INFORMATION BOX",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten44(HWND hwnd)
{
  ...
  if (bw_main1.win_lang==1) MessageBox(hwnd,s3,"INFORMATION BOX",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There are not so much records.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"数据库中没有这么多记录!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten50(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Record number can not little then zero.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"记录号不能为零或负!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten51(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when read database record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"读数据库记录出错!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten52(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Record number is not correct,can not delete record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"记录号不正确，删除记录出错!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten53(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Record number is not correct,can not locate record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"记录号不正确，定位出错,请规定记录号!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten54(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Record number is not correct,can not display record.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"记录号不正确，显示记录出错!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten55(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when commit data.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"数据库提交出错!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten56(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Error when rollback data.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::w_atten57(HWND hwnd,int l)
{
  ...
  if (bw_main1.win_lang==1) MessageBox(hwnd,str3,"ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,str2,"错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten58(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There is not enough data buffer.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
  else MessageBox(hwnd,"没有足够的数据区分配!","错误信息",MB_OK|MB_ICONASTERISK);
  return(0);
}
int bw_win::w_atten59(HWND hwnd)
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There is error in screen format,type name is too long.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There is error in screen format,lack type name.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There is error in screen format,backup type name is too long.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"There is error int screen format,lack backup type name.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Data buffer is already in use.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"The change in text can not be undo.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"You must answer 'save or not' messagebox.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"Sub windows too much.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
{
  if (bw_main1.win_lang==1) MessageBox(hwnd,"const VAL_NUM too small.","ERROR MESSAGE",MB_OK|MB_ICONASTERISK);
...
int bw_win::get_win_timer(int p_wptr)
{
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  return(win_timer[p_wptr]);
}
int bw_win::set_win_timer(int p_wptr,int val)
{
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
...
int bw_win::get_win_focus_buf(int ptr)
{
  if ((ptr<0)||(ptr>=BW_WIN_FOCUS_NUM)) return(0);
  return(win_focus_buf[ptr]);
}

int bw_win::set_win_focus_buf(int ptr,int val)
{
  if ((ptr<0)||(ptr>=BW_WIN_FOCUS_NUM)) return(0);
  win_focus_buf[ptr]=val;
  return(0);
}

int bw_win::get_win_created(int ptr)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  return(win_created[ptr]);
}

int bw_win::set_win_created(int ptr,int val)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  win_created[ptr]=val;
  return(0);
...
int bw_win::get_win_painted(int ptr)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  return(win_painted[ptr]);
}

int bw_win::set_win_painted(int ptr,int val)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  win_painted[ptr]=val;
  return(0);
...
int bw_win::get_win_focused(int ptr)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  return(win_focused[ptr]);
}

int bw_win::set_win_focused(int ptr,int val)
{
  if ((ptr<0)||(ptr>=WIN_NUM)) return(0);
  win_focused[ptr]=val;
  return(0);
...
int bw_win::get_win_focused_wp(int ptr)
{
    if ((ptr < 0) || (ptr >= WIN_NUM)) return(0);
    return(win_focused_wp[ptr]);
}

int bw_win::set_win_focused_wp(int ptr, int val)
{
    if ((ptr < 0) || (ptr >= WIN_NUM)) return(0);
    win_focused_wp[ptr] = val;
    return(0);
}
