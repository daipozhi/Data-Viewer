...
int bw_xbase::tst_xbase1()
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    sprintf(str,"ptr=%d,recno012=%ld,%ld,%ld,%ld,%ld,%ld,cuseri12345=%d,%d,%d,%d,%d,rcntr=%ld,lcntr-01=%d,%d,ext=%s,lnktab=%d,"
	       ...
	       ,get_win_rec_cntr(i)
	       ,get_win_fld_cntr(i/*,0*/)
	       ,get_win_len_cntr(i/*,1*/)
	       ,sfn
	       ,get_win_lnktab(i));
...
{         
  ...
  set_win_rec_cntr(p_dbptr,0);  // total record number
  set_win_fld_cntr(p_dbptr,0);  // field number
  set_win_len_cntr(p_dbptr,0);  // record length
...
int bw_xbase::w_open_xbase(char *p_fn,int p_fn_size,int backup,int p_dbptr)
{
  ...
  while(1)
  {
    win_fh[p_dbptr]=/*s*/open(s_fn1,O_RDWR/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
    if (win_fh[p_dbptr]>=0) break;            /* open success */
    else if (bw_inkey1.w_find_file(s_fn1,FN_SIZE)==1) // open error , file exist
    {
      s_state1=bw_win1.w_atten8(bw_main1.win_hwnd1,s_fn1);   /* open error */
      if (s_state1==1) continue;          /* retry      */
      else return(1);                     /* cancel     */
    }
    else                                
    {
      s_state1=bw_win1.w_atten9a(bw_main1.win_hwnd1,s_fn1); // dbf file not exist , create ? yes no cancel 
  ...
  {
    s_state2=bw_win1.w_atten18(bw_main1.win_hwnd1,s_fn1);  // if to update structure
    ...
    else			         // update structure
    {
      ...
      win_fh[p_dbptr] = /*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY, SH_DENYNO*/, S_IREAD|S_IWRITE);
  ...
  if (backup==1)
  {
    ...
    s_state1=bw_inkey1.w_cpy_file(s_fn1,FN_SIZE,s_fn2,FN_SIZE);
    if (s_state1==1) return(1);

    close(win_fh[p_dbptr]);

    win_fh[p_dbptr]=/*s*/open(s_fn2,O_RDWR/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  ...
  set_win_basename(p_dbptr,p_fn,p_fn_size);
...
int bw_xbase::w_create_xbase(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  j=get_win_fld_cntr(p_dbptr);   /* write head line 1 */
  s_tmpn1=(j+1)*32+1;
  c1=s_tmpn1/256;
  c2=s_tmpn1-c1*256;
  s_tmpn1=get_win_len_cntr(p_dbptr);
  s_tmpn1++;
  c3=s_tmpn1/256;
  c4=s_tmpn1-c3*256;
...
int bw_xbase::w_compare_xbase(int p_fh,int p_dbptr)
{
  ...
  uc3=str1[6];
  uc4=str1[7];
  set_win_rec_cntr(p_dbptr,uc4*256*256*256+uc3*256*256+uc2*256+uc1);
  ...
  for (k=bw_fldstru1.get_t2_fldpoin(p_dbptr,0);k<bw_fldstru1.get_t2_fldpoin(p_dbptr,1);k++)
  {
    ...
    for (i=0;i<11;i++) fldname[i]=str1[i];
...
int bw_xbase::w_read_xbase_rec(int recno,int p_dbptr)
{
  ...
  if (s_state1!=0)
  {
    for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar(i,' ');
    return(1);
  }

  j=read_win_recvar(win_fh[p_dbptr],get_win_len_cntr(p_dbptr)+1);

  if (/*eof(win_fh[p_dbptr])&&*/(j<get_win_len_cntr(p_dbptr)+1))
  {
    for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar(i,' ');
    return(1);
  }
...
int bw_xbase::w_save_xbase_rec(int recno,int p_dbptr)
{
  ...
  write_win_recvar(win_fh[p_dbptr],get_win_len_cntr(p_dbptr)+1);
...
int bw_xbase::w_loc_xbase_rec(int recno,int p_dbptr)
{
  ...
  if (recno<=0) return(1);
  //if (recno>get_win_rec_cntr(p_dbptr)) return(1);  				//get_win_rec_cntr(p_dbptr) total record number

  i=(get_win_fld_cntr(p_dbptr)+1)*32+1+(recno-1)*(get_win_len_cntr(p_dbptr)+1); //get_win_fld_cntr(p_dbptr) field number,
                                                                              //get_win_len_cntr(p_dbptr) record length
...
int bw_xbase::w_up_xbase_rec(int recno,int p_dbptr)
{
  ...
  if (recno1>get_win_rec_cntr(p_dbptr)+1) recno1=get_win_rec_cntr(p_dbptr)+1;
...
int bw_xbase::w_down_xbase_rec(int recno,int p_dbptr)
{
  ...
  while (1)
  {
    if (recno1<get_win_rec_cntr(p_dbptr)) recno1++;
    else
    {
      if (w_xbase_rec_del()==1)
      {
...
int bw_xbase::w_insert_xbase_rec(int recno,int p_dbptr,int aft)
{
  ...
  for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar2(i,0,get_win_recvar(i));  // current record copy to buffer0
  ...
  else
  {
    ...
    set_win_rec_cntr(p_dbptr,get_win_rec_cntr(p_dbptr)+1);
    w_set_xbase_recno(get_win_rec_cntr(p_dbptr),p_dbptr);
    return(0);
  }
  ...
  while (1)
  {
    ...
    for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar2(i,1,get_win_recvar(i)); // put current rec to buffer1
    for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar(i,get_win_recvar2(i,0)); // put buffer0(prev rec) to current rec
    ...
    for (i=0;i<=get_win_len_cntr(p_dbptr);i++) set_win_recvar2(i,0,get_win_recvar2(i,1)); // put buffer1 to buffer0
  }
}
...
int bw_xbase::w_set_xbase_recno(int recno,int p_dbptr)
{
  ...
  set_win_rec_cntr(p_dbptr,recno);
...
int bw_xbase::w_commit_xbase(int p_wptr,int backup,int p_dbptr)
{
  ...
  while(1)
  {
    ...
    {
      k=bw_win1.w_atten8(bw_main1.get_win_hwnd(p_wptr),s_fn1);     /* delete error */
  ...
  if (l<200)  // if deleted record's number is 200 or more than pack
  {
    ...
    while(1)
    {
      ...
      {
        k=bw_win1.w_atten8(bw_main1.get_win_hwnd(p_wptr),s_fn1);     /* copy error */
        if (k==1) continue;            /* retry      */
        else  return(1);               /* cancel     */
      }
    }
...
int bw_xbase::w_rollback_xbase(int p_wptr,int backup,int p_dbptr)
{
  ...
  while(1)
  {
    ...
    {
      k=bw_win1.w_atten8(bw_main1.get_win_hwnd(p_wptr),s_fn2);   /* delete error */
      if (k==1) continue;          /* retry        */
      else return(1);              /* cancel       */
    }
  }
  ...
  while(1)
  {
    ...
    {
      k=bw_win1.w_atten8(bw_main1.get_win_hwnd(p_wptr),s_fn2);   /* copy error */
      if (k==1) continue;          /* retry      */
      else return(1);              /* cancel     */
    }
  }
  ...
  set_win_rec_cntr(p_dbptr,w_read_recno(win_fh[p_dbptr]));
...
int bw_xbase::w_append_from_sdf(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  /* find max recno */
  recno=get_win_rec_cntr(p_dbptr);
...
int bw_xbase::w_copy_to_sdf(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  while (1)
  {
    ...
    set_win_recvar(get_win_len_cntr(p_dbptr)+1,'\n');
    set_win_recvar(get_win_len_cntr(p_dbptr)+2,0);
...
int bw_xbase::w_find_empty_file(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  strcpy(s_str2,p_fn);
  s_str1[0]='$';
  k=bw_inkey1.str_seek_char_last(s_str2,FN_SIZE,'.');
  ...
  for (i=1;i<=36;i++)
  {
    ...
    for (j=1;j<=36;j++)
    {
      ...
      l=bw_inkey1.w_find_file(s_str2,FN_SIZE);
      if (l==0) break;
    }
    if (l==0) break;
  }
...
int bw_xbase::w_close_xbase(int p_wptr,int p_dbptr)
{
      ...
      (get_win_base_backup(p_dbptr)==1))
  {
    ...
    i=bw_win1.w_atten9(bw_main1.get_win_hwnd(p_wptr),s_fn1); // if to save
...
int bw_xbase::w_fox_use(char *p_fn,int p_fn_size)
{
  ...
  win_use_fh=/*s*/open(p_fn,O_RDWR/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
  ...
  while (1)
  {
    ...
    for (i=0;i<11;i++) fldname[i]=s_tmps1[i];
    fldtype=s_tmps1[11];
...
int bw_xbase::w_fox_append(int p_dbptr)
{
  ...
  while (1)
  {
	...
	s_str1[j-get_t4_fldposi(i)+1]=0;
      ...
      bw_inkey1.lowers(s_str6,SMG_SIZE);
	...
	if (bw_fldstru1.w_find_fldtype(s_str6,SMG_SIZE,p_dbptr)=='n')
	{
      ...
      w_fld_sav(p_dbptr,s_str6,SMG_SIZE,s_str4,SMG_SIZE);
    }
    ...
    w_append_xbase_rec(get_win_rec_cntr(p_dbptr),p_dbptr);
...
int bw_xbase::get_win_recno(int p_dbptr,int p_wptr,int p02)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
...
int bw_xbase::set_win_recno(int p_dbptr,int p_wptr,int p02,int val)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  if ((p_wptr<0)||(p_wptr>=WIN_NUM)) return(0);
  if ((p02<0)||(p02>2)) return(0);
...
int bw_xbase::get_win_rec_cntr(int p_dbptr)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  return(win_rec_cntr[p_dbptr]);
}
int bw_xbase::set_win_rec_cntr(int p_dbptr,int val)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  win_rec_cntr[p_dbptr]=val;
  return(0);
}

int bw_xbase::get_win_fld_cntr(int p_dbptr)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  return(win_fld_cntr[p_dbptr]);
}
int bw_xbase::set_win_fld_cntr(int p_dbptr,int val)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  win_fld_cntr[p_dbptr]=val;
  return(0);
}

int bw_xbase::get_win_len_cntr(int p_dbptr)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  return(win_len_cntr[p_dbptr]);
}
int bw_xbase::set_win_len_cntr(int p_dbptr,int val)
{
  if ((p_dbptr<0)||(p_dbptr>=STRU_NUM)) return(0);
  win_len_cntr[p_dbptr]=val;
  return(0);
}
...
int bw_xbase::read_win_recvar(int fh,int len)
{
  int rl;
  if (fh<0) return(0);
...
int bw_xbase::write_win_recvar(int fh,int len)
{
  int wl;
  if (fh<0) return(0);
  if ((len<0)||(len>REC_SIZE)) return(0);
...
int bw_xbase::fgets_win_recvar(FILE *fh,int len)
{
  if (fh==NULL) return(0);
  if ((len<0)||(len>REC_SIZE)) return(0);
...
int bw_xbase::get_t4_recnumb(int p03)
{
  if ((p03<0)||(p03>3)) return(0);
  return(t4_recnumb[p03]);
}
int bw_xbase::set_t4_recnumb(int p03,int val)
{
  if ((p03<0)||(p03>3)) return(0);
  t4_recnumb[p03]=val;
  return(0);
