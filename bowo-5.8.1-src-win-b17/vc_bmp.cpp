...
int bw_bmp::tst_b1()
{
  ...
  strcpy(s_ts0,"*.bmp");
  strcpy(s_ts6,"Select *.bmp File For Display");
...
int bw_bmp::echo_bmp(HWND hwnd,char *comm,int comm_size,int mcntr,int p_wptr)
{
  ...
  if (strncmp(str4,".smf",4)==0)
  {
    ...
    lk=bw_buttonvar1.get_t5_btn_mcntr(bw_buttonvar1.t5_btn_ptr3);
    if (lk>lj) lk=lk-lj;
    bw_buttonvar1.set_t5_btn_mcntr(bw_buttonvar1.t5_btn_ptr3,lk);
...
int bw_bmp::w_bmp_add(int p_dbptr,int p_wptr,char *p_fldname,int p_fldname_size)
{
  ...
  strcpy(s_ts0,"*.bmp");
  strcpy(s_ts6,"Select BMP File To Add");
  bw_main1.dlg_f1.lStructSize=sizeof(OPENFILENAME);
