...
int bw_link::tst_link1()
{
  ...
  for (i=0;i<60;i++)
  {
    sprintf(str1,"ptr=%d,win_lkpntr0,1=%d,%d,lkmodi=%d,lkseri=%d,lkbase=%d,lkrecn=%d,lkwind=%d,"
    ,i
    ,get_win_lkpntr(i,0)
    ,get_win_lkpntr(i,1)
    ,get_win_lkmodi(i)
...
int bw_link::tst_link1_lkchar()
{
  ...
  for (i=0;i<60;i++)
  {
    sprintf(str1,"ptr=%d,win_lkpntr0=%d,lkpntr1=%d,lkmodi=%d,lkseri=%d,lkbase=%d,lkrecn=%d,lkwind=%d,"
    ,i
    ,get_win_lkpntr(i,0)
    ,get_win_lkpntr(i,1)
    ,get_win_lkmodi(i)
  ...
  for (i=0;i<60;i++)
  {
    ...
    {
      for (j=get_win_bspntr(i,0);j<=get_win_bspntr(i,1);j++)
      {
	str3[j-get_win_bspntr(i,0)+0]=get_win_chlink(j);
	str3[j-get_win_bspntr(i,0)+1]=0;
      }
    }
    else str3[0]=0;
    ...
    sprintf(str1,"ptr=%d,win_bslink0,1,2=%d,%d,%d,bspntr0,1=%d,%d,win_lnktab=%s,win_chlink=%s,"
    ,i
    ,get_win_bslink(i,0)
    ,get_win_bslink(i,1)
    ,get_win_bslink(i,2)
...
int bw_link::tst_link2()
{
  ...
  for (i=0;i<60;i++)
  {
    if (get_win_bslink(i,2)!=0)
    {
      for (j=get_win_bspntr(i,0);j<=get_win_bspntr(i,1);j++)
      {
	str1[j-get_win_bspntr(i,0)]=get_win_chlink(j);
	str1[j-get_win_bspntr(i,0)+1]=0;
      }
    }
    else str1[0]=0;
    ...
    sprintf(str3,"ptr=%d,win_bslink0,1,2=%d,%d,%d,bspntr0,1=%d,%d,win_chlink=%s,win_lnktab=%s,"
...
int bw_link::tst_link3()
{
  ...
  {
    str1[0]=0;
    for (j=get_win_svpntr(i*5,0);j<get_win_svpntr(i*5,1);j++)
    {
      str1[j-get_win_svpntr(i*5,0)]=get_win_svar(j);
      str1[j-get_win_svpntr(i*5,0)+1]=0;
    }

    sprintf(str3,"win_svpntr1,2=%d,%d,svused=%d,svseri=%d,svbase=%d,svrecn=%d,svwind=%d,svar=%s,"
    ,get_win_svpntr(i*5,0)
    ,get_win_svpntr(i*5,1)
    ,get_win_svused(i*5)
    ,get_win_svseri(i*5)
    ,get_win_svbase(i*5)
    ,get_win_svrecn(i*5)
...
int bw_link::tst_link5()
{
  ...
  {
    str1[0]=0;
    for (j=get_win_svpntr(i,0);j<get_win_svpntr(i,1);j++)
    {
      str1[j-get_win_svpntr(i,0)]=get_win_svar(j);
      str1[j-get_win_svpntr(i,0)+1]=0;
    }

    sprintf(str3,"ptr=%d,win_svpntr0,1=%d,%d,svused=%d,svseri=%d,svbase=%d,svrecn=%d,svwind=%d,svar=%s,"
    ,i,get_win_svpntr(i,0)
    ,get_win_svpntr(i,1)
    ,get_win_svused(i)
    ,get_win_svseri(i)
    ,get_win_svbase(i)
...
int bw_link::w_dele_link(int wptr)
{
  ...
  if (i>=0)
  {
    l=get_win_lkpntr(i,0);
    m=get_win_lkpntr(j,1);
    p=get_win_lkpntr(n,1);

    r=get_win_bspntr(i,0);
    s=get_win_bspntr(j,1);
    t=get_win_bspntr(n,1);
...
*/
  if (i>=0)
  {
    for (k=j+1;k<=n;k++)
    {
      if (get_win_lkpntr(k,0)-o>=0)
	set_win_lkpntr(k-u,0,get_win_lkpntr(k,0)-o);
      else
	set_win_lkpntr(k-u,0,0);

      if (get_win_lkpntr(k,1)-o>=0)
	set_win_lkpntr(k-u,1,get_win_lkpntr(k,1)-o);
      else
	set_win_lkpntr(k-u,1,0);

      if (get_win_bspntr(k,0)-v>=0)
	set_win_bspntr(k-u,0,get_win_bspntr(k,0)-v);
      else
	set_win_bspntr(k-u,0,0);

      if (get_win_bspntr(k,1)-v>=0)
	set_win_bspntr(k-u,1,get_win_bspntr(k,1)-v);
      else
	set_win_bspntr(k-u,1,0);
    ...
    for (y=m+1;y<=p;y++)
    {
      ...
      set_win_lkfield(y-o,get_win_lkfield(y));
  ...
  if (i>=0)
  {
	...
	set_win_bspntr(k,0,0);
	set_win_bspntr(k,1,0);

	set_win_lkpntr(k,0,0);
	set_win_lkpntr(k,1,0);
      }
    }
...
int bw_link::w_dele_svar(int wptr)
{
  ...
  if (i>=0)
  {
    l=get_win_svpntr(i,0);
    m=get_win_svpntr(j,1);
    p=get_win_svpntr(n,1);
  }
  else
  {
    if (r>=0)
    {
      l=get_win_svpntr(r,1);
      m=get_win_svpntr(r,1);
      p=get_win_svpntr(n,1);
    }
    else
    {
...
*/
  ...
  if (i>=0)
  {
    for (k=j+1;k<=n;k++)
    {
      ...
      set_win_svpntr(i+k-j-1,0,get_win_svpntr(k,0)-o);
      set_win_svpntr(i+k-j-1,1,get_win_svpntr(k,1)-o);
    }
	...
	set_win_svpntr(k,0,0);
	set_win_svpntr(k,1,0);
      }
    }
...
int bw_link::w_read_link(char *p_fn,int p_fn_size,int p_wptr,int num)  //read link file
{
  ...
  if (i>0) l=get_win_bspntr(i-1,1)+1;
  else l=0;
  ...
  while (1)
  {
    ...
    if (bw_inkey1.strempty(s_tmps1,SMG_SIZE)==0) break;
    if (s_tmps1[0]=='&') break;
    ...
    l=w_find_link_cd(p_wptr,s_tmps1,SMG_SIZE,i,l);
    if (l<0)
    {
      fclose(fp1);
      return(1);
...
int bw_link::w_find_link_tn(char *p_str,int p_str_size,int p_wptr,int i) // get table name
{
  int  j,l,m,n;
  ...
  if (j>=p_str_size) return(1);
  p6=j-1;
  bw_inkey1.cut_string(p_str,p1,p2-p1+1,p_str_size,tn1,FN_SIZE);
  bw_inkey1.cut_string(p_str,p3,p4-p3+1,p_str_size,tn2,FN_SIZE);
  bw_inkey1.cut_string(p_str,p5,p6-p5+1,p_str_size,tn3,FN_SIZE);
...
int bw_link::w_find_link_cd(int p_wptr,char *p_str,int p_str_size,int i,int k) // link condition string
{
  int  l,m;
  int  p1,p2;
  char s_tmps1[SMG_SIZE];
  ...
  if (l>=p_str_size) return(-1);
  p2=l-1;
  bw_inkey1.cut_string(p_str,p1,p2-p1+1,p_str_size,s_tmps1,SMG_SIZE);
  l=k;

  if (l+strlen(s_tmps1)>=LINK_CHAR)
  {
    bw_win1.w_atten10(bw_main1.get_win_hwnd(p_wptr));
    return(-1);
  }
  ...
  set_win_bspntr(i,0,m);
  set_win_bspntr(i,1,k);
...
int bw_link::w_link_modi(int p_wptr)  // fetch link var in condition // and link var if modified // prepare for link
{
  ...
  for (i=0;i<COND_NUM;i++)
  {
    ...
    if (i>0) p1=get_win_lkpntr(i-1,1)+1;
    else p1=0;

    set_win_lkpntr(i,0,p1);
    p2=get_win_bspntr(i,0);
    ...
    while (1)
    {
      ...
      if (bw_inkey1.strempty(fn,SMG_SIZE)==0) continue;
      if (bw_inkey1.strempty(tn,SMG_SIZE)==0) continue;
    ...
    set_win_lkpntr(i,1,p1-1);
  }
/*
  for (i=0;i<=COND_NUM-1;i++)
  {
    loc(i+1,1);
    printf("p1,p2=%d,%d,",win_lkpntr[i][0],win_lkpntr[i][1]);
  }
  inkey();
...
int bw_link::w_fetch_link_var(int p1,int p2,char *p_val,int p_val_size)
{
  ...
  for (i=p2;i<=get_win_bspntr(p1,1);i++)
  {
    exist=0;
    ...
    for (k=i;k<=get_win_bspntr(p1,1);k++)
    {
      exist=0;
      exist2=0;
...
int bw_link::w_link_modi_mark(int p_wptr,int p_dbptr,int seri,int field)   // if field changed ,mark it 
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    for (j=0;j<COND_NUM;j++)
    {
      ...
      {
	for (k=get_win_lkpntr(j,0);k<=get_win_lkpntr(j,1);k++)
	{
	  if (get_win_lkbase(k)==i)
	  {
	    if ((seri>=0)&&(get_win_lkseri(k)!=seri)) continue;
	    set_win_lkmodi(k,1);
	    set_win_lkfield(k,field);
	  }
	}
      }
    }
...
/*
  ...
  {
    loc(i+1,1);
    for (j=win_lkpntr[i][0];j<=win_lkpntr[i][1];j++)
  ...
  for (r=0;r<COND_NUM;r++)
  {
	...
	for (j=0;j<COL_NUM;j++)
	{
	  if (bw_buff1.get_t3_linecol(p_wptr,i,j)!=0)
	  {
	    ...
	    bw_fldstru1.win_field_num=bw_fldstru1.w_find_fldnum(s_fldname,SMG_SIZE,k);
	    ...
	    bw_fldstru1.win_field_num=bw_fldstru1.w_find_fldnum(s_fldname,SMG_SIZE,k);
	    if (bw_fldstru1.win_field_num>=0)
	    {
...
int bw_link::w_link_modified(int p_wptr,int basep)
{
  ...
  for (i=0;i<COND_NUM;i++)
  {
    ...
    {
      for (j=get_win_lkpntr(i,0);j<=get_win_lkpntr(i,1);j++)
...
int bw_link::w_link_cond_ok(int condnum,int p_wptr,int basep,int recn,int page)
{
  ...
  state=0;
  p2=get_win_bspntr(i,0);     /* load condition */
  ...
  while (1)
  {
    ...
    state=1;  // table
    if (bw_inkey1.strempty(tn,SMG_SIZE)==0)
    {
      bw_win1.w_atten13(bw_main1.get_win_hwnd(p_wptr),str1);
      return(0);
    }
    else
    {
    ...
    if (bw_inkey1.strempty(fn,SMG_SIZE)==0) // field
    {
      bw_win1.w_atten14(bw_main1.get_win_hwnd(p_wptr),str1);
      return(0);
    }
    else
    {
    ...
    if (ptr1==basep) // rec no.  seri number
    {
      ...
      if (ptr3>=VAL_NUM)
      {
        bw_win1.w_atten12(bw_main1.get_win_hwnd(p_wptr));
        return(0);
      }

      for (j=1;j<=bw_fldstru1.get_t2_fldlen(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0));j++)
      {
	set_win_cbb1(ptr3,j-1,bw_xbase1.get_win_recvar(bw_fldstru1.get_t2_fldposi(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0))+j-1));
	set_win_cbb1(ptr3,j-0,0);
      }
      set_win_bb3(ptr3,bw_fldstru1.get_t2_fldtype(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0)));
	...
	if ((get_win_svwind(j)==p_wptr)&&(get_win_svbase(j)==ptr1)&&(get_win_svseri(j)==ptr2))
      ...
      for (k=get_win_svpntr(j,0);k<get_win_svpntr(j,1);k++)
      {
	set_win_cbb1(ptr3,k-get_win_svpntr(j,0),get_win_svar(k));
	set_win_cbb1(ptr3,k-get_win_svpntr(j,0)+1,0);
      }
      set_win_bb3(ptr3,bw_fldstru1.get_t2_fldtype(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0)));
    }
  ...
  //sprintf(str1,"link_cond_ok,sv1=%s,sv2=%s,",sv1,sv2);
  //bw_getread1.deb_record(str1);
  ...
  for (i=0;i<LIN_NUM;i++)
  {
	...
	if (bw_win1.get_t_fldedt(n)==0)
	{
	  ...
	  p2++;
          if (p2>=WIN_VAR_NUM)
	  {
             bw_win1.w_atten11(bw_main1.get_win_hwnd(p_wptr));
             return(1);
          }
	}
      }
...
/*
  for (i=1;i<=20;i++)
  {
    ...
    printf("pntr=%d,seri=%d,base=%d,",
	      win_svpntr[i],win_svseri[i],win_svbase[i]);
  }
  inkey();
*/
  return(0);                         
...
int bw_link::w_save_sv(int p_wptr,int base,int seri,int recn,int posi,int len)  /*save scrn var to arr*/
{
  ...
  for (j=1;j<=len;j++) set_win_svar(get_win_svpntr(i,0)+j-1,bw_xbase1.get_win_recvar(posi+j-1));
/*
  loc(20,1);
  printf("%d,",win_svpntr[exist]);
  for (i=1;i<=len;i++)
    putchar(win_svar[win_svpntr[exist]+i-1]);
  inkey();
*/
  return(0);
}
...
int bw_link::w_get_sv(int p_wptr,int base,int seri,int recn,char *p_str,int p_str_size)  /*save scrn var to arr*/
{
  ...
  for (j=get_win_svpntr(i,0);j<get_win_svpntr(i,1);j++)
  {
    if (j-get_win_svpntr(i,0)<p_str_size-1)
    {
      p_str[j-get_win_svpntr(i,0)+0]=get_win_svar(j);
      p_str[j-get_win_svpntr(i,0)+1]=0;
    }
  }
/*
  loc(20,1);
...
int bw_link::w_keep_linkfld(int p_wptr,int p_dbptr,int dir)
{
  ...
  k=get_win_lkpntr(i,0);
  l=get_win_lkseri(k);
  w_get_sv(p_wptr,j,l,1,str4,SMG_SIZE);

  for (m=get_win_bspntr(i,0);m<=get_win_bspntr(i,1);m++)
  {
    str1[m-get_win_bspntr(i,0)]=get_win_chlink(m);
  }
  str1[get_win_bspntr(i,1)-get_win_bspntr(i,0)+1]=0;
...
int bw_link::get_win_svpntr(int vptr,int p01)
{
  if ((vptr<0)||(vptr>=WIN_VAR_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(win_svpntr[vptr][p01]);
}
int bw_link::set_win_svpntr(int vptr,int p01,int val)
{
  if ((vptr<0)||(vptr>=WIN_VAR_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  win_svpntr[vptr][p01]=val;
  return(0);
}
...
int bw_link::get_win_bspntr(int dptr,int p01)
{
  if ((dptr<0)||(dptr>=COND_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(win_bspntr[dptr][p01]);
}
int bw_link::set_win_bspntr(int dptr,int p01,int val)
{
  if ((dptr<0)||(dptr>=COND_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  win_bspntr[dptr][p01]=val;
  return(0);
}
int bw_link::get_win_lkpntr(int cptr,int p01)
{
  if ((cptr<0)||(cptr>=COND_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(win_lkpntr[cptr][p01]);
}
int bw_link::set_win_lkpntr(int cptr,int p01,int val)
{
  if ((cptr<0)||(cptr>=COND_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  win_lkpntr[cptr][p01]=val;
  return(0);
}
...
int bw_link::get_win_lkfield(int lptr)
{
  if ((lptr<0)||(lptr>=LINK_MODI_NUM)) return(0);
  return(win_lkfield[lptr]);
}
int bw_link::set_win_lkfield(int lptr,int val)
{
  if ((lptr<0)||(lptr>=LINK_MODI_NUM)) return(0);
  win_lkfield[lptr]=val;
  return(0);
}
