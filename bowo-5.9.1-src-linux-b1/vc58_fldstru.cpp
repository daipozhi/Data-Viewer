...
int bw_fldstru::w_read_stru(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  if (fp1==NULL)
  {
    bw_win1.w_atten1(bw_main1.win_hwnd1,p_fn);
    return(1);
  }
  ...
  while (!feof(fp1))
  {
    ...
    {
      fclose(fp1);
      bw_win1.w_atten2(bw_main1.win_hwnd1);
      return(1);
    }
  ...
  bw_xbase1.set_win_fld_cntr(p_dbptr,w_cnt_tabfield(p_dbptr));   /* field number */
  bw_xbase1.set_win_len_cntr(p_dbptr,  w_cnt_reclen(p_dbptr));   /* record length */
...
int bw_fldstru::w_dele_stru(int p_dbptr)
{
  ...
  bw_xbase1.set_win_fld_cntr(p_dbptr/*,0*/,0);   /* field number */
  bw_xbase1.set_win_len_cntr(p_dbptr/*,1*/,0);   /* record length */
...
//		bw_xbase1.set_win_recno(k,p_wptr,0,s_seri[k]);
	    ...
	    if (s=='l')
	      ...
	      if (bw_xbase1.win_state_dbf_err!=0) s_fldval[0]=' ';
	      ...
	      if (bw_xbase1.win_state_dbf_err!=0) s_fldval2[0]=' ';
...
int bw_fldstru::w_tv_get_read_g(char *p_fldname,int p_fldname_size,int p_wptr,int l,int c,char comm)  // 'tv'(table variant)
{
  ...
  if ((w_find_fldtype(s_fldname,SMG_SIZE,j)=='n')||(w_find_fldtype(s_fldname,SMG_SIZE,j)=='c'))
  {
    ...
    {
      w_get_tab_var(s_fldname,SMG_SIZE,j,s1,SMG_SIZE);
    }
  ...
  if (p_fldname[0]==':')
  {
    ...
    k=bw_getread1.get_read(p_wptr,
                        l-bw_win1.get_win_con(p_wptr,1),
			 c-bw_win1.get_win_con(p_wptr,0),
		       "",
		       s3,
		       bw_win1.get_t_fldlen(i),
		       fldtype,
...
int bw_fldstru::w_get_tab_var(char *p_fldname,int p_fldname_size,int p_dbptr,char *p_val,int p_val_size)
{
  ...
  for (m=0;m<j;m++)
  {
    if (m+1<p_val_size)
    {
      p_val[m+0]=bw_xbase1.get_win_recvar(k+m);
      p_val[m+1]=0;
    }
  }
...
int bw_fldstru::w_cmp_tab_var(char *p_fldname,int p_fldname_size,int p_dbptr,char *p_val,int p_val_size)
{
  ...
  for (m=0;m<l;m++)
  {
    if (m<p_val_size)
    {
      if (p_val[m]!=bw_xbase1.get_win_recvar(k+m))
      {
        exist=1;
	break;
      }
...
int bw_fldstru::w_sav_tab_var(char *p_fldname,int p_fldname_size,int p_dbptr,char *p_val,int p_val_size)
{
  ...
  for (m=0;m<j;m++)
  {
    ...
    bw_xbase1.set_win_recvar(k+m,c1);
  }
...
int bw_fldstru::w_fetch_rec(int p_dbptr,int p_wptr,int seri)
{
  ...
  if (bw_xbase1.get_win_rec_cntr(p_dbptr)<1)
  {
    ...
    else  // linked tab
    {
      ...
      for (i=1;i<=seri;i++)
      {
        while (1)
        {
          r2=bw_mbase1.w_down_rec(p_dbptr,bw_main1.win_hnd_id,r1);
/*
          m=bw_buff1.get_t3_linecol(bw_main1.win_hnd_id,bw_win1.pline,bw_win1.pcolu);
...
int bw_fldstru::w_insert_rec(int p_dbptr,int p_wptr)
{
  ...
  if (s_state1!=0)
  {
    bw_win1.w_atten7(bw_main1.get_win_hwnd(p_wptr));                   /* insert record error */
    return(0);
  }
...
int bw_fldstru::w_add_rec(int p_dbptr,int p_wptr)
{
  ...
  if (s_state1!=0)
  {
    bw_win1.w_atten7b(bw_main1.get_win_hwnd(p_wptr));                   /* insert record error */
    return(0);
  }
...
int bw_fldstru::w_delete_rec(int p_dbptr,int p_wptr)
{
  ...
  if (s_state1!=0)
  {
    bw_win1.w_atten7a(bw_main1.get_win_hwnd(p_wptr));                   /* delete record error */
    return(0);
  }
...
int bw_fldstru::w_rollback_base(int p_dbptr,int p_wptr)
{
  ...
  bw_xbase1.w_rollback_xbase(p_wptr,bw_xbase1.get_win_base_backup(p_dbptr),p_dbptr);
...
int bw_fldstru::w_commit_base(int p_dbptr,int p_wptr)
{
  ...
  bw_xbase1.w_commit_xbase(p_wptr,bw_xbase1.get_win_base_backup(p_dbptr),p_dbptr);
...
int bw_fldstru::w_find_cond(int p_dbptr,int p_wptr)
{
  ...
  bw_main1.get_view(sfn1,FN_SIZE,bw_main1.get_win_hwnd(p_wptr),p_wptr/*bw_win1.get_win_ptr_grp_ptr(p_wptr,0)*/);
...
int bw_fldstru::w_find_recno(int p_dbptr,int p_wptr)
{
  ...
  bw_xbase1.win_fh2[p_wptr]=/*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
...
int bw_fldstru::w_find_cond_ok(int p_dbptr)
{
  ...
  int   exist;
  int   zero=0;
  char  s_fldval[SMG_SIZE]; // scrn field val
  char  s_tabval[SMG_SIZE]; // tab field val
  char  s_fldcond[SMG_SIZE];
  //char  str5[SMG_SIZE];
  char  s_tabfld[SMG_SIZE]; // scrn field name
  char  s_tabfld2[SMG_SIZE];
      ...
      ;i++)
  {
    ...
    strcpy(s_tabfld,"?");
    strcat(s_tabfld,s_fldname);

    strcpy(s_tabfld2,"-");
    strcat(s_tabfld2,s_fldname);

    if ((bw_dialog1.w_mv_exist(s_tabfld,SMG_SIZE,zero)==1)||(bw_dialog1.w_mv_exist(s_tabfld2,SMG_SIZE,zero)==1))
    {
      if (bw_dialog1.w_mv_exist(s_tabfld2,SMG_SIZE,zero)==1) strcpy(s_tabfld,s_tabfld2);

      bw_dialog1.w_mv_fet_val(s_tabfld,SMG_SIZE,zero,s_fldval,SMG_SIZE);

      bw_inkey1.strtrim(s_fldval,SMG_SIZE);
      bw_inkey1.strltrim(s_fldval,SMG_SIZE);

      w_get_tab_var(s_fldname,SMG_SIZE,p_dbptr,s_tabval,SMG_SIZE);

      if (w_find_fldtype(s_fldname,SMG_SIZE,p_dbptr)=='c')
      {
	k=bw_inkey1.str_seek_char_first(s_fldval,SMG_SIZE,';');

	if (s_fldval[k]!=';')
        {
	  if (bw_inkey1.strfind(s_tabval,SMG_SIZE,s_fldval,SMG_SIZE)<0)
	  {
	    j=0;
	    break;
	  }
	...
	else
	{
	  ...
	  while (1)
	  {
	    ...
	    {
	      s_fldval[k]=' ';
	      bw_inkey1.cut_string(s_fldval,l,k-l,SMG_SIZE,s_fldcond,SMG_SIZE);
	      if (bw_inkey1.strfind(s_tabval,SMG_SIZE,s_fldcond,SMG_SIZE)>=0)
	      {
		m=1;
		break;
              }
      ...
      {
        f1=bw_inkey1.str2float(s_tabval,SMG_SIZE);
	...
	{
          exist=0;
	  for (l=k;l<(int)strlen(s_fldval);l++)
	  {
	    if ((s_fldval[l]=='<')||(s_fldval[l]=='=')||(s_fldval[l]=='>'))
	    {
	      exist=1;
	      break;
            }
	  ...
	  else
	  {
		...
		else
		{
		  ...
		  m=2;
            ...
            k=l+1+o;
	    exist=0;
	    for (n=k;n<=(int)strlen(s_fldval);n++)
	    {
	      if ((s_fldval[n]=='<')||(s_fldval[n]=='=')||(s_fldval[n]=='>')||(s_fldval[n]<=' ')||(s_fldval[n]==','))
	      {
	        exist=1;
	        break;
              }
	    ...
	    {
	      bw_inkey1.cut_string(s_fldval,l+1+o,n-l-1-o,SMG_SIZE,s_fldcond,SMG_SIZE);
	...
	if ((s_fldval[0]=='1')||(s_fldval[0]=='y')||(s_fldval[0]=='t')
          ||(s_fldval[0]=='Y')||(s_fldval[0]=='T'))
	{
          if ((s_tabval[0]!='1')&&(s_tabval[0]!='y')&&(s_tabval[0]!='t')
            &&(s_tabval[0]!='Y')&&(s_tabval[0]!='T'))
	  {
	    j=0;
	    break;
	  }
...
//      {
//	if ((s_tabval[0]=='0')||(s_tabval[0]=='n')||(s_tabval[0]=='f')
//          ||(s_tabval[0]=='N')||(s_tabval[0]=='F'))
//	{
//	  j=0;
//	  break;
//	}
...
//      fclose(fh);
  ...
  bw_edit1.get_edit(str3,FN_SIZE,p_wptr,bw_main1.get_win_hwnd(p_wptr));
*/
  return(0);
}
...
//  FILE *fh1;
  ...
  bw_main1.dlg_f1.hwndOwner=bw_main1.get_win_hwnd(p_wptr);
