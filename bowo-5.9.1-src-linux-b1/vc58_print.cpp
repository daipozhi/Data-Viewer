...
int bw_print::w_read_page(char *p_fn,int p_fn_size)
{
  ...
  if (s_fp1==NULL)
  {
    bw_win1.w_atten1(bw_main1.win_hwnd1,s_fn);
    return(1);
  }
  ...
  for (i=0;i<PAGE_LINE+10;i++)
  {
    ...
    {
      same=1;  //use table
      for (p2=p1;p2<=p1+strlen(texts1)-1;p2++)  /* recnigze keywords */
      {
  ...
  bw_main1.win_page_line_num=lcntr;
...
int bw_print::w_out_win_print()
{
  ...
  get_win_print_fname(s_fn,SMG_SIZE);
...
*//*
  ...
  while(1)
  {
    for (i=0;i<bw_main1.win_page_size;i++)
    {
...
int bw_print::w_out_linux_print()
{
  ...
  if (s_fp1==NULL)
  {
    bw_win1.w_atten1(bw_main1.win_hwnd1,str1);
    return(1);
  }
  ...
  if (s_fp2==NULL)
  {
    bw_win1.w_atten1(bw_main1.win_hwnd1,str2);
    return(1);
  }
...
int bw_print::w_print_page(int p_wptr)
{
  ...
  for (k=0;k<STRU_NUM;k++)
  {
	...
	{
	  if (s_fldseri[k]!=get_t6_fldseri(m))
	  {
	    s_recno[k]=bw_mbase1.w_down_rec(k,p_wptr,s_recno[k]);
	    s_fldseri[k]=get_t6_fldseri(m);
	  ...
	  if ((s=='c')||(s=='n')||(s=='l'))
	  {
	    ...
	    {
	      if (n+1>(int)strlen(s_fldval)) set_t6_tmps2(i,j+n,' ');
	      else  set_t6_tmps2(i,j+n,s_fldval[n]);
            }
	  }
	  else
	  {
...
int bw_print::w_print_lnk(int p_wptr) /* echo win rec where modified */
{
  ...
  for (r=0;r<COND_NUM;r++)
  {
	...
	if (get_t6_fldedit(v)==1)
	{
	  ...
	  for (o=0;o<get_t6_fldlen(m);o++)
	  {
	    if (o+1>(int)strlen(s_fldval)) set_t6_tmps2(i,j+o,' ');
	    else  set_t6_tmps2(i,j+o,s_fldval[o]);
          }
...
int bw_print::w_print_cal()
{
  ...
  for (m=0;m<=t6_ptr1;m++)
  {
	...
	w_save_sv(win_fcnt,win_field_num-t2_fldpntr[win_fcnt][0]
			,t6_fldseri[ptr1]
...
int bw_print::w_print_read()
{
  ...
  for (k=0;k<=t6_ptr1;k++)
  {
	...
	get_t6_fldname(k,sfn,SMG_SIZE);
      ...
      if (m== 8)  bw_inkey1.int2str(0,s_tmps2,SMG_SIZE);
      if (m== 9)  bw_inkey1.int2str(0,s_tmps2,SMG_SIZE);
      if (m==10)  strcpy(s_tmps2,"");
      if (m==11)  bw_dialog1.w_get_date(s_tmps2,SMG_SIZE);

      for (l=0;l<get_t6_fldlen(k);l++)
      {
        if (l+1>(int)strlen(s_tmps2)) set_t6_tmps2(i,j+l,' ');
        else set_t6_tmps2(i,j+l,s_tmps2[l]);
      }
    }
  }
...
int bw_print::w_save_pv(int p_wptr,int p_dbptr,int seri,int recn,int posi,int len)  /*save scrn var to arr*/
{
  ...
  {
    if ((get_win_pvwind(i)==p_wptr)&&(get_win_pvbase(i)==p_dbptr)&&(get_win_pvseri(i)==seri)&&(get_win_pvrecn(i)==recn))
    {
      exist=1;
      break;
    }
  ...
  for (j=0;j<len;j++) set_win_pv(get_win_pvpntr(i,0)+j,bw_xbase1.get_win_recvar(posi+j));
...
int bw_print::w_get_pv(int p_wptr,int p_dbptr,int seri,int recn,char *p_str,int p_str_size)  /*save scrn var to arr*/
{
  ...
  for (j=get_win_pvpntr(i,0);j<get_win_pvpntr(i,1);j++)
  {
    if (j-get_win_pvpntr(i,0)<p_str_size-1)
    {
      p_str[j-get_win_pvpntr(i,0)+0]=get_win_pv(j);
      p_str[j-get_win_pvpntr(i,0)+1]=0;
    }
  }
...
int bw_print::get_win_pvpntr(int vptr,int p01)
{
  if ((vptr<0)||(vptr>=PRT_VAR_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  return(win_pvpntr[vptr][p01]);
}
int bw_print::set_win_pvpntr(int vptr,int p01,int val)
{
  if ((vptr<0)||(vptr>=PRT_VAR_NUM)) return(0);
  if ((p01<0)||(p01>1)) return(0);
  win_pvpntr[vptr][p01]=val;
  return(0);
}
...
int bw_print::w_print(int p_wptr,char *p_fn,int p_fn_size)
{
  ...
  while (1)
  {
	...
	if ((get_t6_tmps2(i2,j2)>' ')||(get_t6_tmps2(i2,j2)<0))
	{
      ...
      get_s_t6_tmps2(i2,str2,SMG_SIZE);
  ...
  set_win_print_fname(s_fn,FN_SIZE);
...
int bw_print::w_print_page_down(int p_dbptr,int p_wptr)
{
  ...
  {
    s_tmpn1=w_cnt_pageline(p_dbptr,PAGE_LINE);
    //s_tmpn2=s_tmpn1+1;

    for (i=0;i<s_tmpn1;i++)  // move
    {
...
int bw_print::w_print_link_cond_ok(int condnum,int p_wptr,int p_dbptr,int recn,int page)
{
  ...
  if ((bw_link1.get_win_bslink(i,1)==p_dbptr)&&(bw_link1.get_win_bslink(i,2)==p_wptr))
  {
    exist=1;
  }
  ...
  state=0;
  p2=bw_link1.get_win_bspntr(i,0);     /* load condition */
  ...
  while (1)
  {
    ...
    state=1;
    if (bw_inkey1.strempty(tn,SMG_SIZE)==0)  // table name
    {
    ...
    if (bw_inkey1.strempty(fn,SMG_SIZE)==0)  // field name
    {
    ...
    {
      exist=bw_link1.w_name_in_tab(fn,SMG_SIZE,ptr1);
      if (exist<0)
      {
        bw_win1.w_atten14(bw_main1.get_win_hwnd(p_wptr),str1);
        return(0);
    ...
    if (ptr1==p_dbptr)
    {
      ...
      {
	bw_link1.set_win_cbb1(ptr3,j-1,bw_xbase1.get_win_recvar(bw_fldstru1.get_t2_fldposi(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0))+j-1));
	bw_link1.set_win_cbb1(ptr3,j-0,0);
      }
      bw_link1.set_win_bb3(ptr3,bw_fldstru1.get_t2_fldtype(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0)));
	...
	if ((get_win_pvwind(j)==p_wptr)&&(get_win_pvbase(j)==ptr1)&&(get_win_pvseri(j)==ptr2))
      ...
      for (k=get_win_pvpntr(j,0);k<get_win_pvpntr(j,1);k++)
      {
	bw_link1.set_win_cbb1(ptr3,k-get_win_pvpntr(j,0)+0,get_win_pv(k));
	bw_link1.set_win_cbb1(ptr3,k-get_win_pvpntr(j,0)+1,0);
      }
      bw_link1.set_win_bb3(ptr3,bw_fldstru1.get_t2_fldtype(ptr2+bw_fldstru1.get_t2_fldpntr(ptr1,0)));
    }
...
int bw_print::get_s_t6_tmps2(int lptr,char *p_s,int p_s_size)
{
  int i,j;
...
int bw_print::get_win_print_fname(char *p_s,int p_s_size)
{
  int i,j;
  ...
  for (j=0;j<i-1;j++)
  {
    p_s[j+0]=win_print_fname[j];
    p_s[j+1]=0;
  }

  return(0);
}
int bw_print::set_win_print_fname(char *p_s,int p_s_size)
{
  int i,j;
