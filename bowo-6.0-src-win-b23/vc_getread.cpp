...
{  
  ...
  if (command=='g')
  {
    ...
    HideCaret(bw_main1.get_mn_hwnd(p_wptr));
    SetCaretPos(mn_p_x+(int)((posi-1)*bw_main1.mn_edit_xchar)
                 ,mn_p_y);
    ShowCaret(bw_main1.get_mn_hwnd(p_wptr));
  }
  else if (command=='r')
...
smg_ptr2,get_smg_color(smg_ptr2),get_smg_ddec(smg_ptr2),get_smg_posi(smg_ptr2));
	...
	return(1);
                  ...
                  (mn_confirm==0)  )  
      {
	if (mn_edit_mode[array_ptr]==1) bw_main1.mn_key=RIGHT_ROW;
	else bw_main1.mn_key=10;
        return(1);
      }
    }
  }
...
/* --- pre insert when smg_chns_next==1 --- */
  if (bw_main1.mn_key!=0)
  {
    if (bw_main1.mn_key==410) return(0);
    if (bw_main1.mn_key==427) return(0);
    if (bw_main1.mn_key==639) return(0);
...
*/
	  ...
	  HideCaret(bw_main1.get_mn_hwnd(p_wptr));
          if (n2+2>datalen)
          { 
	    /*TextOut(hdc,mn_p_x,mn_p_y,mn_chns_str,1);*/ 
          }
          else
          {
            //TextOut(hdc,mn_p_x,mn_p_y,mn_chns_str,2);
          }
	  ShowCaret(bw_main1.get_mn_hwnd(p_wptr));
/*
          // --- debug ---
...
/*
      if (smg_insert==0)
      {
        if (mn_chns_char==0)
        {
	  set_smg_data(mn_p1+n2-1,c1);
	  n2++;
	  scrn_c++;
	  mn_p_x=(int)(bw_main1.mn_edit_xchar*(scrn_c-1));
        }
        else
        {
          mn_chns_char=0;
	  set_smg_data(mn_p1+n2-1,mn_chns_str[0]);
	  if (mn_p1+n2<=datalen_real)
	    set_smg_data(mn_p1+n2-0,mn_chns_str[1]);
          n2=n2+2;
          scrn_c=scrn_c+2;
        }
	if (mn_p1+n2>datalen_real)
	{
	  ...
	  mn_p_x=(int)((mn_cur1-1)*bw_main1.mn_edit_xchar);
	  for (i=1;i<=datalen;i++)
	  {
	...
	if (n2>datalen)
	{
	  ...
	  p2=get_txt_after(str1,datalen_real,datalen,mn_p1);
	  p3=p2-mn_p1;
	  mn_p1=p2;
	  for (i=mn_p1+1;i<=mn_p1+datalen;i++)
	  {
	    if (i<=datalen_real) str2[i-mn_p1-1]=get_smg_data(i-1);
	    else str2[i-mn_p1-1]=' ';
	  }
	  str2[datalen]=0;
	  mn_p_x=(int)(bw_main1.mn_edit_xchar*(mn_cur1-1));
	  HideCaret(bw_main1.get_mn_hwnd(p_wptr));
          TextOut(hdc,mn_p_x,mn_p_y,str2,strlen(str2));
	  ShowCaret(bw_main1.get_mn_hwnd(p_wptr));
	  n2=n2-p3;
...
*/      
  ...
  for (k=0;k<300;k++)
  {
	  ...
	  scrn_c++;
        ...
        else
        {
          if (mn_p1+n2+1<=datalen_real)
          {
            ...
            mn_chns_char=0;
	    for (i=datalen_real-2;i>=n2+mn_p1;i--)
	    {
	      set_smg_data(i+2-1,get_smg_data(i-1));
	    }
	    set_smg_data(mn_p1+n2+0-1,mn_chns_str[0]);
	    set_smg_data(mn_p1+n2+1-1,mn_chns_str[1]);
	    n2=n2+2;
	    scrn_c=scrn_c+2;
          }
          else
	...
	if (mn_p1+n2>datalen_real)
	{
	  mn_p_x=(int)((mn_cur1)*bw_main1.mn_edit_xchar);
	...
	if (n2>datalen)
	{
	  if (mn_edit_mode[ptr]==1)
	  {
	    ...
	    for (i=mn_p1+1;i<=mn_p1+datalen;i++)
	    {
	      if (i<=datalen_real)
              {
                str2[i-mn_p1-1+0]=get_smg_data(i-1);
                str2[i-mn_p1-1+1]=0;
              }
	      else
              {
	...
	{
	  mn_p_x=(int)(bw_main1.mn_edit_xchar*(scrn_c-n2+1));

	  for (i=1;i<=datalen;i++)
	  {
	    if (i+mn_p1>datalen_real)
            {
              str2[i-1+0]=' ';
              str2[i-1+1]=0;
            }
            ...
            {
              str2[i-1+0]=get_smg_data(mn_p1+i-1);
              str2[i-1+1]=0;
	    }
	  }
	  //str2[datalen]=0;
...
*/
  ...
  for (k=0;k<300;k++)
  {
	...
	{
	  get_s_smg_data(str1,SMG_SIZE);
	  p2=get_txt_before(str1,datalen,mn_p1);
	  p3=mn_p1-p2;
	  mn_p1=p2;
  ...
  for (k=0;k<300;k++)
  {
    ...
    if (pos[k]<0) break;
      ...
      if (n2>w+w2)
      {
        ...
        if (w==1)
        {
  	  for (i=n2+mn_p1;i<=datalen_real;i++)
	  {
	    set_smg_data(i-1-1,get_smg_data(i-1));
	  }
        ...
        for (i=1/*n2*/;i<=datalen;i++)
	{
	  if (i+mn_p1>datalen_real)
          ...
          {
            str2[i-1/*n2*/+0]=get_smg_data(mn_p1+i-1);
            str2[i-1/*n2*/+1]=0;
          }
	}
	//str2[datalen-n2+1]=0;
...
*/
	...
	else
	{
	  ...
	  //{
	  //  if (i+mn_p1>datalen_real) str2[i-n2]=' ';
	  //  else str2[i-n2]=get_smg_data(mn_p1+i-1);
	  //}
	  //str2[datalen-n2+1]=0;
	  //HideCaret(bw_main1.get_mn_hwnd(p_wptr));
          //TextOut(hdc,mn_p_x,mn_p_y,str2,strlen(str2));
...
*/
    ...
    if (key==UP_ROW)
    {
	  ...
	  //get_s_smg_data(str5,SMG_SIZE);
      //sprintf(str4, "in get_num...after up_arrow,data=%s,datalen=%d,datalen_real=%d,",str5,datalen,datalen_real);
      //bw_getread1.deb_record(str4);
      ...
      //sprintf(str4, "val=%s,",str2);
      //bw_getread1.deb_record(str4);

      //get_smg_string(ptr,str2,SMG_SIZE);
      //sprintf(str4, "ptr=%d,string=%s,",ptr,str2);
      //bw_getread1.deb_record(str4);

	  //get_s_smg_data(str2,SMG_SIZE);
      //sprintf(str4, "data=%s,",str2);
      //bw_getread1.deb_record(str4);
	...
	for (i=n2;i<=datalen;i++)
	{
	  ...
	  {
		  str2[i-n2+0]=get_smg_data(mn_p1+i-1);
		  str2[i-n2+1]=0;
	  }
	}
	//str2[datalen-n2+1]=0;
...
/* convert to number */
  ...
  for (j=0;j<i;j++) str1[30-i+j]=get_smg_data(j);  // copy number where before '.'
  
  str1[30]='.';
  
  for (j=i+1;j<datalen_real;j++) str1[30+j-i]=get_smg_data(j);  // copy number where after '.'
