...
int bw_win::w_login_win(int p1,char *p_win_name,int p_win_name_size,char *p_find_name,int p_find_name_size)
{
  ...
  set_mn_ptr_grp_id(j,i);
  set_ms_window_name(j,p_win_name ,p_win_name_size );
  set_ms_find_name(  j,p_find_name,p_find_name_size);

  for (k=0;k<STRU_NUM;k++)
  {
    bw_xbase1.set_mn_cur_seri(k,j,0);
    bw_xbase1.set_mn_recno(k,j,0,0);
    bw_xbase1.set_mn_recno(k,j,1,0);
    bw_xbase1.set_mn_recno(k,j,2,0);
    bw_xbase1.set_mn_state_in_srch(j,k,0);
  }
...
int bw_win::w_dele_win_ptr_wins_dbf(int p_wptr)
{
  ...
  {
    if (get_mn_ptr_wins_dbf(i,j)==1)
    {
      set_ms_tabname(0,i,"",0);
      set_ms_tabname(1,i,"",0);
    }

    set_mn_ptr_wins_dbf(i,j,0);
  }
...
int bw_win::w_close_win(int p1)
{
  ...
  {
    if ((get_mn_ptr_wins_dbf(m,i)==1)&&(bw_xbase1.get_mn_mem_inuse(m)==1))
    {
      for (n=0;n<EDIT_NUM;n++)
      {
        if (bw_edit1.get_mn_edit_wptr(n)==i)
	{
...
int bw_win::w_close_subwin(int p1)
{
  ...
  for (k=0;k<STRU_NUM;k++)
  {
    if (get_mn_ptr_wins_dbf(k,i)==1)
    {
      bw_fldstru1.w_dele_stru(k);
      bw_mbase1.w_close_base(k,i);
  ...
  w_dele_win_ptr_wins_dbf(i);

  /* notice */
  //j=i;
  
  SetFocus(bw_main1.mn_main_hnd);
  k=get_mn_h_w_ptr(i,0);
  ...
  q=get_mn_ptr_fld_in_win(i,1)-get_mn_ptr_fld_in_win(i,0)+1;
  if (q<0) q=0;

  p12=get_mn_ptr_fld_in_win(i,1);  // last fld ptr
  ...
  p11=get_mn_h_w_ptr(i,5);          // last ci ptr
  ...
  if (i<=1) j2=get_mn_h_w_ptr(i,5)-0+1;                    // how many ci in current window
  else      j2=get_mn_h_w_ptr(i,5)-get_mn_h_w_ptr(i,0)+1;
  ...
  k=get_mn_ptr_get_rd(i);  // first ptr
  o=k;
  ...
  else if (mn_ptr_new_win-1>0) SetFocus(get_mn_phh(mn_ptr_new_win-1));
...
int bw_win::w_read_winfile(char *p_fn,int p_fn_size)
{
  ...
  if (is_tab==1)  /* it is a table scrn */
  {
	...
	set_mn_ptr_wins_dbf(j,i,0);
      ...
      for (j=0;j<=s_ps;j++)
      {
        if (str6[j]>=STRU_NUM) continue;
	set_mn_ptr_wins_dbf(str6[j],i,1);
	bw_xbase1.set_mn_state_bff_in_use(str6[j],1);
      }
    }
	...
	set_ms_tabname(1,str6[s_ps1],str_tab2,FN_SIZE);
      }
	...
	set_ms_tabname(0,str6[s_ps1],str_tab2,FN_SIZE);
      }
	...
	if (i-p1>0)
	{
	  ...
	  if (strncmp(str4,"carryon",7)==0)
	    bw_xbase1.set_mn_carry_on(str6[s_ps1],1);
	  else
	    bw_xbase1.set_mn_carry_on(str6[s_ps1],0);
	}
	else bw_xbase1.set_mn_carry_on(str6[s_ps1],0);
      }
      else bw_xbase1.set_mn_carry_on(str6[s_ps1],0);
    }
  }
  ...
  for (s_ps1=0;s_ps1<=s_ps;s_ps1++)  /* database type , backup type */
  {
    ...
    if (strcmp(str2,"xbase")==0) bw_xbase1.set_mn_base_type(str6[s_ps1],0);
    else bw_xbase1.set_mn_base_type(str6[s_ps1],1);

    bw_inkey1.strtrim(str3,FN_SIZE);

    if (strcmp(str3,"1")==0) bw_xbase1.set_mn_base_backup(str6[s_ps1],1);
    else bw_xbase1.set_mn_base_backup(str6[s_ps1],0);
  }
...
int bw_win::w_cre_ci_button(int p_wptr,char *p_s1,int p_s1_size,int p6,int p7,int p8,int p9)
{
  HWND hnd;
  int  i,j;
  ...
  if (get_mn_ci_mode(p_wptr)==0)
  {
    hnd=CreateWindow("button",p_s1
		      ...
		      ,(int)((p9-p7)*bw_main1.mn_edit_ychar-3)
...
int bw_win::w_cre_ci_radio(int p_wptr,int p6,int p7,int p8,int p9)
{
  HWND hnd;
...
int bw_win::w_cre_ci_check(int p_wptr,int p6,int p7,int p8,int p9)
{
  HWND hnd;
  ...
  for (i=0;i<LIN_NUM;i++)
  {
	...
	if (p5==1)
	{
	  ...
	  if (str2[0]=='@')
	  {
	    ...
	    w_cre_ci_button(p_wptr,str1,SMG_SIZE,p6,p7,p8,i);
  ...
  {                                                                                       
	...
	if (p5==1)
	{
	  ...
	  else
	  {
	    if (bw_inkey1.strempty(str2,FLD_NAME_SIZE)==1)
	    {
		w_cre_ci_radio(p_wptr,p6,p7,p8,i);
  ...
  {                                                                                       
	...
	if (p5==1)
	{
	  ...
	  else
	  {
	    if (bw_inkey1.strempty(str2,FLD_NAME_SIZE)==1)
	    {
	      w_cre_ci_check(p_wptr,p6,p7,p8,i);
...
int bw_win::w_field_info2(int p_wptr)
{
  ...
  for (i=0;i<LIN_NUM;i++)
  {
	...
	if ((str2[0]==':')||(str2[0]=='%')||(str2[0]=='+')||(str2[0]=='-'))
	{
	  ...
	  l=0;
	  for (k=bw_buff1.get_t3_linecol(m,i,j)-1;k>=get_mn_ptr_fld_in_win(p_wptr,0);k--)
	  {
