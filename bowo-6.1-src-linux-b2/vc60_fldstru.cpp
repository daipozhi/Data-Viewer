...
int bw_fldstru::w_echo_winrec(int p_wptr)
{
  ...
  for (i=0;i<STRU_NUM;i++)
  {
    ...
    //printf("wins dbf=%d,lnk tab=%d,\n",bw_win1.get_mn_ptr_wins_dbf(i,p_wptr),bw_xbase1.get_mn_lnktab(i));
  ...
  for (k=0;k<STRU_NUM;k++)
  {
	...
	for (j=0;j<COL_NUM;j++)
	{
	  if (bw_buff1.get_t3_linecol(p_wptr,i,j)!=0)
	  {
        ...
        //printf("in paint win,fld=%s,",s_tabfld);
