...
*/
	...
	k=bw_link1.w_link_modi(bw_main1.mn_hnd_id);  /* fetch link var in condition & if link var modified */
      ...
      bw_getread1.set_smg_posi(bw_main1.mn_hnd_id,1);  //init caret position
...
// --- SetFocus() after wm_create,wm_paint,wm_setfocus ---
	  ...
	  SetFocus(bw_win1.get_mn_hnd(i));
        ...
        if ((s_tabfld[0]=='?')||(s_tabfld[0]=='-')||(s_tabfld[0]==':')||(s_tabfld[0]=='+'))
        {
          ...
          //HideCaret(bw_main1.get_mn_hwnd(bw_main1.mn_hnd_id));
          SetCaretPos((j-bw_win1.get_mn_con(bw_main1.mn_hnd_id,0)+bw_getread1.get_smg_posi(bw_main1.mn_hnd_id)-1)*bw_main1.mn_edit_xchar,
...
// --- can't call get_read when set_focus ---
	...
	if ((s_tabfld[0]=='?')||(s_tabfld[0]=='-')) // memory var field
	{
	  ...
	  if (k==1)
	  {
	    ...
	    if (  (bw_main1.mn_key==274)||(bw_main1.mn_key==275)
	        ...
	        ||(bw_main1.mn_key==RET_KEY)  )
	    {
	      bw_getread1.set_smg_posi(bw_main1.mn_hnd_id,1);
	    }
	...
	if ((s_tabfld[0]==':')||(s_tabfld[0]=='+')) // database field
	{
	  ...
	  if (k==1)
	  {
	    ...
	    if ((bw_main1.mn_key==CTRL_D)||(bw_main1.mn_key==CTRL_I)
	      ||(bw_main1.mn_key==PG_UP) ||(bw_main1.mn_key==PG_DOWN)
	    ...
	    if (bw_main1.mn_key==276) bw_win1.w_left_field(bw_main1.mn_hnd_id);
	    if (bw_main1.mn_key==277) bw_win1.w_right_field(bw_main1.mn_hnd_id);
	    if (bw_main1.mn_key==PG_UP)   bw_fldstru1.w_page_up(m,bw_main1.mn_hnd_id);
	    if (bw_main1.mn_key==PG_DOWN) bw_fldstru1.w_page_down(m,bw_main1.mn_hnd_id);
	    ...
	    if (bw_main1.mn_key==F3_KEY)
	    {
	      bw_win1.get_ms_window_name(bw_main1.mn_hnd_id,s_fn1,FN_SIZE);
	      bw_print1.w_print(bw_main1.mn_hnd_id,s_fn1,FN_SIZE);
            }
	    if (bw_main1.mn_key==F1_KEY)
	    {
	      if (bw_fldstru1.w_find_fldtype(s_fldname,SMG_SIZE,m)=='m')
	      {
	...
	if (wp==1011) bw_main1.mn_key=5000+PG_UP;
	if (wp==1012) bw_main1.mn_key=5000+PG_DOWN;
	...
	if (wp==1099) bw_main1.mn_key=5000+ESC_KEY;

	if (wp==1501) bw_main1.mn_key=5000+CTRL_F;
	if (wp==1502) bw_main1.mn_key=5000+CTRL_C;
	if (wp==1503) bw_main1.mn_key=5000+CTRL_N;

	if (wp==1601) bw_main1.mn_key=5000+F1_KEY;
	if (wp==1602) bw_main1.mn_key=5000+F2_KEY;
	if (wp==1603) bw_main1.mn_key=5000+F3_KEY;
	if (wp==1604) bw_main1.mn_key=5000+F4_KEY;
	if (wp==1605) bw_main1.mn_key=5000+F5_KEY;
	if (wp==1606) bw_main1.mn_key=5000+F6_KEY;

	if (wp==1401) bw_main1.mn_key=5000+CTRL_E;
	if (wp==1499) bw_main1.mn_key=5000+ESC_KEY;
...
long    w_tab_ci_wp(HWND hwnd,WORD msg,WORD wp,LONG lp)
{
  ...
  switch(msg)
  {
	...
	if ((bw_main1.mn_key==274)||
	    ...
	    (bw_main1.mn_key==10 )||
	    (bw_main1.mn_key==27) ||
	    (bw_main1.mn_key==PG_UP)||
	    (bw_main1.mn_key==PG_DOWN)||
	    ...
	    (bw_main1.mn_key==F3_KEY)||
	    (bw_main1.mn_key==F1_KEY)||
	    (bw_main1.mn_key==F2_KEY)||
	    (bw_main1.mn_key==F5_KEY)||
	    (bw_main1.mn_key==F6_KEY))
	{
          bw_win1.mn_menu_seri=j;
...
int bw_main::get_view(char *p_fn,int p_fn_size,HWND hwnd,int ptr)
{
  ...
  {
    //printf("prep to read_winfile,fn=%s,\n",bw_main1.ms_m_bs2);

    i=bw_win1.w_read_winfile(bw_main1.ms_m_bs2,FN_SIZE);
  }
  else
  {
