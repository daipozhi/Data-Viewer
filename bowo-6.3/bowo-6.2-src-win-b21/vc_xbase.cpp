...
int bw_xbase::w_open_xbase(char *p_fn,int p_fn_size,int backup,int p_dbptr)
{
  ...
  while(1)
  {
    mn_fh[p_dbptr]=sopen(s_fn1,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
  ...
  if (n_state1==1)                      // differant structure database
  {
    ...
    else			         // update structure
    {
      ...
      mn_fh[p_dbptr] = sopen(s_fn1,O_RDWR|O_CREAT|O_BINARY, SH_DENYNO, S_IREAD|S_IWRITE);  // create new dbf
  ...
  if (backup==1)
  {
    ...
    mn_fh[p_dbptr]=sopen(s_fn2,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
...
int bw_xbase::w_fox_use(char *p_fn,int p_fn_size)
{
  ...
  mn_use_fh=sopen(p_fn,O_RDWR|O_BINARY,SH_DENYNO,S_IREAD|S_IWRITE);
