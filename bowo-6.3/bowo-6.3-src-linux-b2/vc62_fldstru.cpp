...
int bw_fldstru::w_read_stru(char *p_fn,int p_fn_size,int p_dbptr)
{
  ...
  while (!feof(fp1))
  {
    ...
    set_c_t2_fldname(ptr1,0,0);

    for (j=p1;j<p2;j++)
    {
      if ((str2[j]>=0)&&(str2[j]<=' ')) break;
      
      set_c_t2_fldname(ptr1,j-p1+0,str2[j]);
      set_c_t2_fldname(ptr1,j-p1+1,0);
    }

    bw_inkey1.cut_string(str2,p4,p5-p4,SMG_SIZE,str1,SMG_SIZE);
    l=bw_inkey1.str2int(str1,SMG_SIZE);

    set_t2_fldtype(ptr1,str2[p3]);

    if ((get_t2_fldtype(ptr1)=='c')||(get_t2_fldtype(ptr1)=='n'))
    {
      set_t2_fldlen(ptr1,l);
    }
    else if (get_t2_fldtype(ptr1)=='m') set_t2_fldlen(ptr1,254);
    else if (get_t2_fldtype(ptr1)=='b') set_t2_fldlen(ptr1,254);
    else if (get_t2_fldtype(ptr1)=='l') set_t2_fldlen(ptr1,1);
    else
    {
      set_t2_fldtype(ptr1,'c');
      set_t2_fldlen(ptr1,l);
    }

    set_t2_flddec(ptr1,0);

    if (get_t2_fldtype(ptr1)=='n')
    {
      bw_inkey1.cut_string(str2,p6,p7-p6,SMG_SIZE,str1,SMG_SIZE);
      l=bw_inkey1.str2int(str1,SMG_SIZE);
      set_t2_flddec(ptr1,l);
    }

    ptr1++;
  }

  set_t2_fldpntr(p_dbptr,1,ptr1);
...
int bw_fldstru::w_dele_stru(int p_dbptr)
{
  ...
  while (n1>0)
  {
    ...
    for (j=get_t2_fldpntr(p3,0);j<get_t2_fldpntr(p3,1);j++)
    {
      ...
      set_t2_flddec(j-n1,get_t2_flddec(j));
  ...
  bw_xbase1.set_mn_fld_cntr(p_dbptr,0);   /* field number */
  bw_xbase1.set_mn_len_cntr(p_dbptr,0);   /* record length */
...
int bw_fldstru::w_find_recno(int p_dbptr,int p_wptr)
{
  ...
  bw_xbase1.mn_fh2[p_wptr]=/*s*/open(s_fn1,O_RDWR|O_CREAT/*|O_BINARY,SH_DENYNO*/,S_IREAD|S_IWRITE);
